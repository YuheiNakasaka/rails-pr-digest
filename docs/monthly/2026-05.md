...
## [#57438](https://github.com/rails/rails/pull/57438) Preserve IPv6 prefix when dumping PostgreSQL `cidr`/`inet` defaults to schema.rb {#pr-57438}

**マージ日**: 2026/5/30 | **作成者**: [@55728](https://github.com/55728)
...
1. `db:schema:dump` で `::/32` が `"::"` として `schema.rb` に書き出される
2. その後 `db:schema:load` で再読込すると、`"::"` は `IPAddr.new("::")` により `::/128` として解釈される
...
`OID::Inet` は `OID::Cidr` を継承しており `type_cast_for_schema` をオーバーライドしていないため、この修正は `cidr` と `inet` 両方の型に効きます。
...
- IPv6 で:
  - `/32` や `/10`, `/0` のような部分プレフィックスがそのまま保持されること
  - `/128` のときだけ省略されること
...
- 影響を受けるケース:
  ...
  - これまで `db:schema:dump` → `db:schema:load` の往復で、元のデフォルト値から異なる値が設定されていた可能性があります。
- 具体的な問題例:
...
アップグレード時に、もし `schema.rb` に IPv6 のデフォルトがある場合は、**ダンプ結果の差分**を一度確認すると安心です。「これまで silent に壊れていたものが正しく直る」という修正なので、通常は望ましい差分だけのはずです。
...
## [#57506](https://github.com/rails/rails/pull/57506) Fix NullPool#server_version deadlock {#pr-57506}

**マージ日**: 2026/5/29 | **作成者**: [@hmcguire-shopify](https://github.com/hmcguire-shopify)
...
- プール側の `#server_version` は同期化されたメソッド:
  - 通常の `ConnectionPool`:
    - ロックに `Monitor` を使用（**再入可能**なので、同じスレッドがネストしてロックを取り直しても OK）
  - 単体接続用の `NullPool`:
    - ロックに `Mutex` を使用（**再入不可**のため、同じスレッドで `server_version` を再入するとデッドロック）
...
- `NullPool` が利用する同期プリミティブを `Mutex` → `Monitor` に変更:
  - これにより、`server_version` 呼び出し中に `connect!` → `configure_connection` → `check_version` → 再度 `server_version` という再入が起きても、同一スレッドであればロックが再取得可能になりデッドロックが発生しなくなる。
...
- パフォーマンス/互換性:
  - `Mutex` → `Monitor` への変更は Ruby の標準的な同期プリミティブの置き換えであり、一般的なユースケースで性能面の顕著な悪化は想定しにくい
...
## [#57475](https://github.com/rails/rails/pull/57475) Active Record Ractor-safe freezing {#pr-57475}

**マージ日**: 2026/5/29 | **作成者**: [@gmcgibbon](https://github.com/gmcgibbon)
...
- **対象はほぼ内部実装**であり、一般的な Rails アプリで直接触れる API（`Model.where` など）の表面的な挙動は変わらない想定です。
...
- 類似 PR:  
  - https://github.com/rails/rails/pull/57323  
    → 同じく「Ractor セーフティ向上のために freeze/constantize を進める」系の変更。
- Ractor の制約（公式ドキュメント）:  
  - Ruby 3.0 Ractor 概要: https://docs.ruby-lang.org/ja/latest/doc/spec=2fractor.html  
    - 「共有できるオブジェクトは基本的に『凍結された（freeze 済みの）オブジェクト』に限られる」などの制約が説明されています。
- 実務的には:
...
## [#57478](https://github.com/rails/rails/pull/57478) Acquire reaper_lock in preconnect to avoid racing with the reaper {#pr-57478}

**マージ日**: 2026/5/29 | **作成者**: [@yahonda](https://github.com/yahonda)

1. 概要 (1-2文で)  
ActiveRecord の接続プールで `preconnect` 実行時に `reaper_lock` を取得するようにし、リーパー（メンテナンス用スレッド）との競合状態（race condition）で CI の `test_preconnect` が落ちる問題を解消する PR です。`disconnect` や `discard!` と同じ「プール全体に影響する操作は reaper_lock で囲む」という設計に揃えています。
...
`ConnectionPool#preconnect` を `reaper_lock` で囲むように変更しています。
...
- 逆に、ユーザースレッドからの `preconnect` は `reaper_lock` を取りに行くため、reaper がメンテナンス中であれば待機し、コネクションが中途半端な状態のまま処理されることを防ぎます。
...
- **デッドロックの懸念**
  - `reaper_lock` はすでに `disconnect` や `discard!` 等で使われており、そこに `preconnect` が加わるだけなので、ロック設計としては既存方針の拡張です。
  - `Monitor` ベースで再入可能な点と、reaper 側がもともと `reaper_lock` → `preconnect` の順で呼んでいる点から、新たなロック順序逆転は発生していません。
...
- デバッグ用ブランチ（問題再現とログ出力付き）:  
  - https://github.com/yahonda/rails/tree/fix-preconnect-reaper-lock-race-debug
- 対象テスト:  
...
## [#57501](https://github.com/rails/rails/pull/57501) Return a completed promise for async_count with limit(0) {#pr-57501}

**マージ日**: 2026/5/28 | **作成者**: [@fallintoplace](https://github.com/fallintoplace)

1. 概要 (1-2文で)  
`Relation#async_count` が `limit(0)` のときに整数 `0` を直接返してしまう不整合が修正され、常に `ActiveRecord::Promise` を返すようになりました。これにより、`async_count` の戻り値で `value`, `then`, `pending?` などの Promise API を一貫して利用できます。
...
内部的には、`execute_simple_calculation` の `limit(0)` での早期 return の取り扱いを調整し、「async の場合は早期 return も Promise で包まれる」ように揃えています。  
...
- 該当ファイル:
  ...
  - `activerecord/test/cases/calculations_test.rb`
    - `test_count_should_shortcut_with_limit_zero` に `async_count` 分の検証が追加
...
## [#50979](https://github.com/rails/rails/pull/50979) Action Cable server adapterization {#pr-50979}

**マージ日**: 2026/5/28 | **作成者**: [@palkan](https://github.com/palkan)
...
- `ActionCable::Server::Socket` およびそのサブクラス群は「実際のトランスポート実装」を担当。
  ...
  - `Stream`、`MessageBuffer`：ソケット I/O のためのバッファリング・ストリーム処理
...
- PR 本文での詳細な背景説明・設計意図（外部ドキュメント）  
  - https://docs.google.com/document/d/1oGf86NAzQL9sDw6rTfUNSe3mzzg7wX64sc34D-_3EDI/edit#heading=h.d3db1fd03ano
- actioncable-next gem（この PR の機能を Rails 7+ で先行利用可能）  
  - https://github.com/anycable/actioncable-next
- playground（実験用・サンプル・ベンチマーク）  
  - https://github.com/anycable/action-cable-next-playground
- Action Cable over SSE のスケッチ実装  
  - https://gist.github.com/palkan/270a192e79b05d5601fe497ad3ec83b5
- Iodine サーバーアダプタ実装例  
  - https://github.com/anycable/action-cable-next-playground/blob/master/lib/servers/iodine.rb
- AnyCable Rails 統合の新バージョン（本 PR ベース）  
  - https://github.com/anycable/anycable-rails/pull/205
- rspec-rails 側の対応 PR  
  - https://github.com/palkan/rspec-rails/pull/1
...
## [#57268](https://github.com/rails/rails/pull/57268) Fix: registers ERBTracker instead of RubyTracker when eager_load = true {#pr-57268}

**マージ日**: 2026/5/28 | **作成者**: [@joaoGabriel55](https://github.com/joaoGabriel55)
...
- 外側の `after_initialize` が完了した**後**
- すでに eager load 等の処理が終わった**後**

に `ActionView.render_tracker` が設定されていました。
...
より**前に** `ActionView.render_tracker` が設定されるようになる。
...
- 注意点
  - 挙動が「後ろにずれる」のではなく「前にくる」方向の変更なので、`after_initialize` の中で `ActionView.render_tracker` に依存していたコードがある場合、今回の修正で**むしろ安定**する方向の変更です。
...
## [#39525](https://github.com/rails/rails/pull/39525) Add support for default order {#pr-39525}

**マージ日**: 2026/5/28 | **作成者**: [@udan11](https://github.com/udan11)
...
`finder_methods.rb` / `relation.rb` への変更により、`default_order` が `first`, `last`, `take`, `find_by` などの finder メソッドを呼び出した際にも適切に考慮されるようになっています。
...
- PR 本体: https://github.com/rails/rails/pull/39525  
- 提案ディスカッション (Rails Forum):  
...
## [#53101](https://github.com/rails/rails/pull/53101) [Fix #52229] Respect `#reject` in Action Cable `before_subscribe` callbacks {#pr-53101}

**マージ日**: 2026/5/28 | **作成者**: [@joshuay03](https://github.com/joshuay03)
...
- 影響を受けるのは、**Action Cable のチャンネルで `before_subscribe` を使い、その中で `reject` を呼んでいる場合**です。
- これまで:
  - `before_subscribe` で `reject` しても `subscribed` が呼ばれてしまっていたため、
    - `subscribed` 内で「購読成功前提」の処理（DB 更新・ログ・他チャンネルへのブロードキャストなど）を書いていると、実は reject されているのに動いてしまう、というバグがありました。
- 修正後:
  - `before_subscribe` で `reject` すると、その後の `subscribed` は確実に呼ばれません。
...
- 対応 Issue: https://github.com/rails/rails/issues/52229  
- PR 本体: https://github.com/rails/rails/pull/53101  
- 関連コンポーネント:  
  - `ActionCable::Channel::Base`  
  - `ActionCable::Channel::Callbacks`  
  - コールバック種別: `before_subscribe` / `subscribed` / `reject`（`reject_subscription`）

---


## [#57500](https://github.com/rails/rails/pull/57500) Fix Array#to_query producing double && when array contains empty hash {#pr-57500}

**マージ日**: 2026/5/28 | **作成者**: [@bogdan](https://github.com/bogdan)

1. 概要 (1-2文で)  
Array に空 Hash (`{}`) が含まれている場合に `Array#to_query` が `&&` を連続させた不正なクエリ文字列を生成してしまうバグを修正した PR です。空 Hash 由来の空文字列を結合前に捨てることで、仕様に沿った `application/x-www-form-urlencoded` 形式になるようにしています。
...
ここで `a%5B%5D` は `a[]` を URL エンコードした文字列です。  
...
## [#57467](https://github.com/rails/rails/pull/57467) Add shims for Ractor shareability methods {#pr-57467}

**マージ日**: 2026/5/28 | **作成者**: [@andrewn617](https://github.com/andrewn617)
...
2. **no-op 実装であることの意味**  
   ...
   - それでもコードは「動いてしまう」  
   ため、**Ractor あり／なしで挙動（特に並行性まわり）が変わる**可能性があります。  
...
## [#57493](https://github.com/rails/rails/pull/57493) [ci skip] Add step to create path for redirection before launching the application {#pr-57493}

**マージ日**: 2026/5/28 | **作成者**: [@maurobusso](https://github.com/maurobusso)
...
## [#57483](https://github.com/rails/rails/pull/57483) Make exception wrapper ractor safe {#pr-57483}

**マージ日**: 2026/5/28 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
Rails の `ActionDispatch::ExceptionWrapper` を Ractor セーフにするため、内部で保持している例外関連の設定ハッシュを凍結・再生成する形に変更し、あわせて `wrapper_exceptions` と `silent_exceptions` に対する公式な設定インターフェースが追加されました。これによりマルチ Ractor 実行時のスレッドセーフ性を高めつつ、例外ハンドリングをより柔軟に設定できるようになっています。
...
### 2-3. `wrapper_exceptions` と `silent_exceptions` の公開設定追加

これまで `wrapper_exceptions` と `silent_exceptions` は内部定義のみで、`config` からカスタマイズするための公式な API が存在しませんでした。
...
2. **ランタイムでの直接ミューテーションは非推奨 (事実上困難に)**  
   ...
   - 推奨パターン: **すべて `config.action_dispatch.*` を通した設定**に切り替えること
...
## [#57344](https://github.com/rails/rails/pull/57344) Update Active Job Attributes documentation {#pr-57344}

**マージ日**: 2026/5/28 | **作成者**: [@bdewater-thatch](https://github.com/bdewater-thatch)
...
- この PR は**コードの挙動にはほぼ影響しないドキュメント/サンプル修正**です。
...
- 元となった PR: https://github.com/rails/rails/pull/57208  
  - Active Job Attributes 機能そのものの追加/改善 PR です。
- PR 内で言及されている混乱の発端:  
  - https://ruby.social/@getajobmike/116540627534276308  
  - Attributes の用途や意図が分かりにくかったというフィードバックを受けて、  
    ドキュメントを明確化したのが本 PR の背景です。

---


## [#57496](https://github.com/rails/rails/pull/57496) Fix docs for ActiveSupport::SecurityUtils.fixed_length_secure_compare {#pr-57496}
...
- `fixed_length_secure_compare` に対応するコメントブロックを、**最初のメソッド定義の直前**に移動しただけです。
- その結果、下記URLにある `fixed_length_secure_compare` の説明が正しく出るようになります。  
...
- 関連PR（作業中と書かれているもの）:  
  ...
  （こちらの作業中PRの中でドキュメント表示の不具合に気づき、本PRで切り出して修正した形と思われます）

---


## [#57489](https://github.com/rails/rails/pull/57489) Improve documentation/testing of abort in Action Mailer `before_action {#pr-57489}

**マージ日**: 2026/5/28 | **作成者**: [@bensheldon](https://github.com/bensheldon)
...
- **機能追加や挙動変更ではなく、「ドキュメントの修正」と「テストの追加」が中心**です。
...
- ガイド: [Action Mailer Basics (edgeguides)](https://edgeguides.rubyonrails.org/action_mailer_basics.html)  
  - 本 PR マージ後、このガイドの「コールバック」「before_action / abort」に関する節が最新の内容になります。

---


## [#57495](https://github.com/rails/rails/pull/57495) Reject malformed Action Mailbox raw email params {#pr-57495}

**マージ日**: 2026/5/28 | **作成者**: [@afurm](https://github.com/afurm)
...
- **影響するのは Action Mailbox を Mailgun / Postmark / SendGrid ingress で利用しているアプリケーション**です。
- これまでは:
...
- Webhook のレスポンスコードに応じた **リトライ制御やアラート連携**を行っている場合、  
  - 以前: サーバ内部エラー（500 系）として扱っていたかもしれない  
  - 今回以降: 422 でクライアント側（送信側）起因の不正データとみなされる  
  ため、**アラート閾値やリトライポリシーを 422 向けに見直す**必要があるかもしれません。
...
## [#57110](https://github.com/rails/rails/pull/57110) Reduce number of queries during MariaDB column introspection {#pr-57110}

**マージ日**: 2026/5/28 | **作成者**: [@ilianah](https://github.com/ilianah)
...
- その結果、**カラムごとに `SHOW CREATE TABLE` を実行**する実装になっており、  
...
- フローとしては次のように変更:
  ...
  2. 接続先が MariaDB の場合のみ、取得したフィールドの `Default` を走査し、**関数呼び出しのように見える値**があるかを確認。
  ...
  5. こうしてマークされたカラムは、既に MySQL 側で使われている `new_column_from_field` の `DEFAULT_GENERATED` 分岐に乗るようになり、MariaDB でも正しく「関数デフォルト」として扱われる。
...
- 対象DB:
  - **MariaDB 接続時**の `columns` / スキーマ introspection 処理が主対象。
...
## [#57470](https://github.com/rails/rails/pull/57470) Fix `number_to_delimited` mangling non-finite floats {#pr-57470}

**マージ日**: 2026/5/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveSupport::NumberHelper.number_to_delimited` が `Float::INFINITY` などの非有限浮動小数点数を不正に区切ってしまうリグレッションを修正し、非有限値はそのまま文字列表現を返すようにした PR です。これにより `"Infinity"` が `"In,fin,ity"` になる問題が解消されます。
...
- **他 API との整合性**
  - `number_to_human` / `number_to_human_size` は `Float::INFINITY` に対して `FloatDomainError` を投げ続けており、この PR では変更されていません（追って検討する候補として #57451 で言及）。
...
## [#57451](https://github.com/rails/rails/pull/57451) Format non-finite numbers consistently in NumberHelper significant mode {#pr-57451}

**マージ日**: 2026/5/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveSupport::NumberHelper` の `number_to_rounded` などで `significant: true` を指定した際、`Infinity` / `-Infinity` / `NaN` を渡すと `FloatDomainError` で落ちていた不具合を修正し、非 significant モードと同様に `"Inf"` / `"-Inf"` / `"NaN"` として一貫してフォーマットされるようにした PR です。  
...
- もともと 0 の場合も `digit_count` は `1` を返しており、その分岐は今回の変更より前に評価されるため、以前正常に動いていたケースはそのまま維持される
...
- `number_to_rounded` に `Infinity`, `-Infinity`, `NaN` を渡し、`significant: true` でそれぞれ `"Inf"`, `"-Inf"`, `"NaN"` になること
...
これらは `RoundingHelper` 経由ではなく、独自の `calculate_exponent` や `exponent` の中で `Math.log10` / `Math.log` を直接呼び出しているため、`Infinity` を渡すと引き続き `FloatDomainError` になる可能性があります。  
...
- 本 PR で参考にされている旧 PR:  
  - 2016 年の試み #25946  
    - `number_to_rounded` / `number_to_percentage` / `number_to_human` を一気に直そうとして、`number_to_human` の `"Inf"` / `"NaN"` 表現が人間向けでないとの指摘でクローズ  
...
## [#57492](https://github.com/rails/rails/pull/57492) Reject malformed Action Mailbox original recipients {#pr-57492}
...
## [#57477](https://github.com/rails/rails/pull/57477) Refactor ActiveJob::TestHelper to no longer walk descendants {#pr-57477}

**マージ日**: 2026/5/28 | **作成者**: [@byroot](https://github.com/byroot)
...
- ポイント:
  - ActiveJob::TestHelper が使う設定のオーバーライド情報を、**専用の内部ハッシュやセット**に保持
...
- ActiveStorage の integration test で
  ...
  - その teardown 時の状態リセット
 などが正常に機能することを保証する意図があります。
...
- 対応 issue:
  - https://github.com/rails/rails/issues/57440  
  - https://github.com/rails/rails/issues/57441  

- 関連するクローズ済み PR:
  - https://github.com/rails/rails/pull/57473  

- 対象コード:
  - `activejob/lib/active_job/test_helper.rb`  
    - 状態管理の仕組みが `class_attribute` ベースから、内部データ構造ベースに刷新
  - `railties/test/application/active_storage/analyzers_integration_test.rb`  
    - Active Storage の integration test が追加され、新実装の回りの動作がカバーされている

---


## [#57481](https://github.com/rails/rails/pull/57481) Simplify more ```activerecord``` tests with ```NotificationAssertions``` helpers {#pr-57481}

**マージ日**: 2026/5/28 | **作成者**: [@Edilbek](https://github.com/Edilbek)

1. 概要 (1-2文で)  
Active Record のテストコードで、`ActiveSupport::Notifications.subscribe` を直接使っていた箇所の多くを、`NotificationAssertions` の `capture_notifications` などのヘルパーに置き換えたリファクタリングです。挙動の変更はなく、テストの可読性・メンテナンス性の向上のみを目的としています。
...
- 既存テストでは、通知の検証に以下のような「手書き subscribe パターン」が多く使われていました:
  ...
  - `ensure` で `unsubscribe` する、または `subscribed` を使う
...
- PR 説明に明記されている通り、**非同期スレッド側で発火する通知を対象にしているテスト**については、引き続き **直接 `ActiveSupport::Notifications.subscribe` を使用** しています。
...
- 注意点 (テストを書く側向け):
  - `capture_notifications` は `ActiveSupport::Notifications.subscribed` ベースであり、**スレッドローカル** であることを意識する必要があります。
...
## [#57485](https://github.com/rails/rails/pull/57485) Reject malformed Mailgun signatures {#pr-57485}
...
`actionmailbox/test/controllers/ingresses/mailgun/inbound_emails_controller_test.rb` に、**署名パラメータが配列のケース**を再現するテストが追加されています。
...
## [#57087](https://github.com/rails/rails/pull/57087) Only set Postgres timezone when needed {#pr-57087}

**マージ日**: 2026/5/27 | **作成者**: [@matthewd](https://github.com/matthewd)

1. 概要 (1-2文で)  
PostgreSQL アダプタがサーバ側の `TimeZone` 設定を「常に SET する」のではなく、「必要なときだけ変更する」ように調整された PR です。  
あわせて、PostgreSQL の `parameter_status` から取得した `TimeZone` や `variables` を、大文字・小文字の違いに影響されない（case-insensitive）形で扱うようになっています。
...
- Active Record 側が期待するタイムゾーン（例: `ActiveRecord::Base.default_timezone` や `config.time_zone` 等から導かれる値）と比較。
...
## [#56340](https://github.com/rails/rails/pull/56340) [RF-Docs] [ci-skip]  Active Storage Guide {#pr-56340}

**マージ日**: 2026/5/27 | **作成者**: [@bhumi1102](https://github.com/bhumi1102)
...
### 2.4 `has_many_attached` と `.attach` の挙動の訂正・明確化  
...
- **主な影響は「ドキュメントのみ」**であり、Active Storage の API や既存の挙動が変わる PR ではありません。  
  - ただし、これまで誤解されやすかったポイント（`has_many_attached` の `.attach` の挙動、proxy/redirect の違いなど）が明確化されているため、「今まで思い込みで使っていた」部分が修正される可能性はあります。  
- **新しいオプションの把握**  
...
- 該当 PR: https://github.com/rails/rails/pull/56340  
- ガイド（最新版）:
  - Active Storage Overview: `guides/source/active_storage_overview.md`（HTML 版は https://guides.rubyonrails.org/active_storage_overview.html に反映予定）  
  - Testing: `guides/source/testing.md`（HTML 版は https://guides.rubyonrails.org/testing.html）  
- 関連する概念:
  - Polymorphic Associations: https://guides.rubyonrails.org/association_basics.html#polymorphic-associations  
  - Active Storage API ドキュメント: https://api.rubyonrails.org/classes/ActiveStorage.html
