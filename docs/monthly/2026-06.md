...
## [#57913](https://github.com/rails/rails/pull/57913) Keep nil elements named in the series for `in_order_of` {#pr-57913}

**マージ日**: 2026/6/29 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Enumerable#in_order_of` を `filter: true`（デフォルト）で使ったとき、ソート順を指定する配列（series）に `nil` が含まれていても、これまでは結果から `nil` が落ちていましたが、PRにより明示的に指定された `nil` を結果にも残すように修正されました。これにより、`filter: true` と `filter: false` での挙動が `nil` の扱いに関して一貫します。
...
`Enumerable#in_order_of` は、指定したキーと順序リスト（series）に従って要素を並べ替えるメソッドです。
...
しかし、これまでデフォルトの `filter: true` では、series に `nil` を含めていても結果から `nil` が削除されていました。

```ruby
[3, nil, 1, 2].in_order_of(:itself, [1, nil, 2, 3])
# Before: [1, 2, 3]   # nil が落ちる
```

一方で、`filter: false` では `nil` を正しく保持していました。

```ruby
[3, nil, 1, 2].in_order_of(:itself, [1, nil, 2, 3], filter: false)
# => [1, nil, 2, 3]
```
...
- 対象ファイル: `activesupport/lib/active_support/core_ext/enumerable.rb`
  - `in_order_of` の内部実装が 1 行修正され、`filter: true` のときでも、series に明示的に含まれている `nil` を結果から除外しないような条件（またはフィルタ処理）が調整されています。
  - 元々「filter: true のときは series に含まれない値を結果から落とす」というロジックがありますが、その過程で `nil` も一緒に落ちていたため、series 上の `nil` は特別扱いして保持するようにしたと考えられます。

- テスト追加: `activesupport/test/core_ext/enumerable_test.rb`
  - `filter: true` でも `nil` を含む series を正しく再現するテストが 5 行分追加。
  - 想定されるテストイメージ（PR説明から推測される形）:

    ```ruby
    def test_in_order_of_preserves_nil_with_filter_true
      assert_equal(
        [1, nil, 2, 3],
        [3, nil, 1, 2].in_order_of(:itself, [1, nil, 2, 3])
      )
...
- `filter: true`（デフォルト）でも、series に含まれている `nil` が結果に現れるようになりました。
- `filter: false` の挙動はもともと正しかったため、そのまま維持され、両者で `nil` の扱いが揃います。
...
- **挙動の変化（互換性）**  
  - これまで Rails の `in_order_of` を利用していて、`filter: true` かつ series に `nil` を含めていた場合、結果から `nil` が落ちている前提でコードを書いていたとすると、今回の変更により結果に `nil` が含まれるようになります。
  - 特に「`nil` は来ないはず」と決め打ちしている部分（例: `compact` していないループ処理や、`nil` 非許容の変換処理など）は、アップグレード時に再確認が必要です。

- **意図的な使い方との整合性**  
  - series に `nil` を書いている時点で「この位置に `nil` も並べ替え対象として含めたい」という意図が明確なため、今回の修正はより直観的で一貫した仕様になっています。
  - もし本当に `nil` を結果から除外したいのであれば、series に `nil` を含めない、あるいは呼び出し側で `compact` するなどの明示的な処理を行うべき、という方向に仕様が整理されたといえます。

- **filter オプションの意味**  
  - `filter: true` は「series に存在しない値を結果から落とす」というオプションであり、「nil を落とす」という意味ではない、という点が今回の変更でより明確になります。
  - 「series にないものだけを除外し、series に書いたもの（`nil` を含む）は尊重する」というルールとして理解すると扱いやすいです。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57913  
- 関連する API:  
  - `Enumerable#in_order_of`（Active Support 拡張）  
    - 典型的な使い方例:

      ```ruby
      records = User.where(id: [3, nil, 1, 2]) # 実例では nil は通常含まれないが、値としての nil を想定した例
      order   = [1, nil, 2, 3]

      records.in_order_of(:id, order)
      # order の順に並び替え、今回の変更により nil も order 通りに扱われる
      ```

  - Active Support の Enumerable 拡張全般は `activesupport/lib/active_support/core_ext/enumerable.rb` に実装されており、`in_order_of` もそこに含まれます。

---


## [#57915](https://github.com/rails/rails/pull/57915) Include call options in `Cache#exist?` instrumentation payload {#pr-57915}

**マージ日**: 2026/6/29 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Rails.cache.exist?` 実行時に発火する `cache_exist?.active_support` の通知ペイロードに、これまで含まれていなかったオプション引数（namespace, version, expires_in など）が含まれるようになりました。これにより、他のキャッシュ操作（read/write/delete/fetch_hit）と同じ情報が計測・ロギングで利用できます。
...
- 対象の通知イベント: `cache_exist?.active_support`
- 変更前: `Rails.cache.exist?("key", namespace: "foo")` を呼び出しても、通知イベントの `event.payload` には `:options`（およびその中の `:namespace` など）が含まれていなかった。
- 変更後: `exist?` 呼び出し時に渡されたオプションが、そのまま計測用ペイロードに流れるようになった。

PR 説明のコード例をベースにすると、以下のような変化があります。

```ruby
ActiveSupport::Notifications.subscribe("cache_exist?.active_support") do |event|
  # 変更前
  event.payload[:key]       # => "key"
  event.payload[:store]     # => #&lt;ActiveSupport::Cache::Store ...>
  event.payload[:namespace] # => nil （オプションが渡されていない）

  # 変更後
  event.payload[:key]       # => "key"
  event.payload[:store]     # => #&lt;ActiveSupport::Cache::Store ...>
  event.payload[:namespace] # => "foo"  # options が反映される
end

Rails.cache.exist?("key", namespace: "foo")
```

内部的には、`ActiveSupport::Cache` の計測処理で `exist?` に渡されたオプション引数をペイロードに追加しているだけで、キャッシュの挙動（ヒット/ミスの判定やキャッシュストアの動き）は変わっていません。

### 他の操作との一貫性

- すでに以下のイベントのペイロードにはオプションが含まれています:
  - `cache_read.active_support`
  - `cache_write.active_support`
  - `cache_delete.active_support`
  - `cache_fetch_hit.active_support`
- 今回の変更で `cache_exist?.active_support` も同様にオプションを含むため、計測/ロギング/トレース処理をこれらイベント間で共通化しやすくなります。

### テストの追加

`activesupport/test/cache/behaviors/cache_instrumentation_behavior.rb` にテストが追加され、`exist?` 実行時の通知ペイロードにオプションが含まれることが検証されています。  
内容としては、おおよそ次のようなことを確認しています:

- `Rails.cache.exist?` をオプション付きで呼んだとき、
- 発火した `cache_exist?.active_support` イベントのペイロードに、そのオプションが含まれていること。
...
- **互換性**
  - 破壊的変更ではありません。既存コードはそのまま動作します。
  - すでに `cache_exist?.active_support` を購読しているコードは、追加情報（options/namespace/version/expires_in など）を新たに利用できるようになります。

- **モニタリング／ロギングへの影響**
  - `exist?` のイベントペイロードをログ出力している場合、出力内容にオプションが増える可能性があります。
  - namespace や version ごとのキャッシュ利用状況をトレースしやすくなります。
  - カスタムメトリクス収集（Prometheus, Datadog など）で、他の cache_* イベントと同様の次元（タグ）を付与しやすくなります。

- **パフォーマンス**
  - 追加されるのは Hash の受け渡しのみで、非常に軽微な変更です。パフォーマンス影響はほぼ無視できるレベルと考えてよいです。

- **セキュリティ／プライバシー**
  - もともとオプションに機密情報を載せている場合、それが通知経由で外部ロガーや APM に送られやすくなります。
  - `namespace` や独自オプションに機微な情報を入れているプロジェクトは、通知購読側でマスキングやフィルタリングを検討してください。

---

4. 参考情報 (あれば)  

- 関連するイベントの一覧（ActiveSupport::Notifications 経由で購読可能）:
  - `"cache_read.active_support"`
  - `"cache_write.active_support"`
  - `"cache_delete.active_support"`
...
ActiveSupport::Notifications.subscribe(/cache_.*\.active_support/) do |name, start, finish, id, payload|
  # name: "cache_exist?.active_support" など
  # payload 例:
  # {
  #   key: "key",
...
この PR により、`exist?` についても他の cache 操作と同じ粒度で計測・分析ができるようになっています。

---


## [#57911](https://github.com/rails/rails/pull/57911) Allow Arel to be required on its own {#pr-57911}

**マージ日**: 2026/6/29 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`require "arel"` を Active Support なしで実行すると `ActiveSupport::Ractors` の `NameError` が発生していた問題を、Arel 側で明示的に `active_support/ractors` を読み込むことで解消した PR です。Arel を Active Record 抜きで単独利用するユースケースが再び正常に動くようになります。
...
以下のように、Active Record/Active Support を事前に読み込まずに Arel だけを require するとエラーになっていました。
...
- `Arel::Visitors::ToSql` および `Arel::Visitors::PostgreSQL` のクラス定義時に `ActiveSupport::Ractors` を参照している
- `ActiveSupport::Ractors` は Active Support によって autoload されるため、「Active Record 経由で Arel をロードする」場合は問題にならない
- しかし `require "arel"` 単体では Active Support がロードされていないため、`ActiveSupport` 定数が未定義で `NameError` になる

### 対応内容

`activerecord/lib/arel.rb` で、Arel ロード時に `active_support/ractors` を明示的に require するようにしました。
...
# 既存の require 群のどこかに追記
require "active_support/ractors"  # ← これを追加
```

これにより:

- `require "arel"` した時点で `ActiveSupport::Ractors` が定義される
- `Arel::Visitors::ToSql` / `PostgreSQL` のクラス定義内で `ActiveSupport::Ractors` を参照しても `NameError` にならない

### テストの追加

`activerecord/test/cases/arel/require_test.rb` が新規追加されています。

テストのポイント:

- 新しいプロセスを起動し、Active Support を事前にロードしない状態で `require "arel"` を実行
- ロードに成功し、例外が発生しないことを確認
- gem のロードパスのみ有効にした環境を再現しているため、「ライブラリが Arel を直接 require する」現実的ユースケースをカバー

サンプルとなるテストのイメージ（擬似コード）:

```ruby
def test_require_arel_without_activesupport
  # Bundler / gem path だけ有効にしたサブプロセスを起動し
  # ActiveSupport を require せずに arel だけ require しても落ちないことを確認
...
- **影響を受ける利用者**
  - Arel を Active Record 抜きで直接使っており、かつプロセス起動直後に `require "arel"` しているようなライブラリ／アプリケーション
    - 例: SQL を Arel で構築・解析するミドルウェアやツール（`arsi` など）

- **期待される改善**
  - これらのライブラリが、Rails 本体のロード状況に依存せずに `require "arel"` だけで動くようになる
  - Edge Rails で `ActiveSupport::Ractors` 参照部分が増えたことによって壊れていたテストスイート（報告例: `arsi`）が再び通る

- **Active Support への依存の明示化**
  - 実質的に Arel が Active Support（少なくとも `active_support/ractors`）に依存していることがコード上でも明示されました
  - すでに実装上依存していたものを require で追従した形なので、実質的な依存関係は変わっていません

- **注意点**
  - 「Arel は ActiveSupport 非依存であるべき」と考えている場合、この PR により依存がより表面化したと言えます
  - ただし現状、`ActiveSupport::Ractors.shareable_proc` を利用している限り、依存自体は避けられないため、現実的な修正です
  - `active_support` のロード順に依存した微妙なバグ（例: `ActiveSupport::Ractors` の再定義など）がもしあれば影響しうるものの、そのような使い方は通常想定されません
...
- 元 issue: rails#57840  
  - `arsi` (https://github.com/zendesk/arsi) のテストが edge Rails で `require "arel"` に失敗し始めたことから発覚
- この PR により、「Arel は Active Record とは別に require できる」ことが再び保証されるため、Arel 直利用のライブラリは edge Rails 追随時にこの修正が入っているか確認すると良いです。

---


## [#57907](https://github.com/rails/rails/pull/57907) Return a composite has_and_belongs_to_many association_foreign_key as an array {#pr-57907}

**マージ日**: 2026/6/29 | **作成者**: [@ollym](https://github.com/ollym)

1. 概要 (1-2文で)  
`has_and_belongs_to_many` で複合キーを指定した場合、reflection から取得できる `association_foreign_key` が、文字列化された 1 つの値ではなく、`foreign_key` と同様に「カラム名文字列の配列」として返るように修正された PR です。これにより、複合 HABTM を introspection（メタ情報の取得）から正しく扱えるようになります。
...
`has_and_belongs_to_many` は元々、オプションとして複合キーを受け取ることができます:

```ruby
has_and_belongs_to_many :tags,
  foreign_key: [:blog_id, :post_id],
  association_foreign_key: [:tag_id, :tenant_id]
```

このとき:

- `Reflection#foreign_key`  
  → `[:blog_id, :post_id]` を受け取って `["blog_id", "post_id"]`（文字列の配列）を返す

一方で:

- `Reflection#association_foreign_key`  
  → オプション値に `to_s` を呼んでいたため  
    `[:tag_id, :tenant_id]` → `"[:tag_id, :tenant_id]"` という 1 つの文字列になっていた

つまり、**実際の関連自体は正常に動くが、reflection API が複合キーの情報を正しく表現できていない**という不一致がありました。  
複合 HABTM の定義を introspection したいコード（メタプログラミングやジェネレータなど）から見ると、`foreign_key` は配列なのに `association_foreign_key` は謎の 1 つの文字列という、扱いづらい状態になっていました。
...
`ActiveRecord::Reflection#association_foreign_key` の実装を `foreign_key` と揃え:

```ruby
ActiveRecord::Key.for(...).name
```

を通して名前を解決するように変更しました。具体的には:

- 以前: オプション値に対して `to_s` していた
- 今回: `ActiveRecord::Key.for(option_value).name` を使う

その結果:

- スカラキー（単一カラム）の場合は、従来どおり単一のカラム名文字列:

  ```ruby
  has_and_belongs_to_many :tags, association_foreign_key: :tag_id

  reflection.association_foreign_key
  # => "tag_id"
  ```

- 複合キーの場合は、`foreign_key` と同様に「カラム名文字列の配列」:
...
- `Sharded::BlogPost` モデルに、複合キーを使った `has_and_belongs_to_many` を追加
  - 例: `has_and_belongs_to_many :tags, foreign_key: [...], association_foreign_key: [...]` といった定義
- reflection テストで、複合 HABTM に対して:
  - `reflection.foreign_key` が配列を返すこと
  - `reflection.association_foreign_key` も配列を返すこと
  を明示的にアサート

これにより、**複合キー対応の一貫性**（#57904 / #57906 の composite `belongs_to` 修正ラインの延長）が担保されています。

---

3. 影響範囲・注意点

- **影響するのは reflection の戻り値のみ**  
  実際の関連の読み書き（レコードの保存・取得）は元々動いており、今回の変更でもその挙動は変わりません。  
  変わるのは `ActiveRecord::Reflection` 経由で取得するメタ情報です。

- **複合 `has_and_belongs_to_many` を introspect しているコードへの影響**  
  もし既存コードが以下のような前提で動いていた場合は、挙動が変わります:
  - `association_foreign_key` は常に `String` である
  - 複合キー指定時でも `"[:tag_id, :tenant_id]"` といった 1 つの文字列をそのままパースして使っている

  このようなコードは、今後は **配列 or 文字列の両方に対応する**必要があります:

  ```ruby
  afk = reflection.association_foreign_key
  afk = Array(afk) # 単一でも複数でも配列に正規化

  # afk は ["tag_id"] あるいは ["tag_id", "tenant_id"] のような形式
  ```
...
- **新しい期待値**  
  ...
  - HABTM + 単一キー:  
    `reflection.foreign_key` → `"..."`（String）  
    `reflection.association_foreign_key` → `"..."`（String）
  - HABTM + 複合キー:  
    `reflection.foreign_key` → `["...", "..."]`（String の配列）  
    `reflection.association_foreign_key` → `["...", "..."]`（String の配列）

  という形で、`foreign_key` / `association_foreign_key` の仕様が揃います。

---

4. 参考情報 (あれば)

- PR: #57907 — *Return a composite has_and_belongs_to_many association_foreign_key as an array*  
- 関連 Issue: #57908 — `association_foreign_key` が複合キー指定時に `"[:id1, :id2]"` のような String を返してしまう問題  
- 関連 PR:  
  - #57904 / #57906 — composite `belongs_to` 周りの修正・一貫性向上に関する PR  
- 該当コード:  
...
## [#57843](https://github.com/rails/rails/pull/57843) Declare `assert_not_pattern` alias for `refute_pattern` {#pr-57843}

**マージ日**: 2026/6/28 | **作成者**: [@seanpdoyle](https://github.com/seanpdoyle)

1. 概要 (1-2文で)  
Rails の `ActiveSupport::TestCase` に、Minitest の `refute_pattern` に対応するエイリアスメソッド `assert_not_pattern` が追加されました。これにより、他の `assert_not_*` 系メソッドと同じ書き方でパターンマッチングの否定アサーションが使えるようになります。
...
### 何が追加されたか
- `ActiveSupport::TestCase` に以下のエイリアスが定義されました:
  - `assert_not_pattern` → `refute_pattern` のエイリアス

元々 Active Support は以下のように、Minitest の `refute_*` 系メソッドに対して `assert_not_*` のエイリアスを提供していました:
- `assert_not_equal` → `refute_equal`
- `assert_not_includes` → `refute_includes`
- …など

しかし、Minitest 5.18.0 で追加されたパターンマッチング用のアサーション:
- `assert_pattern`
- `refute_pattern`

については、Rails 側に `assert_not_pattern` が存在しておらず、一貫性が欠けていました。この PR はそのギャップを埋えるものです。

### 利用イメージ

Ruby 3 以降のパターンマッチ (`case in`) を使ったテストで、  
「このオブジェクトはこのパターンにマッチ“しない”こと」をテストしたい場合に:

従来 (Minitest を素で使う書き方):
```ruby
refute_pattern({ name: "Alice", age: 30 }, { name:, age: 20..29 })
```

Rails/ActiveSupport のスタイルで書けるようになる:
```ruby
assert_not_pattern({ name: "Alice", age: 30 }, { name:, age: 20..29 })
```

`assert_not_*` を常用している Rails プロジェクトでも、  
書き方の一貫性を保ったままパターンマッチングの否定アサーションを使えます。

### 変更ファイルの概要
- `activesupport/lib/active_support/test_case.rb`
  - `assert_not_pattern` エイリアスの宣言を追加
- `activesupport/test/test_case_test.rb`
  - `assert_not_pattern` が `refute_pattern` と同等の動きをするかを確認するテストを追加
...
- 対象:
  - `ActiveSupport::TestCase` を継承している Rails のテストコード全般
- 影響:
  - 新しいメソッド追加であり、既存メソッドのシグネチャや挙動の変更はないため後方互換性は保たれます。
  - 既存の `refute_pattern` 呼び出しはそのまま動作し、新しく `assert_not_pattern` も利用できるようになります。

- 注意点:
  - `assert_not_pattern` / `refute_pattern` の動作自体は Minitest 5.18.0 以降に依存します。Rails 側からは単にエイリアスを提供しているだけなので、Minitest が古い場合はそもそも `refute_pattern` が存在せず、動作しません。
  - `assert_pattern` / `refute_pattern` のインターフェイスは Minitest 由来です。使い方自体は Minitest のドキュメントに従う必要があります。
...
- Minitest ドキュメント:
  - `assert_pattern`:  
    https://docs.seattlerb.org/minitest/Minitest/Assertions.html#method-i-assert_pattern
  - `refute_pattern`:  
    https://docs.seattlerb.org/minitest/Minitest/Assertions.html#method-i-refute_pattern
- Minitest 5.18.0 リリースノート:  
  https://github.com/minitest/minitest/blob/master/History.rdoc#5180--2023-03-04
- Rails `assert_not_equal` (既存の `refute_equal` エイリアス例):  
  https://edgeapi.rubyonrails.org/classes/ActiveSupport/TestCase.html#method-i-assert_not_equal

---


## [#57896](https://github.com/rails/rails/pull/57896) Apply DISTINCT to a grouped `sum` {#pr-57896}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Relation#sum` に `distinct` と `group` を同時に指定した場合に、内部的に `SUM(DISTINCT column)` ではなく単なる `SUM(column)` が実行されてしまい、数値が誤っていたバグを修正する PR です。未グループのケースで既に行っていた `DISTINCT` フラグ適用処理を、グループ化された集計パスにも適用することで挙動を揃えています。
...
Rails の ActiveRecord で以下のようなコードを書いたとき:

```ruby
Account.where(firm_id: 1).distinct.sum(:credit_limit)
# => 110  # SQL: SELECT SUM(DISTINCT "accounts"."credit_limit") FROM ...
```

これは `SUM(DISTINCT credit_limit)` を実行し、重複値を取り除いた上で合計してくれます。

しかし、`group` を追加した途端に挙動が変わっていました:

```ruby
Account.where(firm_id: 1).group(:firm_id).distinct.sum(:credit_limit)

# 期待される挙動（PR適用後）:
# => {1 => 110}  # firm_id=1 グループ内でも DISTINCT が効いた合計

# PR適用前の挙動:
# => {1 => 160}  # 実際には plain SUM になり、重複がカウントされてしまう
```

- `distinct` を指定しているにもかかわらず、グループ化された集計経路では `DISTINCT` が SQL に反映されていなかったことが原因です。
- `count` については、内部実装が `column.count(distinct)` の形で共通化されていたため、グループ化の有無にかかわらず `DISTINCT` が効いており、この問題は `sum` 特有でした。

### 修正内容

`activerecord/lib/active_record/relation/calculations.rb` に 1 行だけ追加されています。

- 未グループの `sum` 計算パスでは、すでに次のようなロジックがありました（意訳）:
...
- ところが「グループ化あり」のパスにはこの処理が存在せず、`distinct` オプションが SQL の `SUM(DISTINCT ...)` に反映されていませんでした。
- この PR は、その未グループパスと同様に、グループ化された集計パスでも `operation == "sum" && distinct` のときに `select_value.distinct = true` をセットするよう修正しています。
...
Account.where(firm_id: 1).group(:firm_id).distinct.sum(:credit_limit)
# => SELECT "accounts"."firm_id",
#          SUM(DISTINCT "accounts"."credit_limit") AS sum_credit_limit
#    FROM "accounts"
#    WHERE "accounts"."firm_id" = 1
#    GROUP BY "accounts"."firm_id"
#
# 戻り値: {1 => 110}
```
...
`activerecord/test/cases/calculations_test.rb` に約 10 行のテストが追加されています。

- `group` + `distinct` + `sum` の組み合わせで、SQL レベルの `DISTINCT` が効いているかを検証するテストが新設され、修正の回帰防止になっています。
...
- 対象:  
  `ActiveRecord::Relation` に対して

  ```ruby
  relation.group(:some_column).distinct.sum(:other_column)
  ```

  のように「グループ化 + DISTINCT + sum」を組み合わせて使っているコード全般。

- 影響内容:
  - これまで「重複を含めた合計値」が返っていた箇所で、「重複を除いた合計値」が返るようになります。
  - つまり、**既存アプリが、実はバグである“重複込みの合計”に依存していた場合、数値が変わる** 可能性があります。
  - 一方で、API 名 (`distinct.sum`) の意味論としては今回の挙動の方が自然であり、`group` 有無で結果が変わるという一貫性の欠如が解消されます。

- バージョンアップ時の確認ポイント:
  - 分析・レポート・ダッシュボード系コードで、`group(...).distinct.sum(...)` を使っている箇所があれば、期待している値が「重複を含めた合計」なのか「重複を除去した合計」なのかを確認してください。
  - 同様の処理を DB 生 SQL で書いていて、`SUM(DISTINCT ...)` ではなく `SUM(...)` をあえて使っている場合は、ActiveRecord の方も `distinct` を外して挙動を揃える必要があるかもしれません。

- 互換性:
  - 仕様としては「`distinct.sum` が `SUM(DISTINCT ...)` になる」のが自然であり、かつ未グループのケースとはすでにそうなっていたため、**バグ修正による挙動の是正** という位置づけです。
  - `count` についてはもともとグループ有無を問わず `DISTINCT` が効いていたため、この修正による挙動の変化はありません。

---

4. 参考情報 (あれば)  

- 対象 PR: https://github.com/rails/rails/pull/57896  
- 関連する ActiveRecord メソッド:
  - `ActiveRecord::Relation#sum`
  - `ActiveRecord::Calculations`
- 実際のコードベースでの確認手順例:
  - Rails コンソールで、現行バージョンとこの修正が入ったバージョンを比較し、問題になりそうなクエリについて `to_sql` を出して差分を確認すると安全です。

    ```ruby
    scope = Account.where(firm_id: 1).group(:firm_id).distinct
    puts scope.sum(:credit_limit)   # 値の変化を確認
    puts scope.select('SUM(DISTINCT credit_limit)').to_sql
    ```

---


## [#57902](https://github.com/rails/rails/pull/57902) Keep `HashWithIndifferentAccess#filter` returning indifferent access {#pr-57902}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveSupport::HashWithIndifferentAccess` において、`#filter` 呼び出し時にも「シンボル/文字列を区別しないアクセス（indifferent access）」が維持されるように修正した PR です。これにより、`#select` と `#filter` が同じクラス（`HashWithIndifferentAccess`）を返し、期待どおりの挙動になります。
...
Ruby の標準 `Hash` では `#filter` は `#select` のエイリアスですが、`HashWithIndifferentAccess` では以下のような状態になっていました:

- `HashWithIndifferentAccess#select` はオーバーライドされており、戻り値も `HashWithIndifferentAccess` になるようになっている
- 一方で `#filter` は `HashWithIndifferentAccess` 側でエイリアス定義されておらず、`Hash` 実装の `#filter` が呼ばれてしまう
- その結果、`#filter` は「普通の Hash」を返し、indifferent access を暗黙に失っていた
...
h.filter { |k, v| v > 1 }.class # => Hash   (変更前)

h.select { |k, v| v > 1 }[:b]   # => 2
h.filter { |k, v| v > 1 }[:b]   # => nil    (変更前)
```

上記のように、返り値が `Hash` になってしまうため、`[:b]` アクセスでは値を取れず、`"b"` キーでしか取れない状態になっていました（indifferent access が壊れている）。

### 対応内容

`HashWithIndifferentAccess` クラスで、`#filter` をオーバーライド済みの `#select` にエイリアスするようにしました。

イメージとしては以下のようなコードが追加されています（実際の記述はクラス定義内）:

```ruby
alias_method :filter, :select
```

Ruby 本体が `Hash#filter` を `Hash#select` にエイリアスしているのと同様の関係を、`HashWithIndifferentAccess` にも適用した形です。

### なお、破壊的メソッドは問題なかった

- `#filter!` / `#select!` はどちらも `Hash` から継承しており、「自分自身を破壊的に変更する」メソッドのため、レシーバのクラス（`HashWithIndifferentAccess`）は変わらない
- そのため、問題があったのは「新しいハッシュを返す非破壊メソッド」の `#filter` だけでした

### テスト

`activesupport/test/hash_with_indifferent_access_test.rb` にテストが追加されており、`#filter` 実行後も `HashWithIndifferentAccess` であること・indifferent access が維持されることが検証されています。  
例:

```ruby
h = ActiveSupport::HashWithIndifferentAccess.new(a: 1, b: 2)
result = h.filter { |k, v| v > 1 }
...
- 主な影響
  - `HashWithIndifferentAccess#filter` が、これまで `Hash` を返していたのが `HashWithIndifferentAccess` を返すようになります。
  - `#filter` の戻り値に対して `[:symbol]` と `["string"]` の両方でアクセスできるようになります（`#select` と統一された挙動）。

- 後方互換性の観点
  - 「`#filter` の戻り値が `Hash` であること」を前提にしていたコード（例えば `result.is_a?(Hash)` かつ `!result.is_a?(HashWithIndifferentAccess)` を期待しているような処理）は挙動が変わります。
  - ただし `HashWithIndifferentAccess` は `Hash` のサブクラスであるため、単純に `Hash` であることだけをチェックしている分には問題ありません。
  - より実用上のケースとしては、「`#filter` の結果が indifferent ではないことを前提に、`:key` と `"key"` を意図的に区別していた」などのニッチなケースでのみ影響します。

- 利用者側での注意点
  - Rails 7 系などでこの変更が入るバージョンにアップデートする場合、`HashWithIndifferentAccess#filter` の戻り値のクラス・挙動が既存コードの前提とずれていないかを確認すると安心です。
  - ほとんどの場合は「バグ修正」として歓迎される変更であり、通常の Rails アプリケーションコードへの悪影響はほぼないと思われます。
...
- Ruby 本体の挙動  
  - Ruby の `Hash` クラスでは `#filter` と `#select` は同じメソッド（エイリアス）であり、`Hash#select` をオーバーライドするクラスは通常 `#filter` も同様に扱うべき、という設計があります。
- 関連するメソッドの整理
  - `HashWithIndifferentAccess#select` … オーバーライド済み、`HashWithIndifferentAccess` を返す
  - `HashWithIndifferentAccess#filter` … 今回 `#select` のエイリアスになった（非破壊・インデファレント維持）
  - `HashWithIndifferentAccess#select!` / `#filter!` … `Hash` から継承、破壊的なのでクラスは変わらない（もともと問題なし）

---


## [#57894](https://github.com/rails/rails/pull/57894) Reverse a relation by its `query_constraints` when no order is given {#pr-57894}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`query_constraints` を宣言しているモデルに対して `reverse_order` を呼んだ際、暗黙的な並び順（`first` / `last` が使う順序）を正しく反転するように修正した PR です。これにより、`reverse_order.first == last` が直感通りに成り立つようになり、モデルに `query_constraints` が無い場合の挙動は従来通りです。
...
`reverse_order` は、**明示的な `order` が無い Relation に対しては主キー（通常は `id`）で並び替える**、という動きをしていました。

一方で、`query_constraints` を宣言しているモデルでは、`first` / `last` は「`query_constraints` で指定したカラム」を使って暗黙的な並び順を決めます。  
...
- `ClothingItem.first` / `ClothingItem.last` は  
  `ORDER BY clothing_type ASC, color ASC` / `DESC`（相当）でレコードを取得する
- しかし `ClothingItem.all.reverse_order` は、`ORDER BY id DESC` のように「主キーだけ」を反転していた

そのため、以下が成り立ちませんでした:

```ruby
ClothingItem.all.reverse_order.first == ClothingItem.last
# 期待: true
# 実際(修正前): false
```
...
この PR では、**「明示的な `order` が無い Relation を `reverse_order` したときのフォールバックの並び順」を、`query_constraints_list` をベースに組み立てる**ように変更しています。
...
- これまで:  
  - 暗黙順序の反転に **主キーのみ** を使用
- これから:  
  - モデルに `query_constraints` が定義されていれば  
    `query_constraints_list` に含まれるカラムを使って `ORDER BY ... DESC` を構築
  - `query_constraints` が無ければ、従来通り主キーを使う

コードイメージ（説明用・擬似コード）:

```ruby
# 修正後のイメージ
if relation.arel.orders.empty?
  columns = query_constraints_list || Array(primary_key)
  relation = relation.order(columns.map { |c| arel_table[c].desc })
else
  # 既存の order を単純反転 (従来通り)
end
```
...
# 修正前:
#   ... ORDER BY "clothing_items"."id" DESC
# 修正後:
#   ... ORDER BY "clothing_items"."clothing_type" DESC,
#                 "clothing_items"."color" DESC
...
テスト (`activerecord/test/cases/relations_test.rb`) では、`query_constraints` を持つモデルで `reverse_order.first` が `last` と一致することなどが検証されています。

---

3. 影響範囲・注意点

- 影響を受けるのは **「`query_constraints` を持つモデル」かつ「`order` を指定していない Relation」に対して `reverse_order` を使っているコード** です。
  - これまで `id` の逆順に依存していたクエリは、`query_constraints` の逆順に変わります。
- `query_constraints` を定義していないモデル:
  - これまで通り、`reverse_order` のフォールバックは主キーベースで変更なし。
- 明示的に `order` を書いている場合 (`Model.order(:foo).reverse_order` など):
  - その `order` を反転する、という既存の挙動から変わりません。
- `first` / `last` と `reverse_order` の対応:
  - 今回の変更により、以下のような「直感的な」関係が成り立つようになります:
    - `relation.reverse_order.first == relation.last`
    - `relation.reverse_order.last  == relation.first`
  - 特に、ページネーションや「逆順にたどる」ようなロジックで、クエリ制約を順序の暗黙的基準にしている場合に扱いやすくなります。

運用上の注意としては、**既に本番コードで `query_constraints` + `reverse_order` に依存している場合、クエリ結果の順序が変わる可能性がある**ため、ログやテストで期待値を確認することを推奨します。

---

4. 参考情報 (あれば)

- `query_constraints` は Rails 7.2 以降で導入された、マルチテナンシーやパーティション分割などの用途向けの仕組みで、`first` / `last` / `find` などがどのカラムで絞り込む・ソートするかを決めるために使われます。
- この PR は、`first` / `last` が参照する暗黙の順序と `reverse_order` のフォールバック順序を揃えることで、API の一貫性を高める変更です。

---


## [#57817](https://github.com/rails/rails/pull/57817) Use the float value for `:wait` if passed to `ActiveJob.retry_on` {#pr-57817}

**マージ日**: 2026/6/28 | **作成者**: [@p8](https://github.com/p8)

1. 概要 (1-2文で)  
`ActiveJob.retry_on` に `wait:` オプションとして Float（秒数の小数）を渡した場合に、これまで整数に丸められていた待機時間を、Float のまま正しく扱うようにした修正です。`ActiveJob.set(wait: ...)` と同様に小数秒の遅延がサポートされ、CHANGELOG も追記されています。
...
対象: `ActiveJob.retry_on` の `wait:` オプション

従来の挙動（おおよそ以下のようなコード）:
```ruby
# 疑似コードイメージ
def retry_on(error, wait: 3, attempts: 5)
  wait_seconds = wait.to_i   # ここで整数に切り捨てられていた
  # ...
end
```

この PR では `to_i` を `to_f` に変更し、小数を含んだ秒数をそのまま利用するようにしています:

```ruby
# 実際の変更イメージ
wait_interval = wait.to_f
```

これにより、以下のような指定が期待どおりに動きます。

```ruby
class MyJob < ApplicationJob
  retry_on SomeError, wait: 1.5.seconds, attempts: 3
  # 1.5.seconds -> 1.5 (Float) がそのまま待ち時間に使われる
end
```

以前は `wait.to_i` によって `1.5.seconds` が `1` に切り捨てられており、サブ秒ないし小数秒の遅延指定が実質できませんでした。

### `ActiveJob.set(wait: ...)` との一貫性

Rails ではすでに以下のようなコードで Float の wait をそのまま使っています:

```ruby
MyJob.set(wait: 4.5.seconds).perform_later
```

`ActiveJob.set` ではリンク先の実装の通り float 値を尊重しており、`retry_on` だけが整数化してしまうという不整合がありました。この PR はその不整合を解消し、どちらも Float をサポートする形で統一しています。

### テストの修正

テストコードも以下のような形で修正されています。

- `retry_on` の `wait:` に指定する値を `4` → `4.5` のように変更  
  - 理由: リトライ回数 (`attempts: 2` など) と区別しやすくし、Float が使われていることをテストからも明示するため。
- これにより、`wait` が整数ではなく Float として扱われる前提でテストが書かれています。

### CHANGELOG の更新

`activejob/CHANGELOG.md` に以下の内容が追加されています（要旨）:

- `retry_on` の `wait:` オプションが Float（小数秒）をサポートするようになったこと
- 過去の PR (#57727) で漏れていた挙動変更の記録を補完
...
- **対象バージョン**  
  この PR がマージされた Rails の次のリリース（Active Job の該当バージョン）から有効になります。既存のアプリは、そのバージョンにアップデートしたタイミングで挙動が変わります。

- **互換性 (breaking かどうか)**  
  - 以前から整数の `wait` を使っていた場合:
    ```ruby
    retry_on SomeError, wait: 5, attempts: 3
    ```
    `5.to_i` も `5.to_f` も 5 秒のため挙動に変化はありません（後方互換）。
  - Float を使っていたが、整数切り捨てを前提にしていた場合:
    ```ruby
    retry_on SomeError, wait: 1.9, attempts: 3
    ```
    以前: 実質 1 秒待ち  
    今後: 1.9 秒待ち  
    となるので、もし「小数部分は切り捨てられる」という前提でコードを書いていた場合は挙動が変わります。ただし、その前提自体は API 意図からすれば特殊ケースと考えられます。

- **`ActiveSupport::Duration` (例: `1.5.seconds`) のサポート**  
  `1.5.seconds` は `ActiveSupport::Duration` ですが、`to_f` によって小数秒に展開されるため、これがそのまま利用されるようになります。  
  これにより、以下のようなコードが自然に動作するようになります:
  ```ruby
  retry_on SomeError, wait: 1.5.seconds
  ```

- **キューアダプタごとの実装上の注意**  
  多くのキュー（Sidekiq, Delayed Job, etc.）は秒数を Float で受け取っても問題なく扱いますが、
  - 自前のアダプタ
  - サードパーティ製の特殊なアダプタ  
  を実装している場合、`wait` / `schedule_at` の値を「整数前提」で扱っていないかを確認すると安全です（小数が来ても丸めてしまうなどの実装にしておかないと、意図しない差が出る可能性があります）。
...
- 対象 PR:  
  - #57817: Use the float value for `:wait` if passed to `ActiveJob.retry_on`
  - 関連 PR: #57727（この変更系統の先行 PR、CHANGELOG 追記が漏れていたもの）
- 関連実装:
  - `ActiveJob.set(wait: ...)` が Float をそのまま使っている実装  
    https://github.com/rails/rails/blob/b8990a6e917570d9c4e3f6a694fcc5cfa755ae53/activejob/lib/active_job/core.rb#L176-L177

この変更により、`retry_on` 周りでも「小数秒の遅延」「`ActiveSupport::Duration` の Float ベースな利用」が自然に行えるようになり、`set(wait: ...)` と挙動が揃います。

---


## [#57837](https://github.com/rails/rails/pull/57837) Make CookieStore::DEFAULT_SAME_SITE ractor safe {#pr-57837}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionDispatch::Session::CookieStore` のデフォルト `same_site` 設定用 Proc を `ActiveSupport::Ractors.shareable_proc` でラップし、`CookieStore::DEFAULT_SAME_SITE` を Ractor 間で共有可能（Ractor-shareable）にする変更です。これにより、Ractor を用いた並行実行環境でも CookieStore のデフォルト動作が安全に利用できます。
...
主な変更点は 1 行のみで、`CookieStore::DEFAULT_SAME_SITE` に設定される Proc の定義方法を変更しています。

イメージとしては、以下のような変更になります（実際のコードイメージ）:

```ruby
# 変更前（イメージ）
DEFAULT_SAME_SITE = proc { |request|
  # request を引数に受けて same_site を決めるロジック
}

# 変更後（イメージ）
DEFAULT_SAME_SITE = ActiveSupport::Ractors.shareable_proc(proc { |request|
  # request を引数に受けて same_site を決めるロジック
})
```
...
- `DEFAULT_SAME_SITE` は CookieStore の `same_site` オプションのデフォルト値を決める Proc。
- この Proc は引数として `request` を受け取るだけで、外部の変数や状態をクロージャとしてキャプチャしていません（=「閉じ込めていない」）。
- そのため `ActiveSupport::Ractors.shareable_proc` でラップすることで、Ractor の shareable オブジェクト要件を満たしつつ、既存の引数・振る舞いはそのまま維持できます。
- `ActiveSupport::Ractors.shareable_proc` は、Ractor で扱える「共有可能な Proc」としてマーク・生成するための ActiveSupport のヘルパです。

---

3. 影響範囲・注意点  

- Ractor を利用しない通常の Rails アプリケーション:
  - 振る舞いの変更はほぼありません。`same_site` の値決定ロジックは同一のままで、実行経路もほぼ変わりません。
- Ractor を利用する / 将来的に利用するアプリケーション:
  - `CookieStore::DEFAULT_SAME_SITE` が Ractor 間で共有可能になるため、CookieStore 設定を Ractor 跨ぎで利用しても Ractor の shareable 制約に引っかからなくなります。
  - これにより、Rails をマルチ Ractor 構成で動かす際のエラー回避・安定化に寄与します。
- カスタム `same_site` Proc を使っている場合:
  - この PR は「デフォルト値」のみを shareable にしたもので、アプリケーション側で独自の `same_site: ->(request){ ... }` を定義している場合、それが Ractor-safe かどうかは依然としてアプリケーション側の責任です。
    - 例: 外部インスタンス変数をキャプチャした Proc は shareable ではない可能性が高いです。
  - Ractor 環境で問題が出る場合は、同様に `ActiveSupport::Ractors.shareable_proc` を利用する、もしくはクロージャで外部状態を持たない形に書き換える必要があります。
...
- Ractor について:  
  - Ruby 3 から導入された並行実行モデルで、スレッドとは異なり「共有可能オブジェクト」と「移動可能オブジェクト」の制約があるため、グローバルな定数・Proc 等も shareable である必要があります。
- `ActiveSupport::Ractors.shareable_proc` の役割:  
  - Rails/ActiveSupport 側で、Ractor 環境における Proc の制約を吸収するためのヘルパ。
  - 今回の PR は、そのヘルパを Rails の標準ミドルウェア (CookieStore) に適用することで、Ractor 対応を一歩進める位置づけと言えます。

---


## [#57839](https://github.com/rails/rails/pull/57839) Make some ActiveSupport constants ractor safe {#pr-57839}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
ActiveSupport 内の一部の定数（ログレベル判定用 lambda と debug モード判定用 proc）を、`ActiveSupport::Ractors.shareable_lambda` / `.shareable_proc` でラップし、Ractor 間で共有可能（Ractor-shareable）にした PR です。これにより、Ractor を使う並行実行環境でも、これらの ActiveSupport 定数が安全に利用できます。
...
元々これらは「単なる lambda / proc の定数」でしたが、Ruby の Ractor では「Ractor-shareable でないオブジェクトを Ractor 間で共有できない」ため、そのままだと Ractor から参照したときにエラーや制約に引っかかる可能性があります。

PR では、これらを以下のように `shareable_lambda` / `shareable_proc` でラップして定義し直しています。

※実際のコードは概念的には以下のようなイメージです（PR からの意訳・簡略化）:
...
LEVEL_CHECKS = {
  ...
  # ... 他のレベルも同様に shareable_lambda でラップ
}
```
...
DEBUG_CHECK = ActiveSupport::Ractors.shareable_proc {
...
- `shareable_lambda` / `shareable_proc` は「Ractor-shareable になるようにチェック・マーキングされた callable」を返すため、これを定数に格納すると、その定数自体を Ractor 間で安全に共有できます。
- これらの callable は
  - 「Logger インスタンスを引数に受け取る（LEVEL_CHECKS）」  
  - 「`ActiveSupport.event_reporter.debug_mode?` を呼ぶだけ（DEBUG_CHECK）」  
 であり、**どちらもインスタンス変数やクロージャ内部の可変状態をキャプチャしていない**ため、Ractor-shareable として扱いやすい構造になっています。
- PR の説明でも「Neither captures any instance state」と明示されています。  
  → これは「Ractor-shareable 化に伴う副作用がほぼ無い」という保証にもなっています。
...
と非常に小さな差分で、既存のロジックはそのままに、「どのように lambda / proc を生成するか」だけを変更しています。
...
### 影響範囲
- 直接影響を受けるのは:
  - `ActiveSupport::EventReporter::LogSubscriber` を利用するコード（ログレベルに応じてイベントをフィルタ・処理する部分）
  - `ActiveSupport::StructuredEventSubscriber` を利用するコード（構造化イベントの debug 出力など）
- ただし、**挙動（どのログが出るか・どのイベントが処理されるか）は変わりません**。  
  変更は「Ractor 対応のための内部実装レベルの修正」であり、API 仕様や戻り値は互換です。

### Ractor 利用時のメリット
- Ractor 内で `ActiveSupport::EventReporter`/`StructuredEventSubscriber` を使う際に、これらの定数を含む ActiveSupport モジュールをそのまま共有できるようになります。
- これにより、以下のようなケースでエラーや制約にぶつかる可能性が下がります:
  - Ractor 内で `ActiveSupport.event_reporter` ベースのログ・イベント処理を行う
  - Ractor 起動前にロードした ActiveSupport を、そのまま Ractor から参照する

### 注意点
- 自分で `LEVEL_CHECKS` や `DEBUG_CHECK` を再定義・上書きしている場合:
  - その上書き定義が Ractor-shareable でない lambda / proc の場合、Ractor 環境で問題が発生する可能性があります。
  - Ractor 対応を意識するなら、独自の lambda / proc も `ActiveSupport::Ractors.shareable_lambda` / `.shareable_proc` でラップすることが推奨されます。
- Ractor 未使用のアプリケーションにおいては、この変更による実質的な挙動差はありませんが、
  - `ActiveSupport::Ractors.shareable_*` が存在しないバージョンにパッチを backport しようとするとコンパイルエラー / ロードエラーになるため、backport 時は要注意です。
...
- Ruby Ractors の概要:  
  - Ruby 3.x から導入された並行実行モデルで、オブジェクトは
    - Ractor ローカル、または
    - 「Ractor-shareable」とマークされたオブジェクト  
    のみが Ractor 間で共有可能です。
- ActiveSupport 側の Ractor 対応方針:
  - `ActiveSupport::Ractors.shareable_lambda` / `.shareable_proc` のようなヘルパーを介して、内部で使う callable を段階的に Ractor 対応している流れの一部と考えられます。
- 類似の変更が今後他の ActiveSupport コンポーネントにも波及する可能性があり、Ractor 対応を見据えたコードを書く場合は「定数に保持するクロージャにインスタンス状態をキャプチャしない」ことがより重要になります。

---


## [#57840](https://github.com/rails/rails/pull/57840) Make some Active Record constants ractor safe {#pr-57840}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Active Record内部で定数として保持しているいくつかのProcを、`ActiveSupport::Ractors.shareable_proc` でラップして Ractor 共有可能にした変更です。これにより、ArelのSQLバインド処理およびネストした属性の`:all_blank`判定で使われるProcが、Ractor間で安全に共有できるようになります。
...
1. `Arel::Visitors::ToSql` 内の bind-substitution 用のProc  
2. `Arel::Visitors::PostgreSQL` 内の bind-substitution 用のProc  
3. `ActiveRecord::NestedAttributes::ClassMethods` 内の `:all_blank` reject 用のProc  

これらはいずれも「定数的に定義された Proc」であり、Ractor間で共有したいオブジェクトです。

PR説明にもある通り、これらのProcは「外部状態をキャプチャしていない（＝クロージャではない）」「必要な情報はすべて引数として受け取る」ので、`shareable_proc` でラップしても動作上の意味は変わりません。

### 2-2. 実際の変更イメージ

具体的なコードはPR本文に出ていませんが、変更はおおむね次のような形です（イメージサンプル）:
...
```ruby
BIND_SUBSTITUTION = proc do |column, value|
  # 何らかのバインド用の処理
end
```
...
```ruby
BIND_SUBSTITUTION = ActiveSupport::Ractors.shareable_proc(proc do |column, value|
  # 何らかのバインド用の処理
end)
```
...
同様に、PostgreSQL固有のバインド処理用Procを `shareable_proc` でラップ:

```ruby
PG_BIND_SUBSTITUTION = ActiveSupport::Ractors.shareable_proc(proc do |column, value|
  # PostgreSQL 向けのバインド処理
end)
```

（定数名は説明用の仮名）

#### ActiveRecord::NestedAttributes::ClassMethods (`:all_blank`)

`accepts_nested_attributes_for` でよく使われる `:reject_if => :all_blank` が内部で利用するProcも対象です。

従来:

```ruby
ALL_BLANK_PROC = proc do |attributes|
...
```ruby
ALL_BLANK_PROC = ActiveSupport::Ractors.shareable_proc(proc do |attributes|
  attributes.all? { |key, value| value.blank? }
end)
```

（実際の中身はRails本体の定義に準じますが、構造としてはこういった形）

### 2-3. なぜ `shareable_proc` が必要か

RubyのRactorでは、Ractor間で共有できるオブジェクトは「shareable」でなければなりません。  
通常のProcは、外部環境（レキシカルスコープ）をキャプチャしていたり、内部にRactor非対応な状態を持つ可能性があるため、そのままでは shareable ではありません。

`ActiveSupport::Ractors.shareable_proc` は、以下のような性質を持つ「Ractor間で共有可能なProc」を作るためのActiveSupportヘルパです:

- 外部状態をキャプチャしない（＝クロージャではない）Procをラップする
- そのProcがRactor間で共有しても安全であることをRails側で保証する

このPRでは、**「定数として定義されていて、なおかつ外部状態をキャプチャしていないProc」** をすべて `shareable_proc` 化することで、Active Record自身がRactor対応しやすいように整えています。
...
- Arelを通じたSQL生成時の「バインド変数の置き換え処理」
  - `Arel::Visitors::ToSql`
  - `Arel::Visitors::PostgreSQL`
- `accepts_nested_attributes_for` の `reject_if: :all_blank` の挙動

ただし、いずれも「Procをshareableにしただけ」であり、ロジック自体は変わっていないため、**通常の単一スレッド/非Ractor環境での挙動は変化しません**。

主なメリットは以下です。

- Ractorを使ってRailsアプリケーションや一部処理を並列実行する場合に、Active Record / Arel内部の定数Procが原因で `Ractor::IsolationError` になる可能性が下がる
- Rails側で公式に「この定数ProcはRactor対応している」と宣言した形になるため、将来的なRactorサポートを進めやすくなる

### 3-2. 互換性・注意点

- APIのシグネチャは変わっていないため、アプリケーションコードから見た挙動の変化は基本的にありません。
- `ActiveSupport::Ractors.shareable_proc` を自前で利用する場合も同様ですが、「外部変数をキャプチャしているProc」を `shareable_proc` に渡すとエラーになる可能性があります。
- 本PRは「**Rails内部の定数ProcをRactor安全にした**」に留まり、アプリケーション側でRactor使用時に必要となるすべての対応が完了したわけではありません。  
  依然として、アプリケーションコードや他のRails内部実装にRactor非対応な部分が残っている可能性があります。

---

4. 参考情報 (あれば)  

- Ruby公式: Ractor (並行実行のための仕組み)  
  - https://docs.ruby-lang.org/en/master/Ractor.html
- Railsガイド (参考):  
  - Nested attributes: https://guides.rubyonrails.org/v6.1/association_basics.html#nested-attributes  
  - Arel Visitors の概要については公式ガイドは薄いものの、ArelのSQL生成に関わるコンポーネントです。
- ActiveSupport::Ractors (暫定APIの可能性があるため、実際のRailsバージョンのソースを参照することを推奨)

---


## [#57841](https://github.com/rails/rails/pull/57841) Make ActionMailbox::Callbacks::TERMINATOR ractor safe {#pr-57841}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Action Mailbox のコールバック用終端処理 (`TERMINATOR` lambda) を `ActiveSupport::Ractors.shareable_lambda` でラップし、Ractor 間で安全に共有できるようにした PR です。これにより、すでに Ractor 対応されたコールバック機構 (#57629) において、Action Mailbox 固有の終端処理も Ractor-safe になります。
...
実質的な変更は、`ActionMailbox::Callbacks::TERMINATOR` の定義方法だけです。
...
module ActionMailbox
  module Callbacks
    TERMINATOR = ->(mailbox, chain) do
      # コールバックチェーンを打ち切る条件や処理
      # 例: mailbox が処理を続行しない場合に true を返す、など
    end
  end
end
```

これを、`ActiveSupport::Ractors.shareable_lambda` で包むように変更しています:

```ruby
module ActionMailbox
  module Callbacks
    TERMINATOR = ActiveSupport::Ractors.shareable_lambda do |mailbox, chain|
      # 上と同じロジック（mailbox と chain を受け取り、外部状態をキャプチャしない）
    end
  end
end
```
...
- `TERMINATOR` は「コールバックチェーンをいつ終了するか」を判定するための lambda。
- 引数として `mailbox` と `chain` を取り、外部の変数をキャプチャしていない（= クローズオーバなし）。
- そのため `ActiveSupport::Ractors.shareable_lambda` でラップするだけで Ractor-shareable なオブジェクトとして扱える。
- #57629 で「コールバック機構」自体は Ractor 安全化されており、この PR は「Action Mailbox が渡す terminator lambda も同じく Ractor-safe にする」補完的変更。

`ActiveSupport::Ractors.shareable_lambda` は、内部で Ractor が要求する「shareable」オブジェクトの制約（外部状態をキャプチャしないなど）を満たす lambda を生成するためのヘルパーです。
...
- Action Mailbox のコールバックチェーン処理において、`TERMINATOR` が Ractor 間で共有されるケース（マルチ-Ractor 環境）での安全性が向上。
- Ractor を利用した並列処理で Action Mailbox を使う場合に、`TERMINATOR` が原因で Ractor による共有エラーが出る可能性が解消されます。

実装・運用上の注意点:

- 既存のシングルスレッド / シングルプロセス / 非 Ractor 環境では振る舞いは変わりません。
- `TERMINATOR` 本体のロジックやシグネチャ（引数など）は変更されていないため、外部 API 的な互換性は維持されています。
- 今後、`TERMINATOR` のロジックを変更する際に「外部変数をキャプチャするような書き方」をすると Ractor-shareable でなくなるので注意が必要です。  
  例: 以下のような書き方は避けるべきです（外部状態をキャプチャしてしまう）:
  ```ruby
  some_state = ...
  TERMINATOR = ActiveSupport::Ractors.shareable_lambda do |mailbox, chain|
    # some_state を参照すると Ractor-shareable ではなくなる可能性
  end
  ```
...
- この PR が補完する以前の変更:  
  - #57629: コールバック機構（`ActiveSupport::Callbacks` 周り）の Ractor-safe 化
- Ractors と shareable オブジェクトについて（Ruby 本体のドキュメント）  
  - https://docs.ruby-lang.org/ja/latest/class/Ractor.html  
- `ActiveSupport::Ractors.shareable_lambda` は Rails 側で Ractor 対応をサポートするためのヘルパーであり、Ractor の制約を満たすラッパーとして利用されます。

---


## [#57890](https://github.com/rails/rails/pull/57890) Fix `week_field` emitting invalid week strings at ISO year boundaries {#pr-57890}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails の `week_field` ヘルパが、年末年始の ISO 週番号境界で仕様上無効な週文字列（例: `2016-W53`）を出力してしまう不具合を修正し、常に ISO 週番号付け年 (week-numbering year) を用いるようにした PRです。これにより `<input type="week">` の値がブラウザ仕様どおり `yyyy-Www` 形式で、かつ実在する週を指すようになります。
...
HTML の `<input type="week">` では、値は `yyyy-Www` 形式ですが、この `yyyy` は「カレンダー年」ではなく **ISO 8601 の週番号付け年 (week-numbering year)** である必要があります。  
Ruby の `strftime` では：

- `%Y` … カレンダー年 (Gregorian year)
- `%V` … ISO 週番号 (01–53)
- `%G` … ISO 週番号付け年 (week-numbering year)

となっています。

元の実装では `WeekField#format_datetime` が:

```ruby
datetime.strftime("%Y-W%V")
```

とフォーマットしており、**カレンダー年 `%Y` と ISO 週番号 `%V` を組み合わせていた**ため、年末年始の境界で矛盾した値が出ていました。

例:

```ruby
Date.new(2016, 1, 1).strftime("%Y-W%V")
# => "2016-W53"  (カレンダー年2016, ISO週番号53 の組み合わせ)
# しかし ISO 週番号付け年 2016 には週 53 は存在しないので無効な week 文字列

Date.new(2016, 1, 1).strftime("%G-W%V")
# => "2015-W53"  (ISO 週番号付け年 2015 の週 53。これが正)
```

ブラウザは存在しない週 (`2016-W53` など) を値として受け付けないため、`week_field` ヘルパから出力される HTML が仕様違反になっていました。

### 修正内容

`actionview/lib/action_view/helpers/tags/week_field.rb` のフォーマット指定を 1 文字だけ変更：

```diff
- datetime.strftime("%Y-W%V")
+ datetime.strftime("%G-W%V")
```

つまり、

- ISO 週番号 `%V` と **対応する ISO 週番号付け年 `%G`** をペアにするように変更。

これにより、境界日時 `DateTime.new(2016, 1, 1)` などに対しても正しい ISO 年・週の組み合わせが出力されます。

### テスト追加

`actionview/test/template/form_helper_test.rb` に、境界ケースをカバーするテストを追加しています。  
主な追加テスト:

- `week_field` に `DateTime.new(2016, 1, 1)` を与えたときに、`value="2015-W53"` となることを確認するテスト。

今までのテストはすべて「年の途中 (mid-year)」の値のみを使っており、`%Y == %G` となる日付しか検証していなかったため、この問題が発覚しませんでした。今回、`%Y != %G` となる代表例をテストに加えることで、退行を防いでいます。
...
- 対象:
  - Rails の `form_with`, `form_for` などで `week_field` / `f.week_field` を使用し、`Date` や `Time` / `DateTime` オブジェクトを渡しているコード。
- 挙動の変化:
  - 年末・年始付近の週について、HTML の `value` が変わる可能性があります。
    - 例:
      - 修正前: `2016-01-01` → `value="2016-W53"`（ブラウザ非互換・無効）
      - 修正後: `2016-01-01` → `value="2015-W53"`（正しい ISO 週表現）
- フロントエンドとの取り決めに注意:
  - もし「カレンダー年ベースの ‘年 + ISO 週番号’」という **独自仕様** で扱うロジック (JS 側やバックエンド) があった場合、今回の修正で値が変わることにより、整合が崩れる可能性があります。
  - HTML 仕様および ISO 8601 に準拠した値を期待している場合は、今回の変更は望ましい挙動です。
- 既存データとの互換性:
  - すでに DB に `2016-W53` のような「カレンダー年 + ISO 週番号」の文字列を保存している場合、それを Rails の `week_field` に再設定・編集する際に、ブラウザ側で拒否される/解釈されない可能性があります。
  - 過去の保存値を ISO 年 (`%G`) ベースに補正することを検討してください。
...
- ISO 8601 (週日付):
  - 週番号付け年とカレンダー年の違い:
    - ISO 週日付では「木曜日を含む年」がその週の属する年とみなされるなどのルールがあり、年末年始でカレンダー年と ISO 年がズレることがあります。
- Ruby `strftime` ドキュメント（ISO week/date 関連）:
  - `%G`: ISO 8601 週番号付け年 (4 桁)
  - `%g`: ISO 8601 週番号付け年 (2 桁)
  - `%V`: ISO 8601 週番号 (01–53)
- Rails 内の一貫性:
  - PR 説明にあるとおり、`DateField`, `MonthField`, `DatetimeLocalField` はもともと「意味的に一貫したディレクティブの組み合わせ」を使っており、`WeekField` だけが不整合になっていました。今回の修正で、これらと同様にフォーマットの意味的整合性が保たれています。

---


## [#57870](https://github.com/rails/rails/pull/57870) Honor `options[:drop]` in CacheStore session deletion {#pr-57870}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionDispatch::Session::CacheStore#delete_session` が `options[:drop]` を無視して常に新しいセッションIDを返していた不具合が修正され、`options[:drop]` が指定された場合は `nil` を返すようになりました。これにより、Rack のインターフェース仕様および `CookieStore` の挙動と整合が取れ、セッションを「ドロップしたつもりなのに新しいセッションが発行される」問題が解消されます。
...
`CacheStore` の `delete_session` は以下のような実装になっており、`options[:drop]` を考慮せず常に新しいセッションIDを返していました:
...
Rack のセッションミドルウェアの契約では:

- `delete_session(env, sid, options)` は
  - 通常は「次のリクエスト用の新しいセッションID」を返す
  - ただし `options[:drop]` が `true` のときは `nil` を返す  
    → これにより `commit_session` 側が「新しい cookie を発行しないで終了」できる

一方で、`CookieStore` ではすでに以下のように `options[:drop]` を尊重する実装になっていました:

```ruby
new_sid = generate_sid unless options[:drop]
```

したがって、`CacheStore` だけがこの契約に違反しており、「セッションを破棄したい (`drop`) のに、新しいIDと cookie が必ず発行される」という挙動になっていました。

### 修正後の挙動

この PR により、`CacheStore#delete_session` も `options[:drop]` を見て `nil` を返すように変更されました。概念的には次のような実装になります:
...
- `options[:drop]` が **指定されていない / false** の場合  
  → これまで通り、セッション削除後に新しいセッションIDが生成される
- `options[:drop]` が **true** の場合  
  → キャッシュ上のセッションは削除されるが、新しいセッションIDは返さず `nil` が返る  
  → Rack 側で新しいセッション cookie が発行されず、本当に「ドロップ」される

### テストの追加

`actionpack/test/dispatch/session/cache_store_test.rb` にテストが追加され、少なくとも次のようなケースがカバーされていると考えられます:

- `delete_session` 呼び出し時に `options[:drop]` を指定した場合に `nil` が返ること
- (暗黙的に) `options[:drop]` がない場合は `String` などの新しいセッションIDが返ること

これにより、`CookieStore` と `CacheStore` の契約上の挙動がテストレベルでも揃えられています。

---

3. 影響範囲・注意点  

- 対象: `ActionDispatch::Session::CacheStore` を利用している Rails アプリケーションのセッション処理
- 影響シナリオの例:
  - ログアウト処理などで `request.session_options[:drop] = true` を設定してセッション破棄を期待しているケース
  - 独自ミドルウェアや Rack レベルで `env['rack.session.options'][:drop] = true` をセットしているケース
- 修正前:
  - セッション破棄時にも、新しいセッションIDが発行され、新しいセッション cookie がクライアントへ送られていた
  - 結果として「ログアウトしたのに空の新セッションが作られる」「セッションを完全に消したいのに cookie が残る」といった挙動になりうる
- 修正後:
  - `options[:drop]` を指定した削除では **一切新しいセッション cookie が発行されなくなる**
  - これを前提としていた（＝Rack 仕様に沿った）コードは、むしろ期待どおりに動くようになる
- 互換性の観点:
  - 正しい Rack 仕様に合わせる方向の修正のため、「古いバグ前提」のコードを書いていなければ問題はないはず
  - もし、`CacheStore` 使用時に「delete_session を呼ぶと必ず新しいセッションIDが来る」という前提に依存した実装があれば、そのコードは見直しが必要

**確認したいポイント:**

- ログアウトやアカウント削除など、「cookie もセッションも完全に消したい」処理で `:drop` を使っているか
- 独自に `delete_session` を呼んでいる場合、戻り値 `nil` を想定しているか
  - これを機に、戻り値が `nil` の可能性を考慮した安全なコードになっているかを再確認する価値があります。

---

4. 参考情報 (あれば)  

- Rack セッションインターフェース仕様（`delete_session` の戻り値の契約）  
  - `delete_session(env, sid, options)` は「新しいセッションID」または「`nil`（drop 時）」を返すことが求められる
- 類似実装:  
  - `ActionDispatch::Session::CookieStore#delete_session` の実装  
    → 今回の修正はこれと同じ挙動になるように `CacheStore` を揃えたもの
- 実アプリケーション側での利用例:
  - ログアウト時の典型パターン:
    ```ruby
    reset_session   # 内部で delete_session + drop が使われるケース
    ```
    などで、今回の修正により「本当に cookie が消える」挙動が保証されやすくなる。

---


## [#57897](https://github.com/rails/rails/pull/57897) Deduplicate columns in `regroup` {#pr-57897}

**マージ日**: 2026/6/28 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveRecord::Relation#regroup` / `regroup!` が、重複したカラム指定をそのまま保持していた問題を修正し、`group` と同様にグループ対象カラムを重複除去（uniq）するようにした PR です。これにより、ドキュメントにある「`regroup` は `unscope(:group).group(fields)` のショートハンド」という仕様が実際の挙動と一致するようになりました。
...
`regroup` はドキュメント上、次のような意味だと説明されています:

```ruby
regroup(fields) ≒ unscope(:group).group(fields)
```

`group` は内部的に

```ruby
self.group_values |= args  # 集合演算で重複除去
```

のように実装されており、同じカラムを複数回指定しても重複が取り除かれます。

一方、`regroup!` はこれまで、渡された引数をそのまま `group_values` に代入していたため、重複が残ってしまい、ドキュメントと動作が食い違っていました:

```ruby
Post.regroup(:author_id, :author_id).group_values
# 修正前: [:author_id, :author_id]
# 期待される動作: [:author_id]

Post.all.unscope(:group).group(:author_id, :author_id).group_values
# => [:author_id]  # こちらは既に重複除去されている
```

### 修正内容

`regroup!` 内で、引数として渡されたカラムリストを重複除去する処理が追加されています。  
PR の説明から推測される実装イメージは以下のようなものです:
...
# 変更後 (イメージ)
def regroup!(*args)
  self.group_values = args.flatten.uniq  # or something equivalent
  self
end
```

説明文に「`reorder!` に合わせた」とある通り、`reorder!` と同じように「代入系メソッドでも重複を取り除く」というポリシーに揃えています。

テスト (`activerecord/test/cases/relations_test.rb`) では、以下のようなケースが追加されていると考えられます:

```ruby
rel = Post.regroup(:author_id, :author_id)
assert_equal [:author_id], rel.group_values
```

これにより、`regroup` が `unscope(:group).group(fields)` と同等の結果を返すことが保証されます。
...
- **主な影響対象**  
  - `regroup` / `regroup!` を利用しつつ、意図的に同じカラムを複数回 `group` に渡していたコードがある場合、`group_values` の内容が配列上は変わります（重複がなくなる）。  
  - ただし SQL の観点では、`GROUP BY author_id, author_id` と `GROUP BY author_id` は結果が同じため、通常のユースケースでは実質的な影響はありません。

- **後方互換性**  
  - 重複した `group` 指定に依存するようなコードは、（ほぼありえませんが）挙動が変わる可能性があります。
  - `group_values` を直接参照しているメタプログラミング的なコードで、「配列の長さ」や「重複の有無」を前提にしている場合は、違いが出る可能性があります。

- **メリット**  
  - ドキュメントと実装の乖離が解消され、`regroup` の意味がより直感的かつ一貫したものになりました。
  - `group`・`reorder` と同じポリシー（重複除去）になり、API 仕様としても整合性が高まりました。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57897  
- 関連メソッド:
  - `ActiveRecord::Relation#group`
  - `ActiveRecord::Relation#unscope`
  - `ActiveRecord::Relation#reorder` / `#reorder!`  
- 実際の等価関係（今回の修正後）:
  ```ruby
  Post.regroup(:author_id, :author_id)
  # ≒
  Post.all.unscope(:group).group(:author_id, :author_id)
  # どちらも group_values は [:author_id] になる
  ```

---


## [#57899](https://github.com/rails/rails/pull/57899) Fix flaky test_parallel_group_stops_launching_new_steps_with_*_flag {#pr-57899}

**マージ日**: 2026/6/28 | **作成者**: [@yahonda](https://github.com/yahonda)

1. 概要 (1-2文で)  
Rails の CI テスト `ContinuousIntegrationTest#test_parallel_group_stops_launching_new_steps_with_*_flag` が並列実行時のタイミング依存で不安定に落ちる問題を、テスト側の実行時間を調整することで解消した PR です。  
本番コードの変更はなく、`activesupport/test/continuous_integration_test.rb` 内のテスト定義のみが修正されています。
...
（`--fail-fast` とその短縮形 `-f` の両方のバリエーション）

テストの目的は：

- 並列実行中にあるステップが失敗したとき  
- `--fail-fast` / `-f` フラグが有効なら  
- 「それ以降のステップ（Should not run N）」が起動されないこと

を確認することです。

しかし実際には、CI 上で以下のような失敗が散発的に発生していました：

- 「Fail」ステップが失敗したあとに  
- 本来実行されてはいけない「Should not run 3」が起動・完了してしまう

ログ抜粋（失敗例）を見ると、

- `Should not run 1` (0.01s)
- `Should not run 2` (0.01s)
- `Fail` (0.02s)
- その後に `Should not run 3` (0.01s)

という順に動いており、**「Fail が失敗した後に 3 がキューから拾われてしまっている」**状況です。  
これは並列ワーカーのスレッド/プロセスがキューからステップを取り出すタイミングと、fail-fast フラグが有効になって「もう新しいステップを起動してはいけない」と認識するタイミングの競合によるものです（spawn jitter / スケジューリングの揺らぎ）。

### PR での対策

PR の要点は次の一つです。

- 「実行されてはならないステップ」の中身を `true` から `sleep 0.1` に変更した

元: （概念的なイメージ）

```ruby
# 擬似イメージ: 実際のコードは test helper 経由で step を定義している
step "Should not run 1" do
  true
end

step "Should not run 2" do
  true
end

step "Should not run 3" do
  true
end
```
...
- 元々の `true` ステップは非常に高速（~0.01s）で終わっていた
- 「Fail」ステップも ~0.02s と短時間
- その結果、「Fail が失敗した」という事実が他のワーカーに伝播する前に、別ワーカーが次のステップ（Should not run 3）をすでにキューから取得してしまう可能性があった

そこで、

- 「Should not run X」側を `sleep 0.1` (約 0.1s) にして、  
  - 各「実行されるべきではないステップ」の実行時間を十分に長くすることで
  - それらの実行中に「Fail」ステップが必ず完了し、fail-fast の状態が確定するようにする
  - すると、**Fail 完了後に新しいステップがデキューされる余地がなくなる**ため、テストがタイミングの揺らぎから独立して安定する

PR の説明にもある通り：

> each should-not-run step (~0.1s) outlasts "Fail" (~0.02s).  
> The failure is then observed before a worker can dequeue the third step, independent of spawn jitter.

という設計意図です。

重要な点：

- **本番コード（parallel 実行の実装）は一切変更していない**
- 変更したのはテスト内のステップの中身だけであり、「Fail したら以降のステップは起動しないべき」という仕様自体には手を付けていません
- あくまで「テストが OS や CI 環境のスケジューリングに過度に依存しないようにする」ための調整です
...
- 影響範囲:
  - `activesupport/test/continuous_integration_test.rb` 内の CI/並列実行テストのみ
  - ライブラリ利用者のコードや挙動には一切影響しません
  - Rails を gem として使う側の挙動変更はゼロです

- 注意点:
  - テストが少しだけ遅くなる（`sleep 0.1` × 3 ステップ分）ため、そのテストケース単体では ~0.3s 程度のオーバーヘッドが追加されます
  - ただしこれは CI 全体から見れば軽微であり、その代わりに「テストのフレークを避けて安定性を上げる」メリットがあるため妥当なトレードオフといえます
  - 今後類似の並列 + fail-fast 系テストを書く場合も、**ステップの実行時間と同期タイミングを意識しないとフレークしやすい**、という知見が得られます
...
- 実際にフレークが発生した CI ログ:
  - `--fail-fast` 版: https://buildkite.com/rails/rails/builds/130019#019ee5dc-f207-4b3a-9d10-f4c7961968ad/L8002
  - `-f` 版: https://buildkite.com/rails/rails-nightly/builds/4492#019f05bb-ba27-427b-b7d9-e0e45b19274b/L7959  

- PR 本体:  
  - https://github.com/rails/rails/pull/57899

この PR は、「並列処理とタイミング依存ロジック（fail-fast）」をテストする際に、タスクの実行時間を明示的に調整してテストを安定化させる、という典型的なフレーク対策になっています。

---


## [#57895](https://github.com/rails/rails/pull/57895) Make `add_foreign_key(if_not_exists: true)` reversible {#pr-57895}

**マージ日**: 2026/6/27 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`add_foreign_key(if_not_exists: true)` を含むマイグレーションをロールバックした際に、正しく可逆（reversible）になるように修正したPRです。`if_not_exists: true` を伴う外部キー追加の逆操作として、`if_exists: true` を付けた `remove_foreign_key` が生成されるようになりました。
...
Rails のマイグレーションで以下のように外部キーを追加したとします:

```ruby
add_foreign_key :articles, :authors, if_not_exists: true
```

`ActiveRecord::Migration::CommandRecorder` は、この操作を逆転（invert）してロールバック用のコマンドを自動生成しますが、従来は次のような結果になっていました:
...
しかし `remove_foreign_key` は `if_not_exists:` オプションをサポートしていません。そのため:

- 実行時にエラーになる可能性がある
- そもそも「存在しないならスキップ」という意味にならない  
といった問題があり、マイグレーションの可逆性・冪等性に反していました。

### 今回の修正

`add_foreign_key(if_not_exists: true)` の逆操作を生成する際に、オプションを以下のように変換するよう修正されました:

- `if_not_exists: true` → `if_exists: true`

つまり、今後は次のような逆操作が生成されます:
...
これは、既に `invert_add_check_constraint` で採用されている挙動と同じで、設計としても一貫した形です。

#### 実際のマイグレーションでのイメージ

```ruby
class AddFkToArticles < ActiveRecord::Migration[7.2]
  def up
    add_foreign_key :articles, :authors, if_not_exists: true
  end

  def down
    # 以前: Rails が自動生成すると仮定した場合、実際には正しく扱えない if_not_exists が渡されうる
    # remove_foreign_key :articles, :authors, if_not_exists: true  # ←サポート外

    # 今後: Rails が自動生成する逆操作はこうなる
    remove_foreign_key :articles, :authors, if_exists: true
  end
end
```

このように、存在しない外部キー削除時もエラーにならずスキップされ、`if_not_exists` 付きで追加したときと対になる挙動になります。

### コード上の変更点

- `activerecord/lib/active_record/migration/command_recorder.rb`  
  - `invert_add_foreign_key` 相当の処理において、オプションハッシュに `if_not_exists: true` が含まれる場合に、それを `if_exists: true` に変換する分岐を追加。
  - 実装方針は `invert_add_check_constraint` の既存コードと同様。

- `activerecord/test/cases/migration/command_recorder_test.rb`  
  - `add_foreign_key` + `if_not_exists: true` を inverse したときに `if_exists: true` に変換されることを検証するテストを追加。
...
- 対象:
  - `add_foreign_key` を `if_not_exists: true` 付きで使用しているマイグレーション。
  - それらを reversible なマイグレーションとして `change` メソッドで書いている場合、または `revert` を利用している場合。

- 期待される挙動の変化:
  - 以前:
    - ロールバック時のコマンドが `remove_foreign_key(..., if_not_exists: true)` の形で扱われ、実行時に不整合やエラーを招く可能性があった。
  - 以後:
    - ロールバック時のコマンドは `remove_foreign_key(..., if_exists: true)` になり、外部キーが存在しない場合でも安全にスキップされる。
    - `if_not_exists` を使ったマイグレーションが、他の idempotent な API（チェック制約など）と同様に、安全に往復（migrate → rollback）できる。

- 後方互換性:
  - `remove_foreign_key` が理解しないオプションを渡していた、ある意味「壊れていた」パスを正しいオプションに修正する形なので、実質的にはバグフィックス扱いです。
  - もしアプリケーション側で `CommandRecorder` の挙動（引数・オプション）を直接前提にしているようなメタプログラミングをしている場合、そのロジックに影響する可能性がありますが、一般的なアプリケーションコードではほぼ影響はありません。
...
- 類似の挙動:  
  - `invert_add_check_constraint` でも `if_not_exists: true` → `if_exists: true` という同様の変換が既に行われており、今回の変更で外部キーに関しても挙動が統一されました。

- 関連する Rails の仕組み:  
  - `ActiveRecord::Migration::CommandRecorder` は、`change` メソッドや `revert` ブロック内の操作を記録し、自動的に逆操作を生成するための仕組みです。  
  - 今回の修正により、外部キー操作におけるこの自動逆変換の信頼性が向上します。

---


## [#57892](https://github.com/rails/rails/pull/57892) Return an `ActiveRecord::Promise` from `async_ids` on a contradictory relation {#pr-57892}

**マージ日**: 2026/6/27 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Relation#async_ids` が「常に `ActiveRecord::Promise` を返す」という仕様に反して、矛盾した where 句（例: `where(id: [])`）の場合だけ素の `Array` を返してしまうバグがあり、それを修正した PR です。`async_ids` がどんな条件でも一貫して `ActiveRecord::Promise` を返すようになりました。
...
`async_ids` は内部的に `ids` 相当のクエリを非同期で発行し、その結果を `ActiveRecord::Promise` でラップして返すメソッドです。ドキュメント上も「常に `ActiveRecord::Promise` を返す」ことが契約になっています。

しかし、以下のように矛盾した where 句（contradictory relation）を持つ場合:

```ruby
Model.where(id: [])     # 絶対にレコードが返らない条件
Model.none              # など
```

`ids` 相当の処理は「どうせ結果は空」ということが分かるので、DB クエリを飛ばさずにショートサーキットします。その際、問題のあったコードは次のような分岐をしていました（概略）:

```ruby
result = if relation.where_clause.contradiction?
  ActiveRecord::Result.empty            # ← async: を渡していない
else
...
- 矛盾していない通常の relation では `select_all(..., async: @async)` が `ActiveRecord::Promise` を返すため、その後の `result.then` は `ActiveRecord::Promise#then` となり、戻り値も `Promise` のままです。
- 一方、矛盾した relation では `ActiveRecord::Result.empty` が **同期的な `ActiveRecord::Result` オブジェクト** を返します。そのため `result.then` は `ActiveRecord::Promise#then` ではなく、Ruby 標準の `Kernel#then` として解決され、戻り値は単なる `Array` になってしまいます。

結果として:

```ruby
Model.where(id: []).async_ids # => []  # Array になってしまう
```

という、ドキュメントと実装上の契約違反が発生していました。

### 修正内容

修正は非常に小さく、ショートサーキット時の空結果生成を次のように変更しています。
...
つまり、「矛盾した where 句でクエリをスキップする場合でも、`async: @async` を渡して、非同期用の `Result` / Promise チェーンと整合する形で空結果を構築する」ようにしました。

`pluck` については既に同様のケースで

```ruby
ActiveRecord::Result.empty(async: @async)
```

を使っており、`async_ids` がその挙動に揃えられた形になります。

### テストの追加

`activerecord/test/cases/calculations_test.rb` に 1 行追加されており、おそらく以下のような形で、「矛盾した relation に対しても `async_ids` が `Promise` を返すこと」を検証しています（概念的な例）:

```ruby
assert_kind_of ActiveRecord::Promise, Model.where(id: []).async_ids
```
...
- **仕様どおりに揃える修正**  
  もともとドキュメントでは「常に `ActiveRecord::Promise` を返す」としていたため、今回の変更は「バグ修正」であり、仕様を変更するものではありません。

- **既存コードへの影響**  
  これまで誤って `Array` を返していたケースに依存しているコードがある場合、挙動が変わります。具体的には:

  ```ruby
  # 以前: relation が矛盾しているときは Array 前提で書けてしまっていた
  result = Model.where(id: []).async_ids
  result << 1  # Promise ではなく Array 前提の操作

  # 修正後: 上記のようなコードは NoMethodError などになる
  ```

  ただしドキュメント上は常に `Promise` であることが前提なので、正しく `async_ids` を使っていたコードは `then`, `await`（使っている環境に応じて）などを通じて値を扱っているはずで、多くの場合はこれで正しい状態に揃ったとみなせます。

- **矛盾した where 句の検出ロジック**  
  `where_clause.contradiction?` によるショートサーキット自体のロジックは変更されていないため、クエリが実行される・されないという意味でのパフォーマンスや挙動には影響はありません。  
  変わるのは「戻り値の型が、非矛盾ケースと一貫して `Promise` になる」という点のみです。

---

4. 参考情報 (あれば)  

- 類似挙動: `pluck` も同様のケース（矛盾する where 句）について `ActiveRecord::Result.empty(async: @async)` を使用しており、今回の修正で `async_ids` と `pluck` の内部実装が揃いました。
- 関係するクラス:
  - `ActiveRecord::Promise`: 非同期クエリの結果を表す Promise ラッパ
  - `ActiveRecord::Result`: クエリ結果のラッパ。`empty(async: ...)` によって、非同期コンテキスト向けの空結果が生成されるようになっています。

---


## [#57891](https://github.com/rails/rails/pull/57891) Fix `number_to_human`/`number_to_human_size` crashing on `precision: nil` {#pr-57891}

**マージ日**: 2026/6/27 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`number_to_human` と `number_to_human_size` に `precision: nil` を渡したときに `NoMethodError` で落ちていた不具合を修正した PR です。ほかの number helper と同様に、`precision: nil` を安全に扱えるようになります。
...
Rails の number helper では、`precision: nil` は有効なオプションとして扱われています。例えば:

```ruby
number_to_currency(1234.56, precision: nil)
number_to_percentage(12.345, precision: nil)
```

といった呼び出しはサポートされており、テストも存在します。

しかし、`number_to_human` / `number_to_human_size` だけは、内部的に `RoundingHelper#absolute_precision` を通る実装になっており、そこで次のようなコードがありました (問題箇所のイメージ):
...
ここで `options[:precision]` に `nil` が渡されると、Ruby の比較演算 (`nil > 0`) が実行されて `NoMethodError: undefined method '>' for nil:NilClass` が発生します。

このため、以下のような呼び出しがクラッシュしていました:

```ruby
number_to_human(123456, precision: nil)
number_to_human_size(123456, precision: nil)
```

### 修正内容

`precision` が存在する場合にのみ `> 0` 比較を行うようガードを追加しています。具体的には、`RoundingHelper#absolute_precision` 内の条件式を、兄弟クラスである `NumberToRoundedConverter#convert` と同等のロジックに揃える形で修正しています。

イメージとしては、以下のような変更です (実際のコードは 1 行差分ですが、意味的にはこういう修正):
...
# 修正後 (概念的)
if options[:significant] && (precision = options[:precision]) && precision > 0
  # precision が nil でない場合だけ > 0 比較を行う
end
```

`NumberToRoundedConverter#convert` では元々:
...
のような形で `precision` の存在チェックをしており、`RoundingHelper` 抽出時にそのガードが落ちてしまっていた、という位置づけです。

### テスト

`activesupport/test/number_helper_test.rb` にテストが追加され、`number_to_human` / `number_to_human_size` に `precision: nil` を渡したときに例外が出ないことがカバーされています。  
(追加行数 2 行なので、既存の number helper テストにケースを 1 つずつ増やした、程度の変更と考えられます。)

---

3. 影響範囲・注意点  

- 影響を受けるのは `number_to_human` / `number_to_human_size` を `precision: nil` 付きで呼び出しているコードです。
  - これまでは例外で落ちていたケースが、正常に動作するようになります。
  - ほかの number helper (`number_to_currency` など) と同じ挙動に揃えられるため、一貫性が向上します。
- `precision: nil` の扱いそのもの（出力フォーマットやデフォルト精度の解釈）が変わったわけではなく、「落ちずに従来どおりのロジックに乗るようになった」修正です。
- 差分は `RoundingHelper` 内の 1 行変更とテスト 2 行追加のみで、小さく局所的です。他の helper の動作への副作用はほぼ考えにくいです。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57891  
- 関連クラス・モジュール:
...
- 実際に問題になっていた呼び出し例:
  ...
  ```ruby
  # 以前は NoMethodError だったが、この PR マージ後は正常に動く
  number_to_human(123456, precision: nil)
  number_to_human_size(123456, precision: nil)
  ```
...
## [#57889](https://github.com/rails/rails/pull/57889) Fix incorrect RDoc examples across frameworks {#pr-57889}

**マージ日**: 2026/6/27 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
このPRは、Rails 各フレームワークに散在していた RDoc の誤ったサンプルコードや出力例を、実際の挙動に合わせて修正する「コメントのみ([ci skip])」の変更です。コードロジックには一切手を入れておらず、ドキュメントの信頼性・再現性を高めることが目的です。
...
#### `errors.rb`
- 誤り: `errors.messages` の戻り値が Array であるかのような例。
- 修正: 実際は「属性名をキーにした Hash」を返すことを反映。
...
#### `serializers/json.rb`
- 誤り: `as_json` の例に不正なタイムスタンプ (`17:27:133.000Z`) が書かれていた。
- 修正: 実在する形式のタイムスタンプ (`17:27:13.000Z`) に修正。

#### `validations.rb`, `validations/validates.rb`
- 誤り: `:except_on` オプションの例で `except:` を使用していた。
- 修正: 実際のオプション名である `except_on:` に修正。
...
- 誤り: `AUTHN_PAIR_DELIMITERS` が `:` 区切りであるかのような記述。
- 修正: 実際には `,` 区切りであることを反映。

#### `strong_parameters.rb`
- 誤り: `UnpermittedParameters` 例外メッセージの文言が実装と不一致。
- 修正: 実際のメッセージ `"found unpermitted parameters: :a, :b"` に合わせて更新。

```ruby
# 実際の例外メッセージに一致させた
raise ActionController::UnpermittedParameters.new([:a, :b])
# => found unpermitted parameters: :a, :b
```
...
#### `content.rb`
1. `to_plain_text` の挙動:
   - 誤り: `<script>` タグ内部のテキストも含まれる例 (`"safeunsafe"`)。
   - 修正: 実際は `<script>` 内の内容を落とすため `"safe"` だけになることを示す。

2. `Content.new` の使い方:
   - 誤り: 引数の渡し方が実際のシグネチャに合っていないサンプル。
   - 修正: 実装どおり「位置引数」で渡す例に変更。

```ruby
content = ActionText::Content.new("<p>safe<script>unsafe</script></p>")
...
#### `continuation/test_helper.rb`
- 誤り: `step :x { }` という構文例（Ruby 的に構文エラー）。
- 修正: 実際に通る構文 `step(:x) { }` に修正。
...
#### `variant.rb`
- 誤り: リダイレクトに `service_call` を使っているかのような例。
- 修正: 実際には `url` メソッドを利用してリダイレクトする例に変更。

```ruby
redirect_to @user.avatar.variant(resize_to_limit: [100, 100]).url
```

#### `mirror_service.rb`
- 誤り: プライマリサービスの存在確認メソッドを `exists?` としていた。
- 修正: 実際のメソッド名 `exist?` に合わせる。
...
#### `configurable.rb`
- 誤り: `config_accessor` を定義していないクラスの例で、`User.allowed_access` というリーダを直接呼んでいた。
- 修正: となりの `#config` の例と同様に `User.config.allowed_access` 経由で読む形に修正。
...
#### `notifications.rb`
- 誤り: サブスクライバが `matches?` メソッドを持っているかのような説明。
- 修正: 実際の API どおり `subscribed_to?` を使う例に変更。

```ruby
subscriber.subscribed_to?("process.action_controller") # => true/false
```

### Action Mailbox

#### `base.rb`
- 誤り: `User.exist?` を呼んでいる例。
- 修正: Active Record の実際のメソッド名 `exists?` に修正。
...
#### `mail_helper.rb`
- 誤り: `format_paragraph` のラップ後の出力例が実装と不一致。
- 修正: 実際の折り返し結果に合わせた例に書き換え。

```ruby
format_paragraph("long long text", 20)
# => 実装どおりに折り返されたテキストに一致
```

### Action Cable

#### `connection/test_case.rb`
- 誤り: コネクション用テストヘルパの説明文が、「チャネル名から推論する」と書かれていた（コピペミス）。
- 修正: 正しく「テストクラス名からコネクションを推論する」と説明。

```ruby
class ChatConnectionTest < ActionCable::Connection::TestCase
  # Connection クラスはテストクラス名から推論される
end
```
...
#### `engine.rb`
- 誤り: すべてのヘルパをインクルードするメソッドを `helper` と解説。
- 修正: 実際のメソッド名 `helpers` に修正。
...
#### `paths.rb`
- 誤り: `root[...]` の戻り値をいきなり `Array` として扱っている例。
- 修正: 実際は `Rails::Paths::Path` を返すので、例では `to_ary` を呼んで `Array` を得る形に修正。

```ruby
Rails.application.config.paths["app/controllers"].to_ary
...
- すべてコメント（RDoc）だけの修正であり、挙動・API 互換性・パフォーマンスには影響しません。
- ただし、**過去にこれらの RDoc サンプルをコピペして使っていた場合**:
  - `exist? / exists?` や `except_on:` といったメソッド名・オプション名の誤用
  - `step :x { }` のような構文エラー
  - `errors.messages` の戻り値型の誤解
  - Strong Parameters 例外メッセージのテストで文言不一致  
 などが起きていた可能性があります。該当箇所があれば、この PR の内容を参考にコードを見直すと安全です。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57889  
- 統合元 PR 群: #57878–#57887（フレームワークごとの RDoc 修正を一つにまとめたもの）  
- 実行コードには手を入れていないため、CHANGELOG レベルの破壊的変更ではなく、ドキュメント整合性の改善と考えてよいです。

---


## [#57868](https://github.com/rails/rails/pull/57868) Fix incorrect RDoc examples in ActionView form helpers {#pr-57868}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
ActionView の form ヘルパーに記載されていた RDoc のサンプルコードが、実際の出力と食い違っていた箇所を修正した PRです。`month_field` / `week_field` と `select` ヘルパーのドキュメント上の出力例のみが変わっており、実装ロジックには変更はありません。

---

2. 変更内容の詳細  

### `form_helper.rb` — `month_field` / `week_field` の RDoc 修正

`month_field` と `week_field` の「デフォルト値」の例が、実際の実装と異なる `type="date"` になっていたのを修正しています。
...
```ruby
# 設定例
month_field(:user, :born_on, value: "1984-01")
...
実装としては `ActionView::Helpers::Tags::MonthField` を使っており、`type="month"` が正しいため、それに合わせた形です。  
同一メソッド内の別の例ではすでに `type="month"` になっており、その整合性を取った形でもあります。
...
```ruby
# 設定例
week_field(:user, :born_on, value: "1984-W19")
...
こちらも実装上は `ActionView::Helpers::Tags::WeekField` で `type="week"` を出力しており、それに合わせています。

---

### `form_options_helper.rb` — `select` の multiple オプションの RDoc 修正

`select` ヘルパーで multiple セレクトを使う例で、HTML の出力例が実装と異なっていました。
...
Rails のタグビルダーでは、boolean 属性は `true/false` を文字列化するのではなく、属性名を値として繰り返す形（`multiple="multiple"`, `disabled="disabled"` など）で出力します。この PR は、その実装にドキュメントを合わせるための修正です。

---

3. 影響範囲・注意点  

- 実装コードは一切変更されておらず、すべて RDoc コメントの修正のみです。アプリケーションの挙動に影響はありません。
- これまで RDoc のサンプルをそのままコピペしてテストや説明資料に使っていた場合:
  - `month_field` / `week_field` について、`type="date"` になっている古い例は誤りなので、`type="month"` / `type="week"` に修正する必要があります。
  - `select` の multiple の例で `multiple="true"` を期待するテストを書いていた場合は、`multiple="multiple"` を期待するよう修正が必要です。
- 既存の HTML 仕様/慣習としても、boolean 属性に文字列 `"true"` を入れるのは一般的ではないため、この修正は Rails の通常の挙動と HTML の慣習の両方に整合的です。
...
- Rails ガイド（フォームヘルパー）:  
  ...
  ※ 今回の RDoc 修正は、将来的にガイドのサンプルや各種記事の整合性にも影響しうるため、ガイドを参照する際も `type` 値や boolean 属性の扱いに注意するとよいです。
- HTML boolean 属性仕様 (WHATWG HTML):  
  https://html.spec.whatwg.org/multipage/common-microsyntaxes.html#boolean-attributes

---


## [#57869](https://github.com/rails/rails/pull/57869) Fix stale instance variable in `stale?` RDoc example {#pr-57869}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionController::ConditionalGet` の RDoc 中の `stale?` の単一レコード例で、未定義のインスタンス変数 `@articles` を参照していた誤記を、正しく `@article` に修正したドキュメント向けの変更です。実装コードではなく、サンプルコードのみの修正です。
...
問題になっていたサンプル（RDoc内のコメント）は、「単一レコードの `stale?` 使用例」で、次のようなコードが書かれていました:
...
- `@article = Article.find(params[:id])` で単一レコードを取得
- `stale?(@article, ...)` でも `@article` を渡している

にもかかわらず、`if` ブロック内で `@articles`（複数形）を参照していました。`@articles` はこの例の中では一度も定義されておらず、単純なタイポ(コピペミス)です。

この PR では、その行を以下のように修正しています:
...
同じファイル内にはコレクション向けの `stale?` 例として `@articles = Article.all` を使うサンプルもあり、そちらでは `@articles` が正しく使われているため、今回の PR では変更されていません。

---

3. 影響範囲・注意点  

- 実行時の挙動には一切影響せず、「ドキュメント内のサンプルコード」のみの修正です。
- ただし、このサンプルをコピペして使っていた場合、以前は `NameError`（未定義の `@articles`）を招く可能性がありました。今後はそのような誤りを避けられます。
- `stale?` を単一レコードで使う場合は、**レコードを格納したインスタンス変数と同じものを `stale?` に渡し、同じ変数をその後の処理でも使う**、という意図がより明確になりました。

---

4. 参考情報 (あれば)  

- 対象モジュール: `ActionController::ConditionalGet`
  - `stale?(record_or_options, **response_options)` は、ETag / Last-Modified を用いた条件付き GET を簡単に扱うためのヘルパです。
- 関連する他のサンプル:
  - 同ファイル内に、コレクション (`@articles = Article.all`) を使った `stale?` の例もあり、そちらは `@articles` が正しい使い方として残されています。

---


## [#57867](https://github.com/rails/rails/pull/57867) Fix incorrect RDoc output examples in ActiveSupport {#pr-57867}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
ActiveSupport の RDoc (ドキュメント用コメント) に記載されていたサンプル出力が、実際のコードの挙動と食い違っていたため、それを修正した PR です。コードの動作自体には変更がなく、あくまでドキュメントの例の整合性を取る修正です。

---

2. 変更内容の詳細

### 2-1. `Hash#assert_valid_keys` のエラーメッセージ例を修正

対象ファイル: `activesupport/lib/active_support/core_ext/hash/keys.rb`

`assert_valid_keys` の RDoc にある例が、実際に発生するエラーメッセージと異なっていたため修正されています。
...
`assert_valid_keys` は内部で

```ruby
valid_keys.map(&:inspect)
```

を使ってメッセージを構築しています。`"name".inspect` は常にダブルクォート付き `"name"` を返すため、実際のメッセージは `"name", "age"` という形式になります。

もともとの例では `'name', 'age'` とシングルクォートで書かれていたため、実際に raise されるメッセージと食い違っていました。なお、同じファイル内のシンボルキー版の例はすでに正しい形式になっており、この例だけが不整合だったものを揃えた形です。

---

### 2-2. `EncryptedConfiguration#config` の返り値例を修正

対象ファイル: `activesupport/lib/active_support/encrypted_configuration.rb`

`EncryptedConfiguration#config` の RDoc における返り値の例が、同じクラス内の他の例と数値だけ異なっていたため修正されています。
...
`config` メソッドは単に `read` の結果をデシリアライズして返すメソッドです。RDoc 内の別の箇所では、`read` の例やクラス説明、`keys` の例などで `another_secret` の値として `456` が使われており、この `config` の例だけ `789` になっていました。

- `read` の例: `another_secret: 456`
- クラス説明・`keys` の例: 同様に `456`
- `config` の例: なぜか `789` → 今回 `456` に修正

これにより、ひとつのストーリーとして示されている暗号化設定の例が全て同じ値 (`456`) で整合するようになります。

---

3. 影響範囲・注意点

- 実行コードには一切変更がなく、挙動やパブリック API に影響はありません。
- 影響するのは RDoc で生成されるドキュメント、およびドキュメントを参考にする開発者のみです。
- エラーメッセージをテストで厳密に比較している場合:
  - もともと `"name", "age"` 形式で出力されており、PR はその通りにドキュメントを直しただけなので、既存テストを壊すような変更はありません。
- EncryptedConfiguration のチュートリアルや社内ドキュメントで `789` の例をそのまま引用している場合、今回の修正に合わせて `456` に揃えると Rails 本体の RDoc と一貫性が取れます。

---

4. 参考情報 (あれば)

- PR: https://github.com/rails/rails/pull/57867  
- 変更ファイル:
  - `activesupport/lib/active_support/core_ext/hash/keys.rb`
  - `activesupport/lib/active_support/encrypted_configuration.rb`  
- `assert_valid_keys` の実装 (参考用・バージョンによって細部は異なる可能性あり):
  ...
  この `valid_keys.map(&:inspect)` の挙動が、今回の RDoc 修正の根拠となっています。

---


## [#57871](https://github.com/rails/rails/pull/57871) Fix `remove_check_constraint` with `if_exists` for the expression form {#pr-57871}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`remove_check_constraint` を式（expression）指定で使った場合に、`if_exists: true` が no-op にならず `ArgumentError` を投げていた不具合を修正した PR です。抽象アダプタと SQLite3 アダプタの両方で、存在確認時にも expression を正しく渡すようにしています。
...
以下のような migration を書いたとします:

```ruby
remove_check_constraint :products, "price > 0", if_exists: true
```

本来 `if_exists: true` は、指定したチェック制約が存在しなければ何もしない (no-op) 挙動を期待しますが、実際には存在しない場合に `ArgumentError` が発生していました。また、存在している場合でも expression での検索が正しく行われていませんでした。

### 原因

`remove_check_constraint` は（制約を実際に削除する段階では）以下のように expression を渡して制約を特定していました:

```ruby
check_constraint_for!(table_name, expression: expression, **options)
```

しかし、削除前に `if_exists` 用の存在確認を行う部分では、expression を渡していませんでした:

```ruby
return if if_exists && !check_constraint_exists?(table_name, **options)
```

`expression` はメソッドの位置引数であり `**options` には含まれていないため、`check_constraint_exists?` は `name` も `expression` も渡されない状態で呼ばれます。その結果:

```text
ArgumentError: At least one of :name or :expression must be supplied
```

が発生していました。これは `if_exists: true` の意味する「存在しなければ静かにスキップする」という仕様と正反対の挙動です。

このバグは以下 2 箇所で同様に存在していました。

- 抽象アダプタ: `activerecord/lib/active_record/connection_adapters/abstract/schema_statements.rb`
- SQLite3 アダプタ: `activerecord/lib/active_record/connection_adapters/sqlite3/schema_statements.rb`  
  （こちらは `remove_check_constraint` をオーバーライドしているため、同じ問題を別実装で抱えていた）

### 修正内容

両アダプタの `remove_check_constraint` において、`if_exists` チェック時にも expression を渡すように 1 行ずつ修正されています。
...
- 削除対象の制約を探すロジック (`check_constraint_for!`)
- 存在確認のロジック (`check_constraint_exists?`)

がどちらも同じキー (`expression: "price > 0"`) で制約を特定するようになりました。

### テスト

`activerecord/test/cases/migration/check_constraint_test.rb` にテストが 4 行追加され、次のようなケースがカバーされています（要約）:

- 式を指定した `remove_check_constraint :products, "price > 0", if_exists: true` が、制約非存在時でも `ArgumentError` にならずに no-op となる
- （加えて、存在する場合に正しく削除されることも保証していると考えられる）
...
- 対象:
  - `remove_check_constraint` を **expression 形式**（例: `"price > 0"`）で呼び出しているコード
  - かつ `if_exists: true` オプションを併用しているケース
  - これを利用する全てのアダプタ（少なくとも抽象アダプタ経由のもの）と SQLite3 アダプタ

- 期待される挙動:
  - 制約が存在しない場合:
    - 以前: `ArgumentError` が発生して migration が落ちる  
    - 修正後: 例外を投げずに何もせずスキップ（no-op）
  - 制約が存在する場合:
    - expression を用いて適切に対象制約が特定され、削除される

- 互換性:
  - `if_exists: true` を指定していない既存コードの挙動は変わりません。
  - すでに name 指定で `remove_check_constraint :products, name: "chk_price_positive"` を使っていたコードにも影響はありません。
  - バグ修正の範囲にとどまっており、API 仕様の変更ではありません。

- 注意点:
  - これまでこのバグに依存して「存在しないことをエラーで検出する」というような変則的な運用をしていた場合は、挙動が変わり例外が出なくなります（ただし `if_exists: true` を指定している以上、通常は no-op が期待される場面なので、そのような依存は非推奨と言えます）。
  - 他のアダプタ（PostgreSQL, MySQL など）が独自に `remove_check_constraint` をオーバーライドしている場合、その実装にも同種の問題がないか確認すると安心です。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57871  
- 対象メソッドのドキュメント（Rails ガイド / API ドキュメント）では、reversible migration のために expression ベースの `remove_check_constraint` が推奨されており、その公式な使用例が今回の修正で期待どおりに動くようになります。

---


## [#57872](https://github.com/rails/rails/pull/57872) Fix reversibility check for `drop_virtual_table` {#pr-57872}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`drop_virtual_table` のマイグレーションを逆変換する際の引数チェックが誤っており、本来は不可逆であるべき操作が「可逆」と判定されてしまう不具合を修正した PR です。これにより、`drop_virtual_table` の逆操作として生成される `create_virtual_table` が正しく実行可能かどうかを正しく判定できるようになりました。
...
`drop_virtual_table` の定義は以下のようなシグネチャを持っています（PR の説明から）:

```ruby
drop_virtual_table(table_name, module_name, values, **options)
# 第1引数: table_name
# 第2引数: module_name
# 第3引数: values (カラム定義などに相当)
```

CommandRecorder が `drop_virtual_table` を記録した操作を逆変換するときに使うメソッドが `invert_drop_virtual_table` ですが、このメソッドが **引数の分解を誤っていた** ため、可逆性のチェックが正しく行われていませんでした。

既存コード（問題のある実装）は概ね以下のような形でした:
...
- `drop_virtual_table` の第3引数が `values` であるにもかかわらず、
- `invert_drop_virtual_table` では **第2引数** を `values` として扱っていた  
  （これは `drop_enum` / `invert_drop_enum` からのコピペで、そちらは第2引数が `values`）

結果として:

- `values` 変数には本来の `values` ではなく `module_name` が入ってしまう
- `module_name` は必須のため、常に「存在する」扱いになる
- したがって、本来 `values` が無くて「逆変換できない = IrreversibleMigration」とすべきケースでも、ガードが発火せず、`create_virtual_table` を生成してしまう
- その `create_virtual_table` は実際には実行に必要な `values` 情報を欠いているため、**リプレイ不能な逆マイグレーション** が生成されてしまう

### 修正内容

PR では、`invert_drop_virtual_table` 内で **第3引数** を `values` として正しく取り出すように修正しています。

実際の差分は 1 行だけですが、概念的にはこんな形になります（擬似例）:

```ruby
def invert_drop_virtual_table(args)
  table_name, module_name, values = args.dup.tap(&:extract_options!)

  # values（第3引数）が無い場合は逆変換できないので例外
  raise ActiveRecord::IrreversibleMigration, "..." unless values
...
あわせてテスト (`activerecord/test/cases/migration/command_recorder_test.rb`) では：

- `drop_virtual_table` を `values` なしで記録した場合、
- その操作を逆変換しようとすると `ActiveRecord::IrreversibleMigration` が発生する

ことを検証するテストが追加されています (+6 行)。

---

3. 影響範囲・注意点  

- 対象: ActiveRecord のマイグレーションにおける `drop_virtual_table` の逆変換（`rake db:rollback` 等で使われる処理）
- 影響:
  - 以前の挙動:
    - `drop_virtual_table` を `values` なしで呼び出しても、CommandRecorder 的には「可逆」と誤判定され、`create_virtual_table` が逆操作として記録されていた。
    - しかし、その `create_virtual_table` には必要な `values` がないため、実際にロールバックしようとすると失敗する or 不完全なテーブルが作られる可能性がある。
  - 修正後の挙動:
    - `drop_virtual_table` に `values` が提供されていない場合、その操作は **不可逆** とみなされ、`invert_drop_virtual_table` は `ActiveRecord::IrreversibleMigration` を投げる。
    - これにより、「ロールバックできる」と誤解させず、早い段階で不可逆性に気づけるようになる。
- マイグレーションを書く側の注意点:
  - 「後でロールバックしたい」前提で `drop_virtual_table` を使う場合、**必ず第3引数の `values`（カラム情報など）を渡す必要がある**。
  - そうでない場合、この PR 適用後はロールバック時に `ActiveRecord::IrreversibleMigration` が発生する。
  - 既存プロジェクトで、`values` を省略した `drop_virtual_table` をすでに使っている場合、  
    - これまでは「一見ロールバック可能に見えるが、実際には動かない/おかしい」状態  
    - 今後は「ロールバック時に明示的に例外が出る」状態  
    に変わるため、挙動が変わる点に注意が必要です。

---

4. 参考情報 (あれば)  

- 類似の実装: `drop_enum` / `invert_drop_enum`  
  - `drop_enum(name, values, **options)` のように、第2引数が `values` であり、本 PR の実装はこれをコピペしたことで引数位置がずれたのが原因と説明されています。
- 関連コード:  
  - `activerecord/lib/active_record/migration/command_recorder.rb`  
    - 各種マイグレーションコマンドの記録と逆変換 (`invert_...`) のロジックがまとまっているファイルです。
- 実務上のベストプラクティス:
  - ロールバック可能なマイグレーションを意識する場合、**drop 系の操作には必ず再作成に必要な情報（カラム・オプションなど）を付与する**のが安全です。
  - 付与できない場合は、明示的に「不可逆マイグレーション」であることをコメントなどで示しておくと、今回のような可逆性の誤検知によるトラブルを避けられます。

---


## [#57873](https://github.com/rails/rails/pull/57873) Fix `unique_constraint_exists?` to accept `:column` instead of `:expression` {#pr-57873}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
PostgreSQL アダプタの `unique_constraint_exists?` が `column:` オプションを正しく受け付けず `ArgumentError` を投げていたバグが修正されました。ガード条件とエラーメッセージが、ユニーク制約の API 仕様（`:column` または `:name`）に整合するようになっています。
...
PostgreSQL のユニーク制約は「カラム」もしくは「名前」で識別されますが、既存実装の `unique_constraint_exists?` は以下のようになっていました:
...
- ユニーク制約の実際の API は `:column` または `:name` を受け付ける設計
- しかしガードは `:name` / `:expression` のどちらか必須としてチェック
- そのため、以下のような正当な呼び出しが例外を投げていました:

```ruby
unique_constraint_exists?(:users, column: :email)
# 本来 true/false を返すべきところで ArgumentError
```

`:expression` は排他制約（`exclusion_constraint_exists?`）向けのオプションで、ユニーク制約では使われません。このメソッドのガードは `exclusion_constraint_exists?` からコピペされたため、API 仕様と整合していませんでした。

### 何がどう直ったか

修正後は、ガード条件とエラーメッセージをユニーク制約用に正しく変更しています:

- チェック対象を `:expression` → `:column` に変更
- エラーメッセージも `"At least one of :name or :column must be supplied"` 的な内容に修正（PR 説明より）
...
これにより、すでに存在している他メソッドとの整合が取れます:

- `unique_constraint_for` は `options.key?(:column)` で枝分かれ
- `remove_unique_constraint` も `column:` で制約を特定

テスト (`unique_constraint_test.rb`) も追加されており、`column:` ベースの呼び出しが例外を発生させず、正しく true/false を返すことが確認されています。

#### 使用例

今回の修正後に期待される正しい振る舞いの例:

```ruby
# カラムで確認
unique_constraint_exists?(:users, column: :email)  # => true / false

# 名前で確認
unique_constraint_exists?(:users, name: "index_users_on_email")  # => true / false

# name / column どちらもない場合は ArgumentError
unique_constraint_exists?(:users)
# => ArgumentError (At least one of :name or :column must be supplied)
```
...
- 対象:
  - PostgreSQL アダプタの `ActiveRecord::ConnectionAdapters::PostgreSQL::SchemaStatements#unique_constraint_exists?`
- 主な影響:
  - これまで `column:` を指定して `unique_constraint_exists?` を使うと常に `ArgumentError` になっていたコードが、正しく boolean を返すようになります。
  - 内部で使っている `unique_constraint_for` / `remove_unique_constraint` と API が揃うため、一貫した挙動を期待できます。
- 互換性:
  - 既に `name:` だけを使っていたコードには挙動の変化はありません。
  - `expression:` を渡していた場合は、そもそもユニーク制約としては意味のない指定だったため、今後はエラーになるか、少なくとも推奨されません（ユニーク制約は式ベースではないため）。
- テスト:
  - `unique_constraint_exists?` が `column:` オプションを受け取るパスがテストでカバーされるようになり、退行を防止します。
...
- 類似 API:
  - 排他制約: `exclusion_constraint_exists?(table, name: ..., expression: ...)`
    - こちらは式ベース（`expression:`）が正当なオプション
  - ユニーク制約:
    - 追加: `add_unique_constraint :table, :column, name: ...`
    - 削除: `remove_unique_constraint :table, column: ..., name: ...`
    - 存在確認（今回修正対象）: `unique_constraint_exists?(:table, column: ..., name: ...)`

この PR により、ユニーク制約系メソッドのインターフェースが統一され、`column:` を起点とした存在確認が安心して使える状態になっています。

---


## [#57842](https://github.com/rails/rails/pull/57842) Make ActiveSupport::JSON encoders Ractor-shareable {#pr-57842}

**マージ日**: 2026/6/26 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1-2文で)  
ActiveSupport::JSON のエンコーダを Ractor 共有可能（Ractor-shareable）にし、Ractor 環境下でも安全に再利用できるようにした PRです。併せて、その挙動をテストするために Isolation テストを導入し、Ruby 本体のクラッシュを回避するようにしています。

---

2. 変更内容の詳細  

### ActiveSupport::JSON encoder を Ractor-shareable に

`activesupport/lib/active_support/json/encoding.rb` の変更点はおおまかに言うと以下です。

- JSON エンコーダ（主に `ActiveSupport::JSON::Encoding` 周り）の実装を、Ractor 間で安全に共有可能なオブジェクトとして扱えるようにリファクタリング
- Ractor から参照される可能性のあるオブジェクトについて、
  - 変更可能なグローバル状態を持たない
  - もしくは Ractor 内ローカルに複製されるようにする
- これにより、`ActiveSupport::JSON.encode` などを Ractor 内から呼び出しても、Ractor 安全性の制約に反しないようにする

Ruby 3 以降の Ractor では、  
「shareable オブジェクトしか Ractor 間で渡せない / 参照できない」という制約があります。  
これまでの実装では、JSON エンコーダが内部的にミューテーブルなオブジェクトやグローバル状態に依存しており、Ractor から利用するとエラーやクラッシュの原因になるケースがありました。  
今回の PR では、そうした依存を排除ないし Ractor セーフな形に整理することで、「エンコーダ自体」を Ractor-shareable にしています。

※ PR タイトルから推測すると、「エンコーダのインスタンス」または「エンコーダを構成する定数・設定情報」を shareable にする変更が中心です。例えば以下のような方向性の変更が典型的です（実際のコードは概念的説明）:

```ruby
module ActiveSupport
  module JSON
    module Encoding
      # 以前: @@global_encoder = SomeMutableObject.new
      # 以後: GLOBAL_ENCODER = SomeImmutableStruct.new.freeze
      # などの形で freeze して shareable にする
    end
  end
end
```

### テスト: ActiveSupport::Testing::Isolation の利用

`activesupport/test/json/encoding_test.rb` にテストが追加されています。

- 目的:
  - Ractor から ActiveSupport::JSON エンコーダを利用したときに動作するか
  - Ruby のバグや VM レベルの問題によるクラッシュを避けつつテストを実行すること
- 手段:
  - `ActiveSupport::Testing::Isolation` を利用して、テストを別プロセス／別 VM 的なコンテキストで実行
  - これにより、Ractor を使ったテストによってメインのテストプロセスが巻き込まれてクラッシュすることを防ぐ

Isolation テストは、Rails がテストスイート内で「危険な操作」（fork、Ractor、グローバル状態の大きな変更など）を扱うときによく使うパターンです。この PR では、以前の PR #57814 で Ruby 側のクラッシュに遭遇したため、それを回避する「Take 2」として Isolation を採用しています。

#### 参考になりそうなテストイメージ（概念的な例）

```ruby
class JsonEncodingRactorTest < ActiveSupport::TestCase
  include ActiveSupport::Testing::Isolation

  def test_json_encoder_is_ractor_shareable
    r = Ractor.new(ActiveSupport::JSON) do |json_mod|
      json_mod.encode(foo: "bar")
    end

    assert_equal '{"foo":"bar"}', r.take
  end
end
```

上記はあくまで説明用ですが、実際のテストも同様に Ractor 内で `ActiveSupport::JSON` を利用する形になっているはずです。

### strict warnings の更新

`tools/strict_warnings.rb` に +3 行の変更があります。

- 新たに追加したコードが Ruby の warning ポリシー（未使用変数・再定義など）に引っかからないよう、strict warnings の設定や除外を調整
- Rails コアのスタイルとしてテスト・ライブラリ両方で警告を抑制し、クリーンなビルドを保つためのメンテナンス的変更
...
- 直接の対象:
  - `ActiveSupport::JSON` / `ActiveSupport::JSON::Encoding` を利用して JSON エンコードしているコード
  - 特に、Ruby 3 以降で Ractor を使うアプリケーションや gem

- 期待される効果:
  - Ractor 内で `ActiveSupport::JSON.encode` 等を呼び出しても、Ractor の shareable 制約違反や予期しないクラッシュが起きにくくなる
  - JSON エンコーダに関するグローバル状態が整理されることで、マルチスレッド・マルチ Ractor 環境でもより予測可能な挙動になる

- 互換性の観点:
  - 行数規模は小さいものの、JSON エンコード周辺の内部実装に手が入っているため、
    - カスタムのエンコーダを `ActiveSupport::JSON::Encoding` の内部構造に強く依存させている場合
    - モンキーパッチで JSON エンコーダの内部定数・クラス変数を直接書き換えている場合
    などでは影響が出る可能性があります。
  - 通常の `to_json` / `as_json` を使っているだけのアプリケーションであれば、挙動の変化はほとんどない想定です。

- 注意点:
  - この PR は Ruby 本体側の Ractor 実装の安定性に依存します。Isolation によってテスト上のクラッシュは避けていますが、
    - 使用している Ruby のバージョン
    - そのバージョン固有の Ractor 周りのバグ
    によっては、実運用環境で異なる挙動を示す可能性があります。
  - Ractor 本番利用を検討している場合は、この変更を取り込んだ Rails バージョンで、アプリケーションの JSON 生成パスを Ractor 下で実際に負荷テストしておくのが無難です。
...
- この PR の前身: #57814  
  - 同様に ActiveSupport::JSON の Ractor 対応を試みたが、Ruby のクラッシュにより断念 → 今回は Isolation テストで再挑戦
- 関連ドキュメント:
  - Ruby Ractor: https://docs.ruby-lang.org/en/master/Ractor.html  
  - Rails ActiveSupport::JSON:
    - https://api.rubyonrails.org/classes/ActiveSupport/JSON.html  
    - https://api.rubyonrails.org/classes/ActiveSupport/JSON/Encoding.html  
- Ractor-shareable の要件:
  - `Ractor.shareable?(obj)` が `true` を返すオブジェクトは、Ractor 間で安全に共有可能
  - そのためには、基本的にイミュータブルかつグローバルに変更されないデータ構造である必要がある（今回の PR は主にその要件を満たすようにエンコーダ実装を整理していると考えられます）。

---


## [#57845](https://github.com/rails/rails/pull/57845) Report PostgreSQL default timestamp/time precision as 6 {#pr-57845}

**マージ日**: 2026/6/26 | **作成者**: [@adrianna-chang-shopify](https://github.com/adrianna-chang-shopify)

1. 概要 (1-2文で)  
PostgreSQL の `timestamp` / `time` 系カラムについて、typmod が省略された「素の」型 (`timestamp` など) でも実際にはマイクロ秒精度(6桁)で動いていることを、Active Record のメタデータ上でも `precision: 6` として正しく報告するようにした変更です。これにより、DBに保存された値と Active Record での型キャスト結果の精度が揃い、タイムスタンプの扱いが一貫します。
...
- PostgreSQL では `timestamp` や `time` の型定義で精度を指定しない場合でも、実際の挙動は「マイクロ秒精度 (6桁)」。
- しかし PostgreSQL はその場合 typmod（型修飾子）情報を省略するため、Active Record 側から見ると「precision 不明」に見えうる。
- この PR は、PostgreSQL アダプタが typmod を正しく参照し、`fmod == -1`（= typmod 省略・bare 型）のときに `precision: 6` と報告することで、DB の実挙動と AR メタデータ・型キャストを揃えるもの。
...
- これまで: PostgreSQL から返される「フォーマット済みの型文字列」（例: `"timestamp without time zone"`）をパースして精度を推定していた。
- これから: PostgreSQL タイプマップから渡される「typmod (fmod)」を直接使うように変更。  
  - `fmod == -1` の場合:
    - 「bare な timestamp/time 型」と見なし、**precision を `6` とみなす**。
  - `fmod != -1` の場合:
    - typmod から計算される「明示された精度」を使用。

結果として、

- `timestamp`        → `precision: 6`
- `timestamp(3)`     → `precision: 3`
- `time`             → `precision: 6`
- `time(0)`          → `precision: 0`

といった形で、**DB が実際に扱う精度と Active Record の precision が一致**するようになります。
...
- `activerecord/test/cases/adapters/postgresql/timestamp_test.rb`
  - PostgreSQL アダプタにおける bare/explicit timestamp 系カラムの precision の振る舞いを検証。
- `activerecord/test/cases/date_time_precision_test.rb`
- `activerecord/test/cases/time_precision_test.rb`
  - datetime/time 型の precision 周りの期待値を、PostgreSQL のデフォルト 6 桁精度に合わせて修正。
- `activerecord/test/cases/defaults_test.rb`
- `activerecord/test/cases/schema_dumper_test.rb`
- `activerecord/test/cases/migration/compatibility_test.rb`
  - precision 情報や schema_dump/migration の互換性関連テストを、今回の精度報告の変更に合わせて更新。

`activerecord/CHANGELOG.md` には、PostgreSQL の timestamp/time precision レポートが 6 になるよう変更されたことが追記されています。
...
- **カラムメタデータの precision**  
  - `connection.columns(:table_name).find { … }.precision` の戻り値が、bare な `timestamp` / `time` カラムで `nil` (もしくは 0 相当) から `6` に変わる可能性があります。
  - precision に応じてアプリ側のフォーマットや丸め処理を分岐している場合は、挙動が変わるか確認が必要です。

- **schema dumper / migration 生成との整合性**  
  - schema dumper がカラム定義を出力する際、PostgreSQL の実際のデフォルト精度に即した情報が利用されるようになります。
  - これにより、将来の migration やスキーマ再構築時に、DB の実挙動とギャップが出る可能性が減りますが、既存期待値テストが precision の値をハードコードしていると失敗することがあります。

- **タイムスタンプの型キャストの一貫性**  
  - Active Record が内部で timestamp/time をキャストする際、DB に保存されている値と同じマイクロ秒精度を前提として扱うようになるため、アプリ内での時間比較・シリアライズ/デシリアライズの結果がより安定します。
  - 逆に言うと、今まで「精度不明」扱いに依存していたコード（例: 秒単位に丸められることを前提にした処理）があると挙動が変わる可能性があります。

総じて、**PostgreSQL を使っていて、timestamp/time の precision をメタデータ経由で参照しているコード・テスト**は、この変更を前提に見直しが必要になりえます。

---

4. 参考情報 (あれば)  

- PostgreSQL の timestamp/time 精度仕様  
  - `timestamp` / `time` は、精度を指定しない場合デフォルトで microsecond (6 桁) 精度を持つ。
  - typmod が省略される (`fmod == -1`) のは、この「デフォルト精度」を意味する。
- Active Record 側の意図  
  - adapter は DB の実際の挙動を忠実に反映し、AR のカラムメタデータと型キャストをそれに合わせるべき、という設計方針に沿った修正。

---


## [#57866](https://github.com/rails/rails/pull/57866) Fix incorrect RDoc output examples in ActiveRecord {#pr-57866}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Active Record の RDoc 内のコード例が、実際の挙動と矛盾していたため、それを正しい出力例に修正した PR です。挙動そのものの変更はなく、あくまでドキュメントの誤記修正です。

---

2. 変更内容の詳細  

### (1) `associations/collection_proxy.rb` の `find` ブロック付き例

対象: `ActiveRecord::Associations::CollectionProxy#find` の RDoc 例
...
背景説明:
- 同じドキュメントブロック内のフィクスチャ定義では、`id: 2` のペットは `name: "Spook"` となっている。
- `pet.name.downcase!` によって `"Spook"` は `"spook"` になるのが正しい。
- `"fancy-fancy"` は `id: 1` の `"Fancy-Fancy"` を downcase した結果であり、`id: 2` として出てくるのは不整合。
- したがって、**フィクスチャとコード例の整合性をとるために、期待出力を `"spook"` に修正**している。

この変更により、`find` にブロックを渡したときの挙動（返されるオブジェクトに対して副作用を与える例）が、実際に再現可能な内容になります。

---

### (2) `sanitization.rb` の `sanitize_sql_for_conditions` 名前付きバインド例

対象: `ActiveRecord::Sanitization#sanitize_sql_for_conditions` の RDoc 例（名前付きバインド）
...
ポイント:
- Active Record の SQL サニタイズでは、**整数値のバインドはクォートされない**（文字列のみ `'...'` で囲われる）。
- 直前の「位置指定バインド」の例では既に `group_id=4`（クォートなし）になっている。
- `sanitize_sql_array` の同様の名前付きバインド例でも `group_id=4` になっており、今回の例だけが `"group_id='4'"` と誤っていた。
- 一方、`"group_id='%s'"` のようにテンプレート文字列内にクォートが書かれている例では、**そのクォートはテンプレート由来のリテラル**なので `'4'` になるのが正しい。この点は既存通りで問題なし。

この修正により、**ドキュメント中の named bind の例が実際の SQL 生成仕様（整数はクォートしない）と一致**します。

---

3. 影響範囲・注意点  

- 実装コードには一切手が入っておらず、**挙動変更はありません**。  
- 影響範囲は RDoc の出力（およびそれを参照する開発者）に限られます。
- これまで:
  - `CollectionProxy#find` のブロック付きの例をそのままコピペしても、コメントに示された出力は再現できませんでしたが、今後はフィクスチャ通りに動かせば同じ結果を再現できます。
  - `sanitize_sql_for_conditions` の例を見て「整数もクォートされる」と誤解する可能性がありましたが、修正により **整数はクォートされない** という実際の仕様が伝わりやすくなります。
- もし既にこの誤ったドキュメントを元にテスト期待値や社内ドキュメントを書いている場合は、**`group_id=4` が正**であることを再確認するとよいです。
...
- 近い API:
  - `sanitize_sql_array`（同様の名前付きバインド／位置指定バインドの例があり、今回の修正と一貫した仕様になっている）

---


## [#57855](https://github.com/rails/rails/pull/57855) Fix incorrect human_size round_mode doc example {#pr-57855}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Numeric#to_fs(:human_size)` の RDoc に記載されていた `round_mode` オプションのサンプルコードが、実際の動作と合っていなかったため修正した PR です。コード本体の挙動に問題はなく、ドキュメント中の例のみを正しいものに差し替えています。

---

2. 変更内容の詳細  

対象: `activesupport/lib/active_support/core_ext/numeric/conversions.rb` の RDoc コメント中のサンプル
...
問題点:
- 1234567 バイト ≒ 1.1774 MB なので、有効桁数2桁でのデフォルト (round_mode: :default) は `"1.2 MB"` になる。
- 実装上 `round_mode: :up` を指定しても、この入力値では結果は `"1.2 MB"` のままで、`"1.3 MB"` にはならない。
- そのため、サンプルは「`round_mode` によって結果が変わる」ことの説明として不適切だった。
...
ポイント:
- 入力値 `1234567` はそのまま維持しつつ、`round_mode: :down` に変更。
- これにより、デフォルト `"1.2 MB"` から `"1.1 MB"` に変わり、`round_mode` による違いが明確に示される。
- 実装は既にこの通り動作しており、PRはコメント(ドキュメント)の1行差し替えのみです。

作者はあわせて、現状の挙動を以下で検証済み:

```ruby
1234567.to_fs(:human_size, precision: 2)                    # => "1.2 MB"
1234567.to_fs(:human_size, precision: 2, round_mode: :down) # => "1.1 MB"
1234567.to_fs(:human_size, precision: 2, round_mode: :up)   # => "1.2 MB"  # default と同じ
```

なお、`number_to_human_size` (ActionView::Helpers::NumberHelper) 側のドキュメント例は、既に `round_mode: :up` で結果が変わる入力 (`123456` など) を用いており、そちらは問題なしとのことです。
...
- 影響範囲:
  - ドキュメント (RDoc) のみ。ライブラリコード、API、挙動には一切変更なし。
  - 既存アプリケーションの動作には影響しません。

- 注意点:
  - これまで「`round_mode: :up` を指定すると `1234567` が `"1.3 MB"` になる」と誤解していた場合は、それがドキュメント上の誤りだったと認識を改める必要があります。
  - `round_mode` の効果を確認したい場合は、今回のドキュメント例のように、切り上げ/切り捨てで実際に値が変わる境界付近の値を使う必要があります。
  - 仕様上、`round_mode: :up` が常に「大きめの値」に寄るとは限らず、「丸めの基準点を越えたときにどちら側に寄せるか」を制御するオプションである点に注意してください（詳細は Ruby / ActiveSupport の rounding オプション仕様を参照）。

---

4. 参考情報 (あれば)  

- 対象メソッド: `Numeric#to_fs(:human_size)` (ActiveSupport のフォーマット拡張)
- 類似機能: `ActionView::Helpers::NumberHelper#number_to_human_size`  
  → こちらのドキュメントには既に `round_mode: :up` の正しい例 (`123456` → `"130 KB"` vs デフォルト `"120 KB"`) が存在します。

---


## [#57865](https://github.com/rails/rails/pull/57865) Fix incorrect RDoc examples in Active Model {#pr-57865}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Active Model の RDoc サンプルコードにおける出力例と実際のコードの不整合を修正したドキュメント専用のPRです。`attribute_method?` と `Errors#each` の例が実際の動作に沿うように、クラス名とオプション値が修正されています。

---

2. 変更内容の詳細  

### 2-1. `ActiveModel::Validations::ClassMethods#attribute_method?`

対象ファイル: `activemodel/lib/active_model/validations.rb`
...
**問題点**  
- 定義しているクラスは `Person` なのに、呼び出し側が `User.attribute_method?` になっており、コピペ由来と思われる不整合がありました。
- これにより、ドキュメントを見てコピペした場合、そのままでは動作しない、もしくは読者が混乱する可能性があります。
...
**技術的な意味合い**  
- `ActiveModel::Validations` を `include` したクラスに対して、属性に応じたバリデーション関連のメソッドが定義される、という一般的な使い方の説明用サンプルです。
- `attribute_method?` は、そのクラスに対して特定の属性メソッドが定義されているかどうかを問い合わせるAPIで、ドキュメント例がそのまま実行可能な形に整えられました。

---

### 2-2. `ActiveModel::Errors#each`

対象ファイル: `activemodel/lib/active_model/errors.rb`
...
**問題点**  
- 実際に `add` しているオプションは `count: 2` なのに、コメントの `options` では `{:count=>3}` と記述されており、サンプルコードと出力例が一致していませんでした。
...
**技術的な意味合い**  
- `ActiveModel::Errors#each` がブロックに渡す `ActiveModel::Error` オブジェクトの中で、`options` に `count` がどのように格納されるかを示すドキュメントです。
- 実際のAPI仕様は変わっておらず、「与えた `count` オプションがそのまま `error.options` に入る」という挙動を正しく反映する形に整えられました。

---

3. 影響範囲・注意点  

- **実装への影響**: コード上の挙動には一切影響ありません。`[ci skip]` が付いている通り、テスト実行も不要なドキュメントのみの変更です。
- **利用者への影響**:
  - RDoc / API ドキュメントを見ながら実装する開発者にとって、サンプルをそのままコピペしても動作しやすくなります。
  - 特に `Errors#each` の例は、`error.options` の中身を正確に理解したいときに役立つため、オプションの取り扱いで混乱しにくくなります。
- **後方互換性**: 完全に後方互換です（動作・シグネチャの変更なし）。
...
- 両方とも Active Model の基本的なAPIのため、Rails のフォームオブジェクトやプレーンオブジェクトのバリデーションを書く際に、ドキュメント例をそのまま使いたい場合に有用です。

---


## [#57864](https://github.com/rails/rails/pull/57864) Add missing `&block` to `merge!` `:call-seq:` in `ActionController::Parameters` {#pr-57864}

**マージ日**: 2026/6/26 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionController::Parameters#merge!` のドキュメント（`:call-seq:`）に、実装上は受け取っているブロック引数 `&block` が記載されていなかった問題を修正した PR です。`Hash#merge!` と同様に、キー衝突時にブロックでマージ方法を指定できることが、ドキュメント上も明示されるようになりました。
...
実際のメソッドシグネチャは `def merge!(*other_hashes, &block)` で、内部で `Hash#merge!` に `&block` をそのまま渡しています。しかし、`:call-seq:` には `&block` が含まれておらず、ユーザーからは「ブロックを取れるのかどうか」がドキュメントだけでは判別しづらい状態でした。

この PR では、`:call-seq:` を以下のように修正しています（実質的な変更は `&block` の追加のみ）:
...
これにより、`Hash#merge!` と同様にブロック付きで利用できることがドキュメントに反映され、同じクラス内の `deep_merge!` のドキュメント:
...
サンプルコード（この PR 以前から動いていたが、明文化された利用例）:

```ruby
params = ActionController::Parameters.new(a: 1, b: 2)

params.merge!(ActionController::Parameters.new(a: 10, c: 3)) do |_key, old, new|
  # 衝突時は大きい方を採用
  [old, new].max
end

# 結果:
# params.to_h #=> { "a" => 10, "b" => 2, "c" => 3 }
```

あるいは単純にハッシュを渡してもよいです:

```ruby
params.merge!(a: 10, c: 3) do |_key, old, new|
  [old, new].max
end
```
...
- 実装の振る舞いは一切変わっておらず、**ドキュメント上の表記のみの変更**です。  
- 既存コードで `Parameters#merge!` にブロックを渡して使用している場合も、その挙動はこれまで通りです（内部で `Hash#merge!` に渡される）。  
- これからコードを書く開発者にとって、`merge!` がブロックを取れることが分かりやすくなり、キー衝突時の挙動をカスタマイズしやすくなります。  
- `merge`（非破壊）ではなく `merge!`（破壊的変更）なので、ドキュメントを見て使い始める人は、レシーバ自身が変更される点に引き続き注意が必要です。

---

4. 参考情報 (あれば)  

- Ruby `Hash#merge!` の仕様（ブロックで衝突時の値を決定できる）  
  https://docs.ruby-lang.org/ja/latest/method/Hash/i/merge.html  
- 同クラス内の類似メソッド `deep_merge!` の `:call-seq:`  
  ```ruby
  # :call-seq:
  #     deep_merge!(other_hash, &block)
  ```
...
## [#57848](https://github.com/rails/rails/pull/57848) Make parameter encodings ractor safe by default {#pr-57848}

**マージ日**: 2026/6/25 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
`ActionController::Base.parameter_encodings` をデフォルトで Ractor セーフにするため、内部で使われるハッシュ構造を常に freeze し、書き換え時は dup→変更→再 freeze する方式に変更した PRです。これに伴い、従来の「default proc でハッシュをその場で書き換える」仕組みは廃止され、空の凍結ハッシュをデフォルト値として返す形に変わりました。
...
`ActionController::Base.parameter_encodings` は以下のような構造を持つクラス属性です:

```ruby
# イメージ:
{
  :index  => { "param_name" => Encoding::UTF_8, ... },
  :create => { "other_param" => Encoding::ISO_8859_1, ... },
}
```

- 第一階層キー: アクション名 (シンボルなど)
- 値: パラメータ名 => Encoding のハッシュ

従来は、このハッシュに default proc を設定し、「初めてアクセスされたキーに対して、その場で内部ハッシュを生成して格納する」ような、**ミュータブル前提の実装**になっていました。

### 今回の主な変更点

1. **`parameter_encodings` のハッシュを常に freeze するようにした**  
   - Ractor セーフにするため、共有されるハッシュを不変（immutable）にします。
   - 新しいエントリを追加するときは、元ハッシュを `dup` して変更し、その結果を再度 `freeze` します。
   - これにより、Ractor 間でのデータ共有時に発生しうるスレッドセーフティやオブジェクト共有制限の問題を回避します。

2. **default proc による「その場での破壊的更新」を廃止**  
   - これまでの実装では、default proc を利用して、存在しないキーにアクセスされたときにハッシュをその場で更新していました。
   - しかし、ハッシュが `freeze` されるようになったため、「デフォルト値生成時に自身を更新する」仕組みは成立しません。
   - そこで、default proc をやめ、**「凍結された空ハッシュ」をデフォルト値として返す**ように変更しています。

   イメージ:

   ```ruby
   # 変更後のイメージ（擬似コード）
   @parameter_encodings ||= {}.freeze

   def parameter_encodings_for(action)
     parameter_encodings[action] || EMPTY_FROZEN_HASH
   end
   ```

3. **設定更新ロジックの変更（dup → append → freeze パターン）**  
   コントローラ側で `parameter_encodings` をカスタマイズするようなメソッド（例えば以下のような DSL）がある場合:

   ```ruby
   class UsersController < ApplicationController
     # 例: あるパラメータを特定のエンコーディングとして扱う設定
     parameter_encoding :create, "name" => Encoding::UTF_8
   end
   ```

   内部実装としては、次のような手順でハッシュを更新する方針になります:

   ```ruby
   # 擬似コード
   existing = self.parameter_encodings   # ここで既に frozen
   updated  = existing.dup               # ミュータブルコピーを作成
   updated[action] = (updated[action] || {}).merge(new_mappings)
   self.parameter_encodings = updated.freeze
   ```

   変更点の主眼は、「共有ハッシュを直接変更しない」「変更後は必ず freeze する」というライフサイクルを徹底することです。

4. **テストの追加・更新**  
   - 新しい不変性・デフォルト値の挙動を確認するためのテストが追加されています。
   - 具体的には「デフォルトで返ってくるハッシュが変更不可であること」「変更操作が新しいハッシュを生み出していること」などを検証していると考えられます。

---

3. 影響範囲・注意点  

1. **`parameter_encodings` を直接ミュータブルに扱っているコードへの影響**  
   アプリケーションやプラグイン/エンジン側で、例えば以下のように**直接ハッシュを変更していた場合**は壊れます:

   ```ruby
   # NG になるパターン（変更後は TypeError: can't modify frozen Hash が発生）
   ActionController::Base.parameter_encodings[:index]["foo"] = Encoding::UTF_8
   ```

   今後は、Rails が提供する公式な DSL や API 経由で設定を行う必要があります。  
   もしどうしても直接操作が必要な場合は、次のように自前で dup→freeze パターンを踏む必要がありますが、推奨されません:

   ```ruby
   enc = ActionController::Base.parameter_encodings.dup
   enc[:index] = (enc[:index] || {}).merge("foo" => Encoding::UTF_8)
   ActionController::Base.parameter_encodings = enc.freeze
   ```

2. **default proc 依存のパターンは使えなくなる**  
   自分で `parameter_encodings.default_proc` を書き換えたり、default proc による遅延生成・破壊的更新を前提にしているようなコードは動かなくなります。

3. **スレッド・Ractor 安全性の向上**  
   - Rails を Ractor と併用しようとしている場合（将来の並列化対応など）において、安全に `parameter_encodings` を共有できるようになります。
   - ただし、Ractor 対応の一部であり、これだけで Rails 全体が完全に Ractor セーフになるわけではありません。

4. **パフォーマンスへの影響**  
   - 設定の更新時に `dup` と `freeze` が入るため、更新頻度が極めて高い場合はわずかなオーバーヘッドがあります。
   - 一般的には `parameter_encodings` は起動時やクラス定義時に一度設定する程度であり、リクエストごとに頻繁に更新するものではないため、実運用上の影響はほぼ無視できるはずです。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57848  
- Ruby Ractor 仕様:  
  - オブジェクトの共有制限（共有できるのは「共有可能オブジェクト（shareable）」のみ）  
  - `freeze` されたハッシュや配列は shareable になりうるため、Ractor 間で共有するためには、「生成 → 設定 → `freeze` → 共有」というパターンが必要になります。  
- 類似の変更は、他の Rails の設定用ハッシュ / 配列（例えば middleware スタックなど）でも順次行われており、「設定オブジェクトは基本 immutable」という方向性に沿った変更です。

---


## [#57850](https://github.com/rails/rails/pull/57850) Make rescue handlers ractor safe by default {#pr-57850}

**マージ日**: 2026/6/25 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1–2文で)  
Rails の `ActiveSupport::Rescuable` が持つ「例外クラスとハンドラの対応表」を、Ractor（マルチスレッド/マルチ Ractor 実行）でも安全に扱えるようにする変更です。データ構造自体を freeze / dup する運用に変えることで、Ractor 間で共有しても壊れない形にしつつ、ハンドラ Proc 自体の Ractor shareable 化は利用者に委ねています。
...
`ActiveSupport::Rescuable` では、以下のように「例外クラス(or キー) と rescue ハンドラ(Proc) のペア」の配列を内部に持っています。

```ruby
# イメージ
@_rescue_handlers = [
  [StandardError, ->(e) { handle_error(e) }],
  [MyError, :handle_my_error],
  ...
]
```

この PR では、**この配列自体を Ractor-safe な運用にする**よう変更しています。

ポイントは説明文に書かれている通りです:

> This is an array contain arrays of key and handler pairs.  
> We freeze the default array and then dup, append and freeze when adding more.

つまり:

- 「デフォルトの rescue handler 配列」を **freeze して不変オブジェクトとして扱う**
- 新しいハンドラを追加するときは
  1. 既存配列を `dup` して新しい配列を作る
  2. そこに `[key, handler]` を `<<` で追加
  3. その新配列を `freeze` する
- これにより:
  - 元の配列は一度 freeze されていて書き換えられない
  - 各 Ractor は「freeze された rescue handler 配列」を安全に共有できる
  - 追記は「新しい配列インスタンスを作って freeze」するだけなので、既存の共有状態を壊さない

変更行数としては `rescuable.rb` が +2/-2 程度なので、ほぼこの配列の扱い（生成・更新）ロジックのみの微修正です。

### ハンドラ Proc について

PR 説明にある通り:

> The handler is proc we `call` so we cannot make it ractor shareable since it may rely on calling methods on `self`. So the user will need to do that.

- rescue ハンドラは `Proc` (あるいはブロック) であり、内部で `self` のメソッド呼び出し等を行うのが一般的
- そのため **Proc インスタンス自体を Ractor.shareable にすることは Rails 側では保証できない**
  - Proc がキャプチャしているコンテキスト次第で shareable かどうかが変わる
- この PR は **「配列構造」を Ractor-safe にするだけ** で、**Proc を shareable にする責任はアプリケーション側** に残されています

### テストの追加

`activesupport/test/rescuable_test.rb` に 14 行追加されており、以下のようなことを検証していると推測されます（実際のコードは省略）:

- rescue handler 配列が期待通り freeze されていること
- ハンドラ追加時に新しい配列が生成され、元配列が変更されないこと
- Ractor 上でも `rescue_from` の定義/参照が問題なく動くこと（あるいは少なくともエラーにならないこと）
...
- 対象: `ActiveSupport::Rescuable` を利用する全コンポーネント
  - `rescue_from` を使う Rails アプリ（主に `ActionController`, `ActiveJob`, `ActiveRecord` など）全般
- 主な変更点:
  - rescue handler の内部表現（配列）のミュータビリティ管理が変わる
  - public API (`rescue_from` の使い方など) に変更はないはず

通常の単一スレッド・単一 Ractor の Rails アプリでは、**ほぼ挙動は変わらない想定**です。  
ただし、以下のような「Rails 内部の rescue handler 配列に直接触る」ようなコードがもしあれば影響します:

- `@_rescue_handlers` を直接参照して `<<` で追記している
- `@_rescue_handlers` を `clear` / `shift` / `pop` などで破壊的に変更している

この PR 後は、`@_rescue_handlers` が freeze された配列になっているため、上記のコードは `FrozenError` を起こします。

### 注意点 / Ractor 利用時のポイント

- Ractor 内で `rescue_from` を定義すること自体は可能ですが、
  - 追加されるハンドラ Proc やオブジェクトが Ractor.shareable でない場合、その Ractor から別 Ractor にそのオブジェクトを渡そうとするとエラーになります
- この PR は **Rails 側の内部状態を「少なくとも壊れない形」に整える**ものであり、
  - アプリケーションの rescue ハンドラを Ractor-safe にするには、引き続き以下を考慮する必要があります:
    - ハンドラで閉じているオブジェクト（`self` の状態、インスタンス変数など）が Ractor 間共有されない前提で設計する
    - Ractor をまたいでハンドラ自体を共有する必要がある場合は、shareable なデータだけを参照するように書く

---

4. 参考情報 (あれば)  

- Ractor と shareable オブジェクトの仕様（Ruby 本体）  
  - `Ractor.shareable?`, `Ractor.make_shareable`, `frozen?` なオブジェクトの扱いなど
- Rails 側では、今後も他の内部データ構造を freeze + copy-on-write 的に扱うことで Ractor 対応を進めていく流れが想定されます。  
  この PR はその一環として、「例外ハンドリング周りの状態管理を Ractor フレンドリーにした」変更と位置づけられます。

---


## [#57851](https://github.com/rails/rails/pull/57851) Make protected ivars ractor safe {#pr-57851}

**マージ日**: 2026/6/25 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
ActionMailer と ActionController に定義されている `PROTECTED_IVARS` 定数を `.freeze` することで、Ractor セーフ（並行実行時にスレッド安全）になるようにした PR です。RuboCop によるチェックから漏れていた未凍結の定数を補正する、極めて小さな内部的変更です。
...
両方で、保護対象インスタンス変数の一覧を表す `PROTECTED_IVARS` 定数が定義されていますが、これを Ractor セーフにするために `.freeze` が追加されました。

イメージとしては以下のような変更です（実際の中身は概念的な例）:
...
ポイント:
- `PROTECTED_IVARS` は配列（または類似の可変オブジェクト）で、  
  Ractor 間で共有される可能性があるクラスレベルの定数。
- Ruby 3 系以降の Ractor モデルでは、Ractor 間で共有するオブジェクトは  
  「イミュータブル（＝凍結済み）」である必要があるため、`.freeze` が必須。
- rubocop の設定上、本来はこうした定数は自動的/半自動的に検出されるべきだったが、  
  この2つの定数はチェックから漏れていたため、今回手動で修正している、という背景です。

---

3. 影響範囲・注意点

- 影響範囲は ActionMailer::Base と ActionController::Base 内部の「保護対象インスタンス変数一覧」を保持する定数のみで、  
  パブリック API の仕様変更はありません。
- `PROTECTED_IVARS` 自体を直接書き換えるようなコード（例: `ActionController::Base::PROTECTED_IVARS << :@foo`）が存在する場合、  
  この変更により `FrozenError` が発生するようになります。
  - もともとフレームワーク内部用の定数であり、アプリケーション側から変更することは推奨されていないため、  
    正常な利用範囲では問題にならないはずです。
- Ractor を使って Rails を並行実行している（あるいは将来的に対応したい）環境では、
  これにより `Ractor::IsolationError` のようなエラーが起きる可能性が一つ減ることになります。
- テストや CHANGELOG の更新は行われていませんが、挙動を変える類の変更ではなく、  
  内部定数の凍結のみなので妥当な範囲と考えられます。

---

4. 参考情報 (あれば)

- Ruby Ractor の仕様（オブジェクト共有と freeze の必要性）:  
  - 公式ドキュメント（英語）: https://docs.ruby-lang.org/en/master/ractor_md.html  
- Rails コアでの Ractor 対応は、3.x 以降で徐々に進められており、  
  今回のような「定数の凍結」「内部オブジェクトのイミュータブル化」はその一環と考えられます。

---


## [#57857](https://github.com/rails/rails/pull/57857) Make ActiveSupport::TimeFormats ractor shareable {#pr-57857}

**マージ日**: 2026/6/25 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
`ActiveSupport::TimeFormats` が保持しているフォーマット一覧 (`@list`) を Ractor で共有可能にするため、内部で使っている lambda を「共有可能 (shareable)」な形に修正した PRです。これにより、Ruby 3 の Ractor を用いた並列実行環境でも `ActiveSupport::TimeWithZone#to_s` などが安全に利用しやすくなります。

---

2. 変更内容の詳細  

※PR本文とファイル名・行数からの推定を含みます。実際のコードは概ね以下のような変更になっていると考えられます。

### 何をしているか

- `ActiveSupport::TimeFormats` は、`Time`/`TimeWithZone` などの `to_s(:format_name)` で使われるフォーマット定義を `@list` というハッシュに持っています。
- その中の一部フォーマットは、`Proc` / `lambda` で定義されており、Ruby 3 の Ractor では「共有可能でないオブジェクト」があると Ractor 間共有に制限がかかります。
- この PR では、その lambda を Ractor で共有可能にするための書き換え・定義方法の変更を行っています。

### なぜ lambda が問題になるのか

Ruby 3 の Ractor では、Ractor 間で共有できるのは「immutable かつ shareable なオブジェクト」に限定されます。  
通常の lambda/Proc は、以下のような理由で shareable でないことがあります。

- 外側のスコープをキャプチャしていて、内部に非 shareable なオブジェクトを持つ  
- 定義方法によっては Ractor の shareable フラグが立たない

この PR では、`ActiveSupport::TimeFormats::DATE_FORMATS` のような定数ないし `@list` に格納される lambda を

- 外部スコープをキャプチャしない「クローズオーバーなし」の形にする
- Ractor 起動前に一度だけ初期化される定数・クラスインスタンス変数にまとめる

などの方法で shareable にしています。

### 変更イメージ（サンプルコード）

例としてイメージすると、次のようなスタイルから:

```ruby
@list[:db] = lambda { |time| time.utc.strftime("%Y-%m-%d %H:%M:%S") }
```

のような「外部状態を取りうる lambda」を、

```ruby
DB_FORMAT = "%Y-%m-%d %H:%M:%S".freeze

@list[:db] = ->(time) { time.utc.strftime(DB_FORMAT) }
```

のように、  
- フォーマット文字列を freeze 済みの定数に切り出し  
- lambda 自体も外部をキャプチャしない & Ractor 起動前にセット  

といった形に変えることで、`ActiveSupport::TimeFormats.@list` が `Ractor.shareable?` をパスするようにしている、というイメージです。

（実際の変更は +4/-4 行なので、よりピンポイントにブロック → lambda、あるいは `Proc.new` → `->` に変えて shareable 判定を通す修正になっている可能性も高いです。）
...
- 主な対象: `ActiveSupport::TimeFormats` を利用する時間/日時の文字列化処理（`Time#to_s(:format)`, `TimeWithZone#to_s(:format)` など）。
- 振る舞いの変更: フォーマット結果自体は変えず、「Ractor で `ActiveSupport::TimeFormats` を共有したときにエラーにならない／より安全に使える」ようにする内部実装変更です。
- 既存フォーマット名 (`:db`, `:number`, `:time`, `:short` 等) やカスタムフォーマットの API には変更が入っていないと考えられます。

### Ractor を使う場合の注意点

- この PR は「標準で定義されているフォーマット (`@list` に最初から入っているもの)」を shareable にするためのものです。
- アプリケーション側で追加するカスタムフォーマットが lambda/Proc を含む場合、それらが非 shareable だと、依然として Ractor からは問題になる可能性があります。

例:

```ruby
# Ractor 的に危ない例（非 shareable かもしれない）
FORMATTER = SomeMutableObject.new

Time::DATE_FORMATS[:foo] = ->(time) { FORMATTER.call(time) }
```

- Ractor を本格的に使う場合は、アプリ側でも:
  - フォーマットの実装が外部のミュータブルオブジェクトをキャプチャしない
  - 文字列・シンボル・数値などの不変オブジェクト（かつ freeze 済）のみを参照する
  といった形にしておく必要があります。
...
- Ruby Ractor ドキュメント（shareable オブジェクトの制約）  
  https://docs.ruby-lang.org/en/master/doc/ractor_md.html
- `ActiveSupport::TimeWithZone` / `ActiveSupport::TimeFormats` の概要  
  - Rails ガイド: Active Support Core Extensions – Date and Time  
    https://guides.rubyonrails.org/active_support_core_extensions.html#extensions-to-date-and-time
- 類似の Ractor 対応 PR（参考になりそうなもの）  
  - 例: ActiveSupport の各種定数・キャッシュを Ractor shareable にする対応が Rails 7.x / 8.0 以降で進んでいます。

---


## [#57858](https://github.com/rails/rails/pull/57858) Only document the public ActionView::Template API [ci skip] {#pr-57858}

**マージ日**: 2026/6/25 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1-2文で)  
このPRは、ActionView::Template のうち「アプリケーションから使うことを想定したメソッドだけ」を公式ドキュメントに載せるよう整理したものです。具体的には `register_template_handler` を ActionView::Template の公開APIとして明示的にドキュメント化し、それ以外のテンプレート関連クラス／メソッドを内部APIとして非公開扱いにしています。

---

2. 変更内容の詳細

### 2-1. `register_template_handler` を正式な公開APIとしてドキュメント化

- これまで:
  - 新しいテンプレートタイプ（例: Haml, Markaby など）を Action View に認識させるための公式な手段は `register_template_handler` でしたが、
  - 実装は `ActionView::Template::Handlers` モジュールにあり、かつそのモジュールには `:nodoc:` が付いていたため、RDoc 上では「非公開っぽく」見えていました。
- 実際の利用例:
  - Haml: `ActionView::Template.register_template_handler(:haml, Haml::Plugin)`
  - Markaby: `ActionView::Template.register_template_handler(:mab, Markaby::TemplateHandler)`

PRの方針:
- `register_template_handler` を **ActionView::Template のクラスメソッドとして** ドキュメント化。
- 実際には `Template` が `Template::Handlers` を `extend` しているため、「最終的な受け皿」として `ActionView::Template.register_template_handler` が正しい呼び出し先であることを、ドキュメントでも明示する。
- ドキュメントの扱いを `local_assigns` と同じ形に揃える（`local_assigns` も Template が extend したメソッドで、Template 自体のシングルトンメソッドとして説明されている）。

Rails アプリ／gem 側での利用イメージ:

```ruby
# config/initializers/custom_template_handler.rb など

module MyApp
  class MyTemplateHandler
    # ActionView が呼び出すインターフェース
    def self.call(template, source = nil)
      # template: ActionView::Template インスタンス
      # source: テンプレートの文字列 (オプション)
      "MyCompiler.compile(#{template.source.inspect})"
    end
  end
end

ActionView::Template.register_template_handler :my, MyApp::MyTemplateHandler
```

このような呼び出し方が「公式にサポートされている」という位置づけになります。

---

### 2-2. `ActionView::Template` のその他のメソッド・クラスを「内部API」として非公開化

PR で明確化された方針:
- `ActionView::Template` のうち、**公開APIとして残すのは次の2つだけ**:
  - `ActionView::Template.register_template_handler`
  - `ActionView::Template#local_assigns`（インスタンス側の公開エントリーポイント）
- それ以外はすべて内部実装として扱い、RDoc に出さない。

具体的な変更:
- `ActionView::Template` 以下で「ハンドラ関連の内部実装」に関する以下を `:nodoc:` または `:stopdoc:` で隠す:
  - レジストリ用ヘルパーメソッド
  - ビルトインのハンドラクラス:
  ...
  - Template 自体のクラスメソッド・インスタンスメソッドの大半
- `actionview/lib/action_view/template/handlers_*.rb` に対して:
  - クラス定義に `:nodoc:` を追加・維持する形で、RDoc の公開API一覧から除外。
- `actionview/lib/action_view/template.rb` に `:stopdoc:` を追加:
  - 公開エントリーポイント（`register_template_handler`, `local_assigns`）以降の部分はドキュメント生成対象から外す。

結果として、APIドキュメント上では ActionView::Template のページに表示されるのは原則この2つのみになります。

---

### 2-3. `class << self` につけていた `:nodoc:` を削除

- 以前は `ActionView::Template` 内の `class << self` に `:nodoc:` を付けていたが、RDoc は **シングルトンクラスに付いた `:nodoc:` を無視する** 仕様。
- そのため、以下のようなクラスメソッドが誤って「公開API」としてドキュメントに出てしまっていた:
  - `mime_types_implementation=`
  - `validate_formats`
  - `normalized_formats`
- 今回:
  - この `:nodoc:` は実質的に意味がなかったため削除。
  - 代わりに `:stopdoc:` などの適切な手段で Template の内部メソッドが RDoc に出ないように制御。
- 意図としては「**これらのメソッドは引き続き存在するが、アプリケーションから触ることを前提にしていない内部API**」であることを明確にした、という整理です。
...
- 変更は主に **ドキュメント生成と可視性（`nodoc` 系の指示）** に関するものです。
- メソッドのシグネチャや挙動を変更しているわけではないため、既存アプリケーション・gem の実行時動作には基本的に影響しません。

### 3-2. 「Rails 公開APIかどうか」の判断基準がより明確になる

- APIドキュメント上から消えたメソッドやクラスは「内部実装」であり、将来の非互換変更の対象になりやすいと解釈すべきです。
- 特に以下を直接使っている場合は注意:
  - `ActionView::Template::Handlers::ERB` などのビルトインハンドラを直接参照しているコード
  - `ActionView::Template` のクラスメソッド・インスタンスメソッドで、公式ガイドやAPIドキュメントに明示されていないものを呼んでいるコード
- 今後は、テンプレートハンドラを拡張したい場合は **`ActionView::Template.register_template_handler` 一択**であることがより明確になります。

### 3-3. gem 作者向けの注意点

- 既に多くのテンプレートエンジン系 gem（Haml, Markaby など）は
  - `ActionView::Template.register_template_handler` を使っているため、このPRにより **「やっていることが正式に推奨される形になった」** という位置づけです。
- もし独自に Template や Handlers の内部メソッドに依存している場合は、将来の Rails アップグレード時に壊れるリスクが高まるため、公開APIへの移行を検討した方が安全です。
...
- PR 本体:  
  https://github.com/rails/rails/pull/57858
- `register_template_handler` の利用例:
  - Haml:  
    https://github.com/haml/haml/blob/v7.2.0/lib/haml/rails_template.rb#L55
  - Markaby:  
    https://github.com/markaby/markaby/blob/v0.9.4/lib/markaby/rails.rb#L9
- 関連する公開API（`local_assigns`）のドキュメントは `ActionView::Template` の RDoc を参照すると、今回の整理後の形が分かります。

---


## [#57784](https://github.com/rails/rails/pull/57784) Fix inverse matching for association composite primary keys {#pr-57784}

**マージ日**: 2026/6/25 | **作成者**: [@Korri](https://github.com/Korri)

1. 概要 (1-2文で)  
Rails の関連付けで「関連自体は複合主キーを使うが、所有側モデルの `id` は単一キー」というケースで `inverse_of` の自動設定が壊れていた問題を修正した PR です。`Association#matches_foreign_key?` の比較ロジックを修正し、子オブジェクトの複合外部キーと親オブジェクト側の値が正しくマッチするようにしています。
...
- 過去の PR [#56664](https://github.com/rails/rails/pull/56664) で「配列 vs 配列」の inverse マッチング（主に複合主キー関連の比較）が修正された。
- しかし次のようなケースが未対応だった:
  - 関連の `primary_key` は「複合キー（配列）」を宣言している
  - しかし所有クラス（親モデル）は通常どおり単一の `id` を持っている
- この場合、`Association#matches_foreign_key?` が親側オブジェクトの値を比較する際に、関連が指定している複合 `primary_key` ではなく、クラスのデフォルト `primary_key` を使ってしまい、inverse マッチングに失敗していた。

イメージとしては次のような比較が期待されるのに、実際にはずれていた:

- 子の外部キー: `[shop_id, order_id]`
- 親側で比較すべき値: `[shop_id, id]` ← 関連が「`[ :shop_id, :id ]`」のような複合 primary_key を宣言しているパターン

この「配列同士」の比較で、親側が関連の `active_record_primary_key` を見ずにクラスの `primary_key` を見ていたためミスマッチが起き、`inverse_of` を正しくセットできない、というリグレッションになっていました。

### コード上の修正ポイント

#### `Association#matches_foreign_key?` の修正

`activerecord/lib/active_record/associations/association.rb` で、関連の逆側（owner）を比較するときに使うキーを、クラスの `primary_key` ではなく「関連に紐づく `active_record_primary_key`」から取得するよう変更しています。

擬似コードで言うと、だいたい以下のようなイメージの変更です（実際のコードとは多少異なる可能性がありますが、意図を示すための例）:

```ruby
# 修正前（イメージ）
def matches_foreign_key?(record)
  owner_key = Array(klass.primary_key).map { |key| owner.public_send(key) }
  foreign_key = Array(reflection.foreign_key).map { |key| record.public_send(key) }
  owner_key == foreign_key
end

# 修正後（イメージ）
def matches_foreign_key?(record)
  # 関連が宣言している primary key を使う
  owner_keys = Array(reflection.active_record_primary_key)

  owner_values   = owner_keys.map  { |key| owner.public_send(key) }
  foreign_keys   = Array(reflection.foreign_key)
  foreign_values = foreign_keys.map { |key| record.public_send(key) }

  owner_values == foreign_values
end
```

ポイントは:

- 「どのカラムを親の比較キーとするか」を `reflection.active_record_primary_key` に委ねるようにしたこと
- これにより、クラス自体は単一 `id` でも、特定の関連だけ「`[:shop_id, :id]`」のような複合 primary_key を宣言でき、その形で比較が行われる

#### テストの追加

`activerecord/test/cases/associations_test.rb` に以下のようなリグレッションテストが追加されています:

- `test_belongs_to_a_model_with_composite_association_primary_key_sets_inverse_of`
  - belongs_to 先のモデルが「複合 association primary key」を持つ場合に、`inverse_of` が正しく設定されることを確認するテスト
  - 具体的には:
    - 子モデル側の外部キーは `[shop_id, order_id]`
    - 親モデル側は `[shop_id, id]`
    - 上記が一致すると判断されて inverse 関連がセットされることを検証

また、`activerecord/test/models/cpk/book.rb` と `activerecord/test/models/cpk/order.rb` に複合主キー関連の振る舞いを確認するためのモデル定義が追記されています。`Book` や `Order` モデルが、CPK (Composite Primary Key) シナリオのために拡張されており、今回の失敗パターンを再現するために利用されています。

#### CHANGELOG

`activerecord/CHANGELOG.md` に今回の修正内容が追加され、複合主キー関連の inverse matching に関する不具合が修正されたことが明示されています。

---

3. 影響範囲・注意点  

影響を受けるのは、次のようなアプリケーションです:

- Active Record の関連で、**association 側で複合 `primary_key` を宣言している**
- しかし、関連先のモデルクラス自体の `primary_key` は単一カラム (`id`) のまま
- その上で `inverse_of` による逆関連の自動設定（キャッシュ）に依存している

この PR によって:

- 上記のような構成でも `inverse_of` が意図どおり効き、親子オブジェクト間でのキャッシュ共有が正しく行われるようになります。
- 逆に言うと、これまで「なぜか inverse が張られない／N+1 的な振る舞いをする／同じレコードが別インスタンスとして存在する」といった怪しい挙動があった場合、今回の修正で自然に解消される可能性があります。

注意点:

- 仕様変更というよりはバグ修正であり、「ドキュメントどおりに設定した複合 primary_key + inverse_of の挙動がやっと正しくなった」という位置づけです。
- もし既にこのバグを前提にワークアラウンドを入れている場合（例: 手動で inverse 設定、比較ロジックの上書きなど）、Rails 更新後に挙動が二重に効いていないかを確認してください。
- テストコードで「inverse が nil であること」を前提としていたようなケースがあれば、そこが失敗する可能性があります。

---

4. 参考情報 (あれば)  

- 元の配列 vs 配列 inverse matching 修正 PR:  
  https://github.com/rails/rails/pull/56664
- 当該メソッド (`Association#matches_foreign_key?`) の実装位置:  
...
この辺りを追うと、「関連ごとに独自の primary_key（複合含む）を宣言し、それに従って inverse マッチングを行う」という現在の設計意図が読み取れます。

---


## [#57859](https://github.com/rails/rails/pull/57859) Better shareable_lambda usage {#pr-57859}

**マージ日**: 2026/6/25 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
Rails内部で使われている `shareable_lambda` の使い方を、よりシンプルで読みやすい形にリファクタリングしたPRです。挙動や外部APIは変えずに、定数として定義されているラムダの書き方を整理しただけの変更です。

---

2. 変更内容の詳細  

※PR本文と変更統計から読み取れる範囲での解説です（実際の差分は概ね以下のようなイメージの変更です）。

### 2-1. 背景: `shareable_lambda` とは

`shareable_lambda` は Rails 内部で使われるヘルパーで、**スレッドセーフかつ再利用可能なラムダオブジェクト**を簡潔に定義するためのものです。  
たとえば、コールバックやミドルウェア、各種ディスパッチ処理で「毎回新しい `lambda` を作るのではなく、定数として共有したい」ような場面で使われます。

従来、以下のように「わざとらしくネストしたり、回りくどい書き方」になっていた箇所があった、というのが作者の自己コメントです。

```ruby
SOME_CONST = shareable_lambda do |*args|
  ->(env) { ... }
end
```

あるいは、無名モジュールや Proc を経由してから `shareable_lambda` を呼ぶような、「何をしているか一目で分かりづらい」構造になっていた部分があったと考えられます。

今回のPRでは、それらをよりストレートな形に書き直しています。イメージとしては以下のような変化です:

#### Before（あくまで例）

```ruby
class ActionController::Metal
  SOME_HANDLER = shareable_lambda do |controller, request|
    # 何か処理
  end
end
```

に対して、更に回りくどいラップがあったり、定数の中で別の lambda を返していたりしたのを、
...
というように、「一目で引数と処理内容が分かる」形に整えています。

### 2-2. どのファイルが対象か

以下の6ファイルで、定数として定義されているラムダの書き方が整理されています（+14/-14 なので、ほぼ1行単位の書き換えのみ）:

- `actionpack/lib/action_controller/metal.rb`  
  - コントローラの基底クラス周辺で使われるラムダ定数（例えば、リクエスト処理やレスポンス生成のための共有ラムダ）に対して、`shareable_lambda` の使い方を簡潔化。
- `actionpack/lib/action_dispatch/http/parameters.rb`  
  - パラメータのパース・フィルタリングなどに使われるラムダ（パラメータ処理用の共通関数）が、より素直な `shareable_lambda` 定義に。
- `actionpack/lib/action_dispatch/journey/visitors.rb`  
  - ルーティングツリー（Journey AST）を辿る visitor 用ラムダの定義が読みやすく整理。
- `actionpack/lib/action_dispatch/routing/mapper.rb`  
  - ルーティング定義（`resources` など）をマッピングする際に使う共有ラムダがリファクタリング。
- `actionpack/lib/action_dispatch/routing/route_set.rb`  
  - ルートセットが内部的に使用するディスパッチ用ラムダなどが、より直接的な `shareable_lambda` 使用に変更。
- `activesupport/lib/active_support/log_subscriber.rb`  
  - `ActiveSupport::LogSubscriber` がログ出力時に使用する共有ラムダの書き方が整理。  
    たとえば、イベントを受け取ってログフォーマットする処理などが `shareable_lambda` で共有されていますが、その宣言方法が素直な形に。

コードレベルでは、

- 不要なネスト（`-> { shareable_lambda { ... } }` のようなもの）
- 意味のない中間変数・中間オブジェクト
- ブロックのスコープが分かりづらい書き方

などを取り除き、**「この定数はこのラムダを指している」ことが一目でわかる**ように整えた、という性質の変更です。
...
- **挙動は変わらない想定**  
  - 変更はリファクタリング（スタイルの改善）であり、ロジック自体は変えていません。
  - テスト追加はされていませんが、既存のテストがグリーンであることが前提の軽量変更です。
- **外部APIへの影響なし**  
  - 変更対象はすべて内部定数・内部実装（ActionPack / ActiveSupport の内部で使用されるラムダ）であり、パブリックAPIのインターフェイスは変わっていません。
- **パフォーマンスへの影響はほぼなし（あるとしても僅少な改善）**  
  - `shareable_lambda` の用途に沿った、より素直な使い方になったことで、不要なオブジェクト生成が減っている可能性はありますが、差分規模的に体感できるほどではないと考えられます。
- **Rails内部コードを読む人にとっては可読性向上**  
  - ルーティングやログ周りなどの内部実装を追う際に、「この定数がどんなラムダなのか」が以前より分かりやすくなっています。
  - gemやアプリでRailsの内部定数を直接参照・モンキーパッチしているようなかなり特殊なケースを除けば、互換性問題はほぼ起こりません。

---

4. 参考情報 (あれば)  

- PR #57859: *Better shareable_lambda usage*  
- 関連するRails内部機構
  - `ActiveSupport::LogSubscriber`  
  - `ActionDispatch::Routing`（`Journey`, `RouteSet`, `Mapper`）  
  - `ActionController::Metal`  
- `shareable_lambda` 自体の使い方の参考としては、Railsコードベース全体での定数定義パターンを見るとよいです（例: `Rails.application.routes` 周辺や、ログサブスクライバの実装など）。

---


## [#57853](https://github.com/rails/rails/pull/57853) Fix incorrect number_to_rounded doc example [ci skip] {#pr-57853}

**マージ日**: 2026/6/25 | **作成者**: [@antonzaharia](https://github.com/antonzaharia)

1. 概要 (1-2文で)  
`ActiveSupport::NumberHelper#number_to_rounded` の RDoc に記載されていた `precision: 0` の出力例が、実装と異なり「切り捨て」のように見えていた問題を修正し、「四捨五入」の挙動に合わせた正しい例に更新した PR です。コードの挙動自体は従来どおりで、ドキュメントのみの修正です。
...
対象:  
`activesupport/lib/active_support/number_helper.rb` 内の `number_to_rounded` の RDoc コメント（ドキュメント例）
...
と記載されていましたが、実際のメソッドの挙動は「四捨五入」であり、返り値は `"12346"` になります。  
そのため、以下のように修正されています:
...
ActiveSupport::NumberHelper.number_to_rounded(12345.4,    precision: 0) # => "12345"  # 0.4 → 切り捨てではなく四捨五入の結果
ActiveSupport::NumberHelper.number_to_rounded(12345.6,    precision: 0) # => "12346"
```

ポイント:
- `precision: 0` の場合も挙動は「小数第1位で四捨五入」であり、整数化・切り捨てではない。
- 既に他の例（`12345.679`, `12345.68`, `12345.00000`）は正しく四捨五入の結果になっており、この 1 行だけが誤っていた。

---

3. 影響範囲・注意点  

- 実装コードは一切変更されておらず、挙動は既存と同じです。
- 影響があるのは **ドキュメントを読んで実装方針を決める開発者の理解** だけです。
  - 特に「`precision: 0` は小数点以下を切り捨てて整数にしてくれる」と誤解していた場合は注意が必要です。
  - 実際には「小数第1位を四捨五入して整数にする」挙動なので、`12345.6789` → `12346` になります。
- 小数点以下を「常に切り捨てたい」場合は、`number_to_rounded` ではなく以下のような別手段を検討すべきです（例）:
  - Ruby の標準メソッド：`number.floor` / `number.truncate`
  - あるいはビジネス要件に応じた前処理をしてから `number_to_rounded` に渡す

---

4. 参考情報 (あれば)  

- 該当メソッド: `ActiveSupport::NumberHelper#number_to_rounded`  
  - 主にビューやヘルパーで数値フォーマットを行うためのヘルパー。
- 丸めモードを詳細に制御したい場合には、`NumberHelper` のオプションや `BigDecimal`、`round` の `half:` オプションなどと組み合わせて挙動を確認するとよいです。

---


## [#57602](https://github.com/rails/rails/pull/57602) Make Active Record Railtie Ractor safe {#pr-57602}

**マージ日**: 2026/6/25 | **作成者**: [@gmcgibbon](https://github.com/gmcgibbon)

1. 概要 (1-2文で)  
Active Record の Railtie 内で使われている message verifier 用の Proc を「Ractor 共有可能（Ractor-shareable）」な形に変更し、Ractor 環境でも安全に動作するようにした PR です。Rails の初期化処理と Ractor の併用時に、Active Record が Ractor の制約に抵触しないようにするのが目的です。

---

2. 変更内容の詳細(あればサンプルコードも含めて)  

※PR 本体の diff は 3 行追加 / 3 行削除のみで、`activerecord/lib/active_record/railtie.rb` 内の「message verifier を生成する Proc」の定義が Ractor で共有可能な形に書き換えられています。

### 背景となる問題

Ruby の Ractor では以下のような制約があります：

- Ractor 間でオブジェクトをやり取りするには「shareable」である必要がある  
  - もしくはコピーして渡す（が、Rails の設定オブジェクトなどはコピーでは困る場合がある）
- Proc / lambda / block などは、環境（クロージャ）に「非 shareable なオブジェクト」が閉じ込められていると shareable にならない

Rails の Railtie は、アプリケーション起動時に設定を行う部分で、Active Record では「メッセージ検証（MessageVerifier）」関連の仕組みを Proc で定義している箇所があります。  
この Proc が Config や logger などの非 shareable なオブジェクトをキャプチャしていると、Ractor からその Proc を参照した時点でエラーになる可能性があります。

### この PR の具体的な変更

この PR では、その「message verifier を返す Proc」について：

- Proc の中でキャプチャしているオブジェクトを見直し
- Ractor.shareable? が true になるような形で定義し直し

という変更が行われています。

擬似コードでイメージを書くと、従来は例えば：

```ruby
# 例: Ractor 非対応になりがちなパターン
config = app.config

ActiveRecord::Railtie.config.active_record.verifier_generator = proc do |name|
  ActiveSupport::MessageVerifier.new(config.some_secret_for(name))
end
```

のように `config` をクロージャ内でキャプチャしており、`config` が shareable でないために Proc 自体も非 shareable になる、という状態だったと考えられます。

これを、例えば次のように「Proc 内でグローバルな設定・クラスメソッド等を参照するだけ」にする、あるいは「Proc を定数として freeze して共有可能にする」といった形に修正していると推測されます：

```ruby
# 例: Ractor 共有可能にしやすい書き方のイメージ
ActiveRecord::Railtie.config.active_record.verifier_generator =
  ->(name) do
    ActiveSupport::MessageVerifier.new(Rails.application.credentials.dig(:active_record, name))
  end
# もしくは config を事前に凍結して shareable にする など
```

実際のコードは Rails 本体に依存しますが、「message verifier を生成する Proc で、Ractor が問題にするような閉じ方をしないよう修正した」という点が本質です。
...
- Active Record の Railtie（`ActiveRecord::Railtie`）が提供する「message verifier 関連の設定」部分のみが対象
- ActiveRecord の通常の利用（モデル定義、クエリ、migration など）にはほぼ影響なし
- Ractor を利用せず、マルチスレッドのみで動いているアプリケーションは基本的に挙動変化はありません

この変更により：

- Rails アプリケーションを Ractor 内で起動する / Ractor 内から ActiveRecord の Railtie 初期化に触れるようなケースで、Ractor の shareable 制約に起因するエラーが発生しにくくなります。
- Ractor を使った並列処理環境で Active Record を利用する場合の安全性が向上します。

### 注意点

- PR のチェックリストでは「テストの追加・更新」「CHANGELOG 更新」は未チェックのままマージされています  
  - 挙動変更はごく小さく、既存テストでカバーされている／後方互換と判断されている可能性が高いですが、Ractor を積極的に使う場合は Ruby のバージョンと Rails 本体の組み合わせで挙動を確認することを推奨します。
- もしアプリ側で `ActiveRecord::Railtie` の message verifier 周りの設定を独自に上書きしている場合：  
  - 自作の Proc / lambda が非 shareable なオブジェクト（例: コネクションプールインスタンス、ミューテックス、非凍結の設定オブジェクト）をクロージャに閉じ込めていると、依然として Ractor で問題になる可能性があります。
  - Ractor を使う予定がある場合は、自前設定も `Ractor.shareable?` が true になるように設計する必要があります。

---

4. 参考情報 (あれば)

- Ruby Ractor の shareable 仕様  
  - https://docs.ruby-lang.org/en/master/doc/ractor_md.html  
- Rails の Railtie について  
  - https://guides.rubyonrails.org/engines.html#railties  
- ActiveSupport::MessageVerifier の概要  
  - https://api.rubyonrails.org/classes/ActiveSupport/MessageVerifier.html  

この PR 自体は極小変更ですが、「Rails コアが Ractor 対応を少しずつ進めている」流れの一環であり、将来的に Ractor + Rails を組み合わせたい場合には重要な基盤整備といえます。

---


## [#57849](https://github.com/rails/rails/pull/57849) Bump cgi to 0.5.2 {#pr-57849}

**マージ日**: 2026/6/24 | **作成者**: [@yahonda](https://github.com/yahonda)

1. 概要 (1-2文で)  
Rails の CI（特に guides ジョブ）で `cgi` のバージョン不一致により落ちていたため、`Gemfile.lock` 内の `cgi` を 0.5.2 に上げて CI を安定化させる PR です。コード挙動を変える修正ではなく、CI 環境と rubygems.org 上の最新版を揃えるためのメンテナンス的変更です。
...
変更は `Gemfile.lock` の 1 行のみで、`cgi` のバージョンを更新しています。
...
※PR 説明中では「0.5.0 → 0.5.2」とありますが、CI のログメッセージには「You have already activated cgi 0.5.1」と出ているため、CI イメージ上の実際のバージョンは 0.5.1 で動いていた可能性があります。いずれにせよ、`Gemfile.lock` を 0.5.2 に揃えることで、CI 上の事前バンドル済みバージョンと rubygems.org 上の最新版を一致させる狙いです。

### なぜ guides ジョブだけが落ちたのか

Rails の `guides` CI ジョブでは、`guides/bug_report_templates/*.rb` が以下のような形で実行されています。

- 各テンプレートは `Bundler.unbundled_system` で実行される  
- さらにテンプレート内で `bundler/inline` の `gemfile(true)` を使っており、**プロジェクトの `Gemfile.lock` とは独立して rubygems.org に対して最新版を解決**する

そのため:

1. CI イメージには、すでに何らかのバージョンの `cgi`（例: 0.5.1）がインストール済み・アクティベート済み
2. `bug_report_templates/action_controller.rb` が最初に実行されるとき、`gemfile(true)` により rubygems.org の最新版 `cgi` 0.5.2 を要求
3. すると Bundler が「もう `cgi 0.5.1` がアクティベートされているのに、Gemfile では `0.5.2` が必要」と判断して、次のエラーで落ちる:

   ```text
   You have already activated cgi 0.5.1, but your Gemfile requires cgi 0.5.2.
   Prepending `bundle exec` to your command may solve this. (Gem::LoadError)
   ```

4. その後に実行される他のテンプレートは、すでにインストール済みの `cgi 0.5.2` を再利用して通るため、**最初のテンプレートだけがコケてジョブ全体が失敗**する状態になる

### この PR の狙い

`Gemfile.lock` 側を `cgi` 0.5.2 に bump することで:

- CI ベースイメージを再構築するときに **最初から `cgi 0.5.2` が `GEM_HOME` に入った状態** になる
- その結果、`guides/bug_report_templates/action_controller.rb` が最初に走る時点で、すでに `cgi 0.5.2` がアクティベートされているので、途中で `0.5.2` を新規インストールしようとしてバージョン競合を起こすことがなくなる

このように、**「CI イメージ内のプリインストール版」と「inline Gemfile が要求する版」を一致させて、Gem::LoadError を回避する**のがこの PR の主目的です。

---

3. 影響範囲・注意点  

- 実際の Rails のアプリケーションコードには一切変更がなく、`Gemfile.lock` の `cgi` バージョンのみが上がるため、**Rails 自体の API や挙動に直接の変更はありません**。
- 影響範囲としては主に以下です:
  - Rails 本体の CI（特に guides ジョブ）が安定して通るようになる
  - Rails を開発している環境で `bundle install` した場合、開発者のローカルにも `cgi` 0.5.2 がインストールされる
- `cgi` 自体は Ruby 標準の default gem 系列であり、今回のような「default gem の新バージョンが rubygems.org で公開され、それを inline Gemfile が取ってくる一方で、CI イメージ内には古いバージョンがある」というパターンで **定期的に同様の問題が発生** していることが説明されています。
- PR の説明にもある通り、これは以下のような既存対応の延長線上にある「1行 Gemfile.lock bump」で、**既存の運用ルールに沿った標準的な対処**です:
  - stringio 3.1.1 への bump
  - strscan 3.0.4 への bump
  - psych 5.1.2 / 5.1.1.1 への bump
  - timeout 0.4.1 への bump

注意点としては、**根本原因は rubygems / Bundler 側の default gem の扱い**にあるため、将来的には rubygems 側の修正でこの種の問題をなくす方向が想定されています（下記参照）。

---

4. 参考情報 (あれば)  

- PR 本文で言及されている、根本対策となりうる rubygems 側の Issue:
  - https://github.com/rubygems/rubygems/issues/5529
  - https://github.com/rubygems/rubygems/issues/5535
- `Bundler.unbundled_system` と `bundler/inline` (`gemfile(true)`) の挙動:
  - `unbundled_system`: 現在の Bundler コンテキストを外して別プロセスを実行し、プロジェクトの Gemfile とは独立した解決を行わせる
  - `bundler/inline` の `gemfile(true)`: スクリプト内から Gemfile を宣言し、その場で bundle install + 実行を行う。`true` にするとスクリプト終了後に Gemfile を破棄する一時的な利用モード  
- 実務的には、Rails 本体に対して:
  - この PR を取り込んだ後に CI ベースイメージを再ビルドする（あるいは CI 側で `bundle install` をやり直す）ことでエラー解消が見込めます。

---


## [#57847](https://github.com/rails/rails/pull/57847) Move Action Cable configuration to top level {#pr-57847}

**マージ日**: 2026/6/24 | **作成者**: [@samuel-williams-shopify](https://github.com/samuel-williams-shopify)

1. 概要 (1-2文で)  
Action Cable の設定クラスを `ActionCable::Server::Configuration` からトップレベルの `ActionCable::Configuration` に抽出し、旧クラスは互換用エイリアスとして残すリファクタリングです。既存コードを壊さずに、今後のサーバー入れ替えフックなどの拡張をしやすくするための下準備となる変更です。
...
- 新規: `ActionCable::Configuration`
  - これまで `ActionCable::Server::Configuration` に実装されていた設定ロジックを、そのままトップレベルの `ActionCable::Configuration` に移動。
  - Action Cable の各種設定（例: `url`, `logger`, `connection_class`, `channels_path` など）が、この新クラスに集約される形になります（実装自体はほぼそのまま移動）。
...
- 互換性用エイリアス:
  ...
  ```
  のように、`ActionCable::Server::Configuration` は `ActionCable::Configuration` への単なる定数エイリアスに変更されました。
  - これにより、既存コードで `ActionCable::Server::Configuration` を参照していても動作は変わりません。

- テストの追加:
  - `actioncable/test/server/base_test.rb` に、以下を満たすテストが追加されています:
    - `ActionCable::Server::Configuration` が `ActionCable::Configuration` のエイリアスになっていること。
    - デフォルトのサーバー設定オブジェクトが正しく生成されること。

- CHANGELOG 更新:
  - Action Cable の CHANGELOG に、この設定クラスのトップレベルへの移動が記載されました。
...
も、内部的には新しい `ActionCable::Configuration` を指すため、そのまま動作します。
...
- 互換性:
  - `ActionCable::Server::Configuration` はエイリアスとして残っているため、**既存アプリケーションは基本的に影響を受けません**。
  - ただし、`ActionCable::Server::Configuration` のクラス名文字列や定義場所に依存したメタプログラミングをしている場合（`name`, `to_s`, `const_source_location` 等）、挙動がわずかに変わる可能性があります。

- 推奨される移行:
  - 新規コードやリファクタリング時には、`ActionCable::Server::Configuration` ではなく **`ActionCable::Configuration` を直接使う** のが今後の標準となります。
  - gem やエンジンなどで Action Cable を拡張している場合は、サポートバージョンを見ながら、将来的に参照先をトップレベルの `ActionCable::Configuration` に切り替えることを検討するとよいです。

- 今後の変更の布石:
  - この PR は、別 PR (#57803) で提案されている「Action Cable サーバー差し替えフック」のための事前分割です。
  - 設定オブジェクトをサーバー実装 (`ActionCable::Server`) から切り離すことで、異なるサーバー実装（例: 独自サーバー / 別プロセス / 別アダプタ）と組み合わせやすくなります。

---

4. 参考情報 (あれば)  

- 対象 PR: https://github.com/rails/rails/pull/57847  
- 関連 PR（サーバー置き換えフック）: https://github.com/rails/rails/pull/57803  
- 実行された検証コマンド:
  ```bash
...
## [#57844](https://github.com/rails/rails/pull/57844) Remove the use of `$,` as a default value for `safe_join` {#pr-57844}

**マージ日**: 2026/6/24 | **作成者**: [@Edouard-chin](https://github.com/Edouard-chin)

1. 概要 (1-2文で)  
Rails の `ActionView::Helpers::OutputSafetyHelper#safe_join` が、暗黙にグローバル変数 `$,` を区切り文字のデフォルトとして使う挙動をやめ、常に `nil`（＝区切りなし）をデフォルトとするように変更されました。これにより、Ractor セーフティ向上と、Ruby 本体で非推奨な `$,` への依存解消が行われています。
...
- `safe_join` のメソッド定義のデフォルト引数が変更されました。
...
- `actionview` の CHANGELOG にこの挙動変更が追記されています。
- `safe_join` が `$,` を使うことを前提としたテストが削除されています（`output_safety_helper_test.rb` で該当テストが -18 行）。

### 動作イメージ

変更後の `safe_join` の基本的な使い方は従来どおりです:

```ruby
# 明示的に区切り文字を渡すパターン（今後も推奨）
safe_join(["a", "b", "c"], ", ")
# => "a, b, c"

# デフォルト（sep を渡さない）時は区切り無し
safe_join(["a", "b", "c"])
# 変更前: $,.nil? でなければ $, に依存
# 変更後: 常に "abc"
```

Ruby のグローバル変数 `$,`（`$OUTPUT_FIELD_SEPARATOR`）を設定していた場合、以前はそれが `safe_join` のデフォルト区切りとして使われ得ましたが、今後は一切利用されず、常に `nil`（区切りなし）となります。
...
- アプリケーション側で `$,` を設定しつつ、`safe_join` を **第2引数無しで呼び出し**、かつ「`$,` がデフォルト区切りとして使われる」ことを暗黙に期待していた場合にのみ影響します。
...
- ただし PR の説明によると:
  - Rails の公式ドキュメントでは「`safe_join` が `$,` を参照する」ことは一切説明されていない。
  - Ruby では `$,` の設定自体が 10 年近く非推奨になっている。
  - GitHub 全体で `$, =` を使っている Ruby コードは非常に少ない。
  
  ため、この挙動に依存しているアプリは非常にレアであることが見込まれています。

### 移行時の注意点・対応策

- `safe_join` に暗黙の区切りを期待しているかもしれないコードを確認する場合:
  - プロジェクト内で `$, =` を検索し、セットしている場所がないか確認する。
  - もし `$,` を設定していたら、その周辺で `safe_join(array)` のように第2引数なしで使っている箇所がないか確認する。
- もし挙動変更の影響を受けている場合は、以下のように **常に第2引数で区切り文字を明示**してください:
...
- Ractor を使う/検討している環境では、この変更により `safe_join` が Ractor セーフになる方向に寄与します（グローバル可変状態への依存が消える）。
...
- PR 本文で言及されている背景:
  - `$,` は Ractor セーフではないため、並行実行モデルとの相性が悪い。
  - Rails は `safe_join` のデフォルト区切りが `$,` であることをドキュメント化しておらず、「ドキュメント外仕様を前提としたコード」に対しては非互換変更とみなさない判断。
- Ruby 側の状況:
  - `$,`（`$OUTPUT_FIELD_SEPARATOR`）の設定は Ruby 本体で長期的に非推奨（deprecated）扱い。
  - 今後の Ruby/Rails の流れとしても、こうしたグローバル変数依存を排除していく方向性と整合的な変更です。

---


## [#57829](https://github.com/rails/rails/pull/57829) Refactor Arel::Table to use keyword name: argument {#pr-57829}

**マージ日**: 2026/6/24 | **作成者**: [@gmcgibbon](https://github.com/gmcgibbon)

1. 概要 (1-2文で)  
`Arel::Table` のコンストラクタ引数が「位置引数＋オプション」で name を渡す形から、「キーワード引数 name:」で渡す形にリファクタリングされた PR です。これに伴い、`Arel::Table` を直接生成している内部コード・テストコードがすべて新しい呼び出し方に書き換えられています。
...
- 直前の PR #57642 で `Arel::Table` に「任意の追加引数」が導入されたが、その追加引数は位置引数として定義されていた。
- その結果、クラス (`klass`) を指定するために「name, klass, …」のようなシグネチャになり、「クラスを指定しない場合でも `nil` を位置引数として渡す」ような呼び方が発生していた。
- `Arel::Table` は `#:nodoc:` で「Rails 内部の非公開 API」として扱われているため、内部仕様を破壊的に整理しやすい。

この PR では、その「オプションの引数」をキーワード引数に切り替えることで、わかりやすく安全なインターフェイスに整理しています。

### `Arel::Table` のコンストラクタ変更

以前は大まかに次のような形でした（イメージ）:

```ruby
# 旧: 位置引数で name, klass を取る
Arel::Table.new("users")                # klass 省略
Arel::Table.new("users", User)         # klass 指定
Arel::Table.new("users", nil, type_caster) # name だけ指定したいが nil を渡す必要があるケース
```

この PR により、`name` がキーワード引数になり、「name」を必ず明示的に指定する形にリファクタリングされています:

```ruby
# 新: name をキーワードで指定
Arel::Table.new(name: "users")
Arel::Table.new(name: "users", klass: User)
Arel::Table.new(name: "users", type_caster: type_caster)
```

ポイント:

- 「`nil` をダミーで渡して位置を合わせる」ようなコードが不要になる。
- `name:` というキーワードを通じて、引数の意味が明示的になる。
- `Arel::Table` 自体は nodoc（内部 API）なので、こうしたシグネチャ変更を割と大胆に行っている。

### 呼び出し側の修正

`Arel::Table.new` を呼んでいる箇所（Active Record 内部や Arel のノード生成周りのコード）がすべて新しい形に書き換えられています。
...
Arel::Table.new(table_name, klass)

# 新
Arel::Table.new(name: table_name)
Arel::Table.new(name: table_name, klass: klass)
```

対象は以下のようなファイル群です（一部抜粋）:
...
- `activerecord/lib/arel/table.rb`（本体）
- Arel 関連のテスト群（`activerecord/test/cases/arel/...` 一式）

テストファイルでは、`Arel::Table.new(:users)` のような呼び出しが大量にあり、それらが一斉に `Arel::Table.new(name: :users)` のような形に置き換えられているため、`+392/-392` と行数だけ見ると大きく見えますが、ロジックとしては「呼び出し方の統一・リファクタ」に留まっています。
...
- 公式には `Arel::Table` は private API（nodoc）ですが、Arel を直接使って SQL を組み立てているアプリ／ライブラリでは影響を受ける可能性があります。
- この PR により、`Arel::Table.new` のシグネチャが変わっているため、**Rails 内部と同じく Arel を直接 new しているコードは修正が必要**になります。

### 想定されるブレイク

もし自前コードで下記のように書いていると壊れます:

```ruby
# 壊れる可能性のあるコード例（旧スタイル）
users = Arel::Table.new(:users)
users = Arel::Table.new(:users, User)
users = Arel::Table.new(:users, nil, some_type_caster)
```
...
users = Arel::Table.new(name: :users, klass: User)
users = Arel::Table.new(name: :users, type_caster: some_type_caster) # 引数名は実装に合わせる
```

### 注意点

- `Arel::Table` は private API なので、この変更はセマンティクス上は「非公開 API の内部整理」に分類されますが、実際には Arel を直接使っているコードが多いため、**マイナーアップデートでも破壊的変更と感じられる**可能性があります。
- この PR 自体には新しいテスト追加はなく、既存テストの引数形式のみ更新されているため、「機能的な振る舞いの変更」は意図していないと見てよいです（インターフェイスの形のみ変更）。
- CHANGELOG は更新されていないため、この変更に気づきにくい可能性があります。Arel を直接触っている場合は、アップデート時に `Arel::Table.new` の使い方を確認することを推奨します。
...
- この PR の動機となった前提変更:  
  ...
  （`Arel::Table` にオプションの追加引数が入った PR）

- 対象 PR 本体:  
  https://github.com/rails/rails/pull/57829

- `Arel::Table` は Rails の public API ではないものの、Arel ベースでクエリを組む高度なユースケースでは利用されることが多いため、Rails アップグレードガイドやコミュニティ情報も確認すると安全です。

---


## [#57833](https://github.com/rails/rails/pull/57833) Only reset predicate builder on table_name exists {#pr-57833}

**マージ日**: 2026/6/24 | **作成者**: [@gmcgibbon](https://github.com/gmcgibbon)

1. 概要 (1-2文で)  
Active Record の `table_name` が「遅延計算(lazy)」になったことで、最初の計算時にも `predicate_builder` がリセットされてしまう不具合があり、それを修正する PR です。具体的には、すでに `predicate_builder` が存在する場合にだけリセットするよう条件を追加し、テーブル名の初回決定ではリセットされないようにしています。
...
- rails/rails#57642 で `table_name` の算出が「必要になるまで計算しない」方式に変更された。
- その実装の中で、「`table_name` が変わったら `predicate_builder` をリセットする」という処理が行われていた。
- しかし「初めて `table_name` を計算したタイミング」でも、このリセットが実行されてしまい、
  - クエリ時にカスタムな型ハンドラ（テストでは Regexp を使用）を使って `table_name` を決定するようなケースで、想定外に `predicate_builder` が作り直され、不整合が起きる可能性があった。
- 上記が issue rails/rails#57824 で報告された不具合。
...
def table_name=(name)
  @table_name = name
  @predicate_builder = nil # 常にリセット
end
```

あるいは、`table_name` の決定ロジックの中で「テーブル名変化 → `@predicate_builder = nil`」としていたが、  
初回計算時にも走ってしまっていた。
...
def table_name=(name)
  @table_name = name
  @predicate_builder = nil if defined?(@predicate_builder) && @predicate_builder # 既にあるときだけリセット
end
```

実際の 1 行変更は:

- 「無条件で `predicate_builder` を `nil` にしていた」コードを
- 「`predicate_builder` がすでにセットされているときにだけ `nil` を代入する」条件付きのコードに変えた

という趣旨です。

### 挙動イメージ

- 初回に `table_name` が計算されるケース:
  - 以前: `predicate_builder` があれば・なければに関わらずリセットしようとしていた。
  - 今回: `@predicate_builder` がまだ存在しなければ、何も起きない（リセットされない）。
- 一度 `table_name` が決まり、その後で `table_name` を書き換えるケース:
  - 以前: リセットされる（正しい）。
  - 今回: 依然としてリセットされる（`@predicate_builder` が既にあれば `nil` になる）。

これにより、「テーブル名が本当に変更された場合だけ predicate_builder をリセットする」という本来意図していた挙動になります。

---

3. 影響範囲・注意点  

- 対象: Active Record のクエリ構築ロジック（`predicate_builder`）と `table_name` を動的に決めるようなコード。
  - 特に、テーブル名をカスタムな型ハンドラ（例: `Regexp`）を通して解決しているケースで不具合が出ていた。
- この変更による影響:
  - テーブル名を「初めて」計算するときには、`predicate_builder` はリセットされない。
  - テーブル名を「すでにある状態から変更した」ときには、今まで通り `predicate_builder` がリセットされる。
- 互換性:
  - 一般的なアプリケーション（`table_name` を普通に固定の文字列で指定しているだけ）の場合、挙動はほぼ変わらない。
  - issue #57824 で報告されていたような「遅延決定されるテーブル名＋カスタム型ハンドラ＋高度なクエリ構築」をしているケースでは、今回の修正によってクエリが安定することが期待される。
- テスト:
  - PR 説明上は「テスト追加/更新」はチェックされていない（未実施扱い）ですが、変更は 1 行の条件追加のみで、リスクは比較的小さい範囲です。
  - それでも、`table_name` を runtime で動的に変えるライブラリ／gem を使っている場合は、Rails 更新後にクエリ周りのテストを一通り回しておくのが無難です。
...
- 本 PR が修正する Issue:  
  - https://github.com/rails/rails/issues/57824
- 関連する以前の変更（テーブル名の lazy 計算導入）:  
  - https://github.com/rails/rails/pull/57642
- 変更ファイル:  
  - `activerecord/lib/active_record/model_schema.rb`（1 行のみ差分）

---


## [#57832](https://github.com/rails/rails/pull/57832) Bump json to 2.20.0 and pass `allow_comments: true` {#pr-57832}

**マージ日**: 2026/6/24 | **作成者**: [@yahonda](https://github.com/yahonda)

1. 概要 (1-2文で)  
json gem を 2.20.0 に更新し、Rails 本体が「コメント付き JSON(JSONC)」をパースしている箇所で `allow_comments: true` オプションを明示的に渡すようにした PR です。これにより、json 2.20.0 で追加された「コメント検出時の非推奨警告」による nightly テストの失敗を解消しています。
...
- `Gemfile.lock`
  - `json` を `2.15.2` → `2.20.0` に更新

このバージョンから、デフォルトでは JavaScript 由来のコメント (`// ...` や `/* ... */`) を JSON 内でサポートすることが非推奨となり、コメントを含む JSON をパースすると警告が出るようになりました。  
将来的な json 3.0 では、この挙動が「警告」から「エラー」に格上げされる予定です。

### `allow_comments: true` を付けた箇所

コメント入り JSON を「意図的に」読み込んでいる箇所にだけ `allow_comments: true` を追加しています。

#### 1) Active Support のメッセージシリアライザ fallback

ファイル: `activesupport/lib/active_support/messages/serializer_with_fallback.rb`

- 不正確(=「イレギュラー」)な JSON を読むテストケース向けの `_load` 実装で、`ActiveSupport::JSON.decode` を経由して JSON を読む箇所に `allow_comments: true` を追加。
...
# 変更後
ActiveSupport::JSON.decode(payload, allow_comments: true)
```

このテストは「コメント付き JSON を読めること」を意図的に検証しているため、コメント許可オプションを明示的に指定する方針になっています。

#### 2) Railties: `devcontainer.json` 関連 (JSONC by spec)
...
`devcontainer.json` は実質 JSONC (コメントを含む JSON フォーマット) が仕様として許容されているファイルであり、Rails 側でもコメントを含んだ状態で生成・再読込しています。

そのため、以下のように JSON 読み込み処理に `allow_comments: true` を追加:
...
# 変更後（概略）
JSON.parse(File.read(devcontainer_path), allow_comments: true)
```

テストヘルパ側でも同様に `JSON.parse` 呼び出しに `allow_comments: true` を付与し、`devcontainer.json` に含まれるコメントで警告が出ないようにしています。

### 解消したテスト失敗

json 2.20.0 適用後に nightly CI で失敗していたのは、Strict Warnings モードで「警告を例外化」しているテストです。  
該当ログはいずれも「Encountered comment in JSON. This will raise an error in json 3.0 unless enabled via `allow_comments: true`」という警告を原因として落ちていました。
...
- **Rails 本体の影響範囲は限定的**
  - 変更ファイルは 4 つ、行数も +5/-5 とごく小規模で、影響箇所も「コメント付き JSON を読むことを前提にしている箇所」に限られています。
  - 既存のコメントなし JSON を読む処理は挙動に変化はありません。

- **将来の json 3.0 への備え**
  - コメント付き JSON を読みたい場合は、今後は `allow_comments: true` を**必須で付ける**べき、という upstream の方針に沿っています。
  - Rails 内部でコメント付き JSON を扱う箇所は、今回の対応で将来の 3.0 でも動作継続できるようになります。

- **アプリケーション側開発者への示唆**
  - 自前コードで `JSON.parse` / `ActiveSupport::JSON.decode` を使ってコメント付き JSON を読んでいる場合、json 2.20.0 以降では同じ警告が出ます。
  - 将来の壊れ方を避けるには以下のどちらかが必要です:
    - JSON からコメントを撤去する（純粋な JSON として扱う）
    - コメントを残したい場合: `JSON.parse(data, allow_comments: true)` のようにオプションを明示する

- **古い json バージョンとの互換性**
  - `allow_comments: true` は 2.7.1 でもエラーにならず受理されることが確認されており、古い環境との後方互換性に問題はないと明言されています。
  - そのため、gem バージョンが環境によって微妙に異なるケースでも、このオプション指定自体がクラッシュの原因になる心配は低いです。

---

4. 参考情報 (あれば)  

- json gem の変更 PR: ruby/json#1001  
- 該当コミット:  
  https://github.com/ruby/json/commit/138b9a2c4932ad2df1024ce3af18a5cc3ae9e901  
- json 2.20.0 のコメント非推奨仕様:  
  - コメント付き JSON をパースすると警告
  - json 3.0 でエラーへ格上げ予定
- Rails nightly 失敗ログの例:  
  - https://buildkite.com/rails/rails-nightly/builds/4483

---


## [#57778](https://github.com/rails/rails/pull/57778) Fix configuring query_log_tags in database.yml {#pr-57778}

**マージ日**: 2026/6/23 | **作成者**: [@hmcguire-shopify](https://github.com/hmcguire-shopify)

1. 概要 (1-2文で)  
`database.yml` で `query_log_tags` を設定した際に正しく読み込まれない不具合があり、`HashConfig` 内でネストされたキーをシンボルではなく文字列として扱うように修正した PR です。これにより、`database.yml` で `query_log_tags` を設定した場合に、期待通りクエリログタグが有効になります。
...
### 背景
- `ActiveRecord::DatabaseConfigurations::HashConfig` では「トップレベルのキーのみ」シンボル化されます。
- そのため、ネストした設定（例: `query_log_tags: { application: ... }` の `application` 部分）は「文字列のキー」のままです。
- ところが、`query_log_tags` を参照する実装がネストしたキーをシンボル (`:tags`, `:application` 等) としてアクセスしていたため、設定が読めない状態になっていました。

### 実際の修正ポイント

`activerecord/lib/active_record/database_configurations/hash_config.rb` の変更  
- ネストされた `query_log_tags` 関連の設定取得時に、キーをシンボルではなく「文字列」で参照するように変更。

イメージとしては、以下のような変更が行われています（実際のコードは簡略化）:

```ruby
# 変更前（イメージ）
def query_log_tags_config
  config[:query_log_tags]&.dig(:tags)
end
...
ポイントは「`config` の中身がトップレベル以外は文字列キーなので、`:query_log_tags` ではなく `"query_log_tags"` でアクセスする必要がある」という点です。

### テストの追加・修正

`activerecord/test/cases/query_logs_test.rb` にテストが追加・修正されています。

- `database.yml` から `query_log_tags` を読んだ場合に、  
  - 設定されたタグが実際のクエリログに出力されること  
  - ネストしたキーが文字列アクセスで正しく参照できること  
を確認するテストが増えています。

例として、`database.yml` で以下のように設定した場合:
...
この設定に基づいて、SQL ログに `"application=my_app"` や `"environment=development"` といったタグが含まれていることをテストしています。
...
- 影響範囲:
  - `database.yml` で `query_log_tags` を設定しているアプリケーションに影響します。
  - これまで「設定しているつもりだが、ログにタグが出てこなかった」ケースが、この修正で正しく動作するようになります。
- 互換性:
  - 外部インターフェース（`database.yml` の書き方）は変わりません。
  - 「今まで動いていたものが壊れる」類の後方互換性の問題は基本的になく、むしろ「今まで効いていなかった設定が効くようになる」という性質の修正です。
- 注意点:
  - Rails をこの修正を含むバージョンに上げた際、クエリログのフォーマットや内容（タグの有無）が変わるため、ログ解析ツールや監視設定でクエリログをパースしている場合は、タグの増加による影響を確認すると良いです。
  - `database.yml` 側でキー名を誤ってシンボル風に書いている（例: `:application:` のような）場合は引き続き無効なので、通常の YAML の文字列キーとして記述しているか確認してください。

---

4. 参考情報 (あれば)  

- 関連 PR: https://github.com/rails/rails/pull/57641  
  - `query_log_tags` 機能まわりの元の変更・議論が含まれていると考えられます。
- 実運用での利用例（イメージ）:
  - マルチテナントや複数サービスから同一 DB に接続している場合に、`application` や `shard` などのタグを付けて、どのコンテキストからのクエリかをログ上で識別しやすくする用途。  
  - 例:
  ...
  - この PR により、上記のような設定が確実にログに反映されます。

---


## [#57021](https://github.com/rails/rails/pull/57021) De-anonymize generated attribute module for Store {#pr-57021}

**マージ日**: 2026/6/23 | **作成者**: [@hmcguire-shopify](https://github.com/hmcguire-shopify)

1. 概要 (1-2文で)  
ActiveRecord::Store がモデルに `store_accessor` などで生成する属性用モジュールについて、これまで匿名モジュールだったものに名前を付け、先祖チェーンで何のモジュールか分かるようにした PR です。あわせて、そのモジュールへの参照方法を「インスタンス変数で保持」から「定義済みの名前で参照」に変更しています。
...
### 背景
`ActiveRecord::Store` は以下のようなコードで、ハッシュ型のカラムに対して仮想属性を定義します:
...
- 匿名モジュールを作成
- その中に `color`, `color=`, `homepage`, `homepage=` などのメソッドを定義
- その匿名モジュールを `User` に `include` する

という動きをしていました。その結果、`User.ancestors` を見ると次のように匿名モジュールが見えていました:
...
この `#&lt;Module:0x...>` が「Store が生成した属性メソッドを持つモジュール」なのかどうかは見ただけでは判別できません。

### この PR の主な変更点

1. **Store 用のモジュールに名前を付ける**

   匿名モジュールではなく、意図の分かる名前を持つモジュールとして定義されるようになりました。  
   具体的なクラス名は差分から正確な文字列は分かりませんが、イメージとしては以下のような形になります（擬似コード）:
   ...
   # 以後: 名前付きモジュール
   module StoreGeneratedAttributes
     # ここに store_accessor が定義するメソッドが入る
   end

   include StoreGeneratedAttributes
   ```

   あるいは、モデルクラス毎に異なる定数名を付けているかもしれませんが、重要なのは「`User.ancestors` に出てきたときに、Store 用のモジュールだと分かるようになる」点です。

   変更後は、`User.ancestors` を見ると:

   ```ruby
   User.ancestors
   # =>
   # [User,
   #  ActiveRecord::Store::GeneratedAttributes,  # 例
   #  ...]
   ```

   のように、何のモジュールかが分かる名前が表示されるようになります。

2. **モジュール参照方法をインスタンス変数から「名前ベース」に変更**

   以前は、生成したモジュールをクラスのシングルトンクラスにインスタンス変数で覚えさせていたようです（擬似コード）:

   ```ruby
   singleton_class.attr_reader :_store_attributes_module

   def store(...)
     @_store_attributes_module ||= Module.new
     include @_store_attributes_module

     @_store_attributes_module.module_eval do
       # attr 定義
     end
   end
   ```
   ...
   この PR では、  
   - モジュールに名前を付けたことで  
   - その名前（定数）を通していつでも参照できる  

   ようになったため、わざわざシングルトンクラスにインスタンス変数で保持する必要がなくなり、その仕組みを削除しています。  
   イメージとしては、次のようなリファクタリングです:

   ```ruby
   # 以前: @_store_attributes_module に保持
   mod = @_store_attributes_module

   # 以後: 名前付き定数を参照
   mod = self::StoreGeneratedAttributes
   ```

   実際のコードでは、`ActiveRecord::Store` 内でクラス用モジュールを探す/作るロジックが整理され、状態をインスタンス変数で抱えずに済むようになっています。

---

3. 影響範囲・注意点  

- **`User.ancestors` などの introspection 結果が変わる**
  - これまで `#&lt;Module:0x000...>` のように見えていた匿名モジュールが、名前付きモジュールとして現れるようになります。
  - テストやメタプログラミングコードで `ancestors` の内容を文字列比較している場合は、結果が変わる可能性があります。

- **内部 API / 非公式挙動に依存している場合の注意**
  - `ActiveRecord::Store` が内部で持っている匿名モジュールのオブジェクト ID や、シングルトンクラスのインスタンス変数 (`@_store_attributes_module` のようなもの) に直接アクセスするようなコードがあれば壊れます。
  - ただし、これらは Rails の公開 API ではなく内部実装に近いので、通常のアプリケーションコードが依存している可能性は低いです。

- **挙動面 (定義されるメソッドや動作) は変わらない**
  - `store`, `store_accessor` 自体の外向きの挙動、生成される属性のアクセス方法、シリアライズ/デシリアライズの仕組みは変更されていません。
  - あくまで「どういうモジュールにそれらを載せるか」と「そのモジュールをどう管理するか」という内部構造の整理です。

- **デバッグ性・可読性の向上**
  - `rails console` などで ancestors を見たとき、「このモジュールは Store が生成した属性メソッド用だ」と視認できるため、問題解析時に役立ちます。

---

4. 参考情報 (あれば)  

- この PR は `#57020` をベースにしていると明記されているため、Store 周辺のリファクタリングの一環と考えられます。
- ActiveRecord::Store の公式ガイド:  
  https://guides.rubyonrails.org/active_record_postgresql.html#hstore  
  (PostgreSQL の `hstore` などと組み合わせる例がありますが、`store` 自体の使い方の参考になります)

---


## [#57642](https://github.com/rails/rails/pull/57642) Remove some memoization in Active Record and Active Model {#pr-57642}

**マージ日**: 2026/6/23 | **作成者**: [@gmcgibbon](https://github.com/gmcgibbon)

1. 概要 (1-2文で)  
Active Record / Active Model 内のいくつかのメモ化(memoization)を削除し、Ractor（マルチスレッド/マルチRactor環境）で扱いやすくする変更です。`arel_table` などを毎回計算するようにし、性能劣化はベンチマーク的に許容範囲に収めつつ、Ractorセーフな設計に近づけています。
...
従来はこれらがインスタンス変数などにキャッシュされており、1プロセス内で共有される形になっていました。Ractor では「Ractor 間で共有するオブジェクトは凍結されていなければならない」ため、メモ化によって「凍結されていないが共有されうるオブジェクト」が増えると Ractor 対応の足かせになります。

このPRでは「毎回計算 or その場で構築する」方向に寄せることで、メモ化のためのインスタンス変数を極力持たないようにしています。

#### `arel_table`

- 以前: `ActiveRecord::Base.arel_table` はクラスインスタンス変数にキャッシュされる実装（クラスごとに一度だけ Arel::Table を生成して再利用）だった。
- 変更後: 毎回 `Arel::Table.new` 相当で生成する方向に変更され、Arel::Table 側に若干の最適化が入っています。

`activerecord/lib/arel/table.rb` の変更から推測できる点:
- Arel::Table 自体が、利用時に必要な情報を都度組み立てる/引き回すように少し整理されており、Active Record 側からの呼び出しで「長生きするキャッシュ」を作りにくくしています。
- テスト (`activerecord/test/cases/arel/table_test.rb`, `fake_record.rb`) にも手が入っており、新しい生成パターンでも挙動が同等であることを確認しています。
...
対象: `ActiveModel::AttributeMethods` / `ActiveRecord::AttributeMethods`

- 各種 `attribute_*` メソッド (例: `respond_to?(:title)`, `method_missing` 経由での属性アクセス) に使う正規表現やパターンをキャッシュしていた箇所がシンプル化。
- パターンのキャッシュ用インスタンス変数が削除され、パターンの構築をメソッド呼び出しごとに行うか、より局所的なキャッシュに切り替えています。

これにより、`respond_to?` や `method_missing` 経由の動的属性メソッド解決が、Ractor 間で共有されるミュータブルなキャッシュに依存しなくなります。

#### `attributes_builder`

対象: モデルインスタンス生成時に使われる Attributes のビルダーオブジェクト。

- 以前: クラスごとに `@attributes_builder` のような形でキャッシュして再利用。
- 変更後: ビルダーを毎回構築する、あるいはキャッシュの持ち方を変え、Ractor セーフでない ivar を持たない方向に変更。

これにより `Post.new` のベンチマークで多少遅くなっていますが、PRのベンチマークを見る限り「μsオーダーで数十％程度」の退行で、トレードオフとして許容と判断されています。

#### `column_names`

対象: `ActiveRecord::ModelSchema#column_names`

- 以前: `@column_names` などのクラスインスタンス変数に配列をキャッシュ。
- 変更後: `columns.map(&:name)` 相当を都度計算するか、既存のスキーマ情報から毎回導出する形へ。
- ベンチマーク上は `Post.column_names` は大きく遅くなっているように見えますが、それでも 4M i/s 程度出ており、多くのアプリでボトルネックにはなりにくいという判断と見られます。

#### `default_scope_override`

対象: `ActiveRecord::Scoping::Default`

- default_scope を「上書きする」ための内部状態をメモ化していた部分を削除。
- これもクラスにぶら下がるミュータブルなキャッシュ（Ractor 間共有しづらいもの）を消す目的です。

### 2-2. Active Model 側の変更

`activemodel/lib/active_model/attribute_methods.rb` での変更 (行+6/-18) から:

- attribute メソッドパターンを管理するロジックが整理され、キャッシュ用 ivar が減っている。
- これにより Active Record 以外の Active Model ベースのクラスも、Ractor の制約下で扱いやすくなります。
...
PR に含まれるベンチマークから読み取れる影響:

- ほぼ同等:
  - `Post#title`
  - `Post#method_missing(:title)`
  - `Post#respond_to?(:title)`
- 明確に遅くなったもの:
  - `Post.arel_table` : 約 26.7M i/s → 約 5.2M i/s
  - `Post.column_names` : 約 30.4M i/s → 約 4.0M i/s
  - `Post#to_partial_path` : 約 19.9M i/s → 約 6.1M i/s
  - `Post.new` : 約 294k i/s → 約 256k i/s
  - 一部のクエリ構築 (`Post.where(title:)`, `DefaultScopedPost.all`) も ~15〜20% 程度遅くなっている

とはいえ、絶対値としてはいずれも十分高速で、実アプリでボトルネックになるケースは限定的と想定されています。著しく影響が出るケース (例: `column_names` を tight loop で叩きまくるメタプログラミング) があれば、今後のリファクタや局所的キャッシュなどで吸収する余地があります。

### 3-2. Ractor 対応への影響

- Ractor ではクラスインスタンス変数などに「変更可能なオブジェクト」を持ち、それを複数Ractorで共有するのが難しい／禁止されます。
- このPRにより、Active Record / Active Model のクラスレベル API が、内部状態をほとんど持たずに動作する方向に近づきます。
- 将来的に「モデルクラスを Ractor 間で共有しつつ、各Ractorから普通にクエリを投げる」というユースケースをサポートしやすくなります。

### 3-3. 互換性・振る舞い

- 公開APIのシグネチャや返り値の型は変わっていないため、通常のアプリコードの挙動は変わりません。
- 挙動に依存していた内部的な副作用（「最初の呼び出しだけ遅くて、以降速い」などのメモ化前提の最適化）には依存しないほうがよくなります。
- メモ化されなくなったことで、アプリ側で頻繁に `column_names` などを呼んでいると CPU 使用率がわずかに上がる可能性があります。もし問題になる場合は、アプリ側で独自にキャッシュする、あるいは別の API (schema cache など) を検討するのが良いです。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57642  
- 関連する概念:
  - Ruby Ractors: マルチコア並列実行のための機構。オブジェクト共有の制約が厳しく、メモ化やグローバルキャッシュと相性が悪い場合がある。
  - Memoization vs. Ractor-safety: クラスレベルでメモ化すると「長く生きるミュータブルオブジェクト」が増え、Ractor フレンドリーでなくなるため、今回のように「パフォーマンスを多少犠牲にしてでも Ractor 対応を優先する」設計判断が行われています。

---


## [#57783](https://github.com/rails/rails/pull/57783) Split template lookup into raising and non-raising {#pr-57783}

**マージ日**: 2026/6/23 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1-2文で)  
このPRは、テンプレート探索APIを「例外を投げる版」と「例外を投げない版」に整理し、`find_all(...).first` や `rescue` に頼らずにテンプレート探索できるようにした変更です。あわせて、テンプレートが「どれか1つでも存在するか」を調べる `any_formats?` を追加し、テンプレート探索ロジックを `LookupContext` に集約しやすくしています。
...
これまでの状況（推測される従来API）:
- `LookupContext#find` または `PathSet#find` が「見つからないと例外を投げる」インターフェイスになっていた
- 例外を避けたい場所では、以下のようなパターンが必要だった:
  ```ruby
  template = lookup_context.find_all(name, prefixes, partial: false).first
  # template が nil かどうかで存在チェック
  ```
  あるいは
  ```ruby
  template = lookup_context.find(name, prefixes, partial: false) rescue nil
  ```

このPRの主な変更点:
- 「例外を投げない `#find`」を導入し、呼び出し側が `find_all(...).first` や `rescue` に頼らずに、単純に `nil` チェックで存在確認できるようにしています。
- それに伴い、一部コードで `find_all(...).first` → 非例外版 `find` に置き換えられています。

PR本文にある文言から読み取れる意図:
> Use the non-raising #find in places where we had to use find_all(...).first to avoid exceptions or where we had to rescue.

つまり:
- `find_all` は「複数候補 + ソート済み」なためオーバースペックで、単に「一番良い候補が1つ欲しい」場面には非効率。
- 今後、「ベストマッチ1件だけを返す `#find`」をより効率的に実装しやすくするために、インターフェイスを整理している。

### 2-2. `any_formats?` の追加

`LookupContext` に新たな finder メソッドとして `any_formats?` が追加されています:

- 役割:
  - 指定したパラメータ（名前・パス・フォーマットなど）でテンプレート探索を行い、「該当するテンプレートがひとつでもあるか」を `true/false` で返すメソッド。
- PR文中の言及:
  > Also introduce an #any_formats? finder to keep the template lookup in LookupContext, similarly to ImplicitRender using any?/any_templates?

`ImplicitRender` では、暗黙レンダリングの可否を判断するために `any?` / `any_templates?` といったメソッドが使われており、同様の役割を `LookupContext` レベルに整理したものと考えられます。

これにより:
- 「テンプレートが存在するかどうか」だけ知りたい箇所で、パスセットや呼び出し側が直接 `find_all` に触る必要がなくなる
- テンプレート探索のロジックが `LookupContext` に集約され、後続の最適化（キャッシュ戦略・検索アルゴリズム変更など）がやりやすくなる

### 2-3. 具体的な変更箇所の概要

ファイルごとの主な変更意図（コード行数は小さいが、インターフェイスの使い方に影響）:

- `action_view/lookup_context.rb`
  - 非例外版 `find` の導入・整理
  - `any_formats?` の追加
- `action_view/path_set.rb`
  - `LookupContext` 側の新しい `find` / `any_formats?` に対応するための委譲や実装調整
- `template_renderer.rb` / `partial_renderer.rb` / `layouts.rb` / `etag_with_template_digest.rb` / `digestor.rb`
  - それぞれで使われていた `find_all(...).first` や `rescue` ベースの呼び出しを、新しい非例外版 `find` や `any_formats?` ベースに書き換え
  - 例外に依存せず、より明示的な「見つからない場合は nil / false」という分岐に統一
- `lookup_context_test.rb`
  - 新しいふるまい (`find` が nil を返すケース、`any_formats?` の結果など) に合わせたテスト更新

簡略イメージ（※実際のコードとはインターフェイスが多少異なる可能性あり）:
...
# 変更後 (非例外版 find)
template = lookup_context.find(name, prefixes, partial: false)
if template
  render template
else
  # フォールバック処理
...
# 存在チェックだけしたい場合
if lookup_context.any_formats?(name, prefixes, partial: false)
  # 何らかのテンプレートが存在する
else
  # 存在しない
end
...
- 主な影響範囲:
  - ActionView のテンプレート探索まわり
  - レイアウト解決 (`layouts.rb`)、部分テンプレート (`partial_renderer`)、通常のテンプレートレンダリング (`template_renderer`)
  - テンプレートダイジェスト (`digestor`, `etag_with_template_digest`)

- Rails アプリケーション側への直接的な影響:
  - 既存のパブリックAPI（例: `render` ヘルパー）の挙動は変わらない想定で、主に内部実装の整理・最適化の下地作り。
  - ただし、`ActionView::LookupContext` や `ActionView::PathSet` の内部APIを直接使っているアプリ・エンジン・gem は注意が必要。
    - `find` が「常に例外を投げる前提」で rescue していたコードは、今後 nil 戻りになるかもしれないため、挙動が変わる可能性があります。
    - `find_all(...).first` で「見つからないときは nil」としていた箇所は、新しい非例外版 `find` に置き換えられる余地があります。

- パフォーマンス上の含意:
  - 現時点では大きな最適化というより「将来、`#find` を『ベストマッチ1件だけ返す』形で高速化しやすくする」ための設計変更。
  - `find_all` は引き続き「候補を全件ソートして返す」ためオーバーヘッドが大きい場面で、新 `find` に移行することで今後の恩恵を受けやすくなります。

- ライブラリ作者・高度な利用者向けの注意:
  - 独自のテンプレートリゾルバやカスタムの `LookupContext` 拡張をしている場合、新しく追加されたメソッド（`any_formats?`）や非例外型 `find` のふるまいと整合性が取れているか確認した方がよいです。
  - 内部的に `find` の実装が変わることを見越し、「例外で見つからないことを検出する」より「戻り値の nil / false で判定する」スタイルに寄せるのが将来互換的です。
...
- PR 本文で参照されている関連コード:  
  - `ImplicitRender` の `any?` / `any_templates?` 実装例  
    https://github.com/rails/rails/blob/8817776b7864df906542ef14cf7f12db5e4d48ae/actionpack/lib/action_controller/metal/implicit_render.rb#L40  
  - ここでは「テンプレートがひとつでもあるか」をライトウェイトに確認するためのAPIパターンが既に使われており、今回の `any_formats?` はその `LookupContext` 版と言える位置づけです。

- 今後想定される発展:
  - `LookupContext#find` の内部実装が「全候補探索 + ソート」から「ベストマッチ1件を効率的に返す」ものへ差し替えられる可能性が高く、ビュー探索のパフォーマンス改善の足掛かりとなる変更です。

---


## [#57809](https://github.com/rails/rails/pull/57809) Fix broken anchor links in guides [8-1-stable] {#pr-57809}

**マージ日**: 2026/6/23 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails 8.1 系（8-1-stable ブランチ）のガイド内で壊れていたアンカーリンク（ページ内リンク）を、実際に存在する見出し ID に合わせて修正するバックポート PRです。主にガイド同士の参照で古い見出し ID やコピペミスを新しい ID に置き換えており、ドキュメントのナビゲーション品質を改善します。
...
### 全体像
- 対象ブランチ: `8-1-stable`
- 元 PR: #57806 のバックポート（main 向けの修正のうち、8-1-stable にも該当するものだけを移植）
- 修正対象:
  - 11 ファイル
  - 15 箇所のリンクを書き換え（15 insertions / 15 deletions）

いずれも「別ガイド内の見出しへのリンク」のアンカー部分を、実際の生成 HTML（`id` 属性）に合わせた形へ修正しています。

#### 代表例（before / after）

1. `action_mailer_basics.md`  
   - 変更前:
     ```markdown
     [リンクテキスト](i18n.html#translations-for-action-mailer-e-mail-subjects)
     ```
   - 変更後:
     ```markdown
     [リンクテキスト](i18n.html#action-mailer-e-mail-subjects)
     ```
   - 意味: i18n ガイド内の対象セクション名・ID が変わったため、存在しない `#translations-for-...` ではなく、現行の `#action-mailer-e-mail-subjects` に合わせています。

2. `active_record_basics.md`
   - 変更前:
     ```markdown
     [fixtures の説明](testing.html#the-low-down-on-fixtures)
     ```
   - 変更後:
     ```markdown
     [fixtures の説明](testing.html#fixtures)
     ```
   - 意味: `testing.html` 内の見出し ID が `#the-low-down-on-fixtures` から短く `#fixtures` に変わっているため修正。

3. `active_record_callbacks.md`
   - 変更前:
     ```markdown
     [動的ファインダ](active_record_querying.html#dynamic-finders)
     ```
   - 変更後:
     ```markdown
     [動的ファインダ](active_record_querying.html#dynamic-finder-methods)
     ```

4. `active_record_validations.md`（has_one / has_many へのリンク修正）
   - 変更前（例）:
     ```markdown
     [has_one](association_basics.html#the-has-one-association)
     [has_many](association_basics.html#the-has-many-association)
     ```
   - 変更後:
     ```markdown
     [has_one](association_basics.html#has-one)
     [has_many](association_basics.html#has-many)
     ```
   - 同じ誤リンクが 2 箇所ずつあり、合計 4 箇所修正されています。

5. `caching_with_rails.md`（STI へのリンク）
   - 変更前:
     ```markdown
     [単一テーブル継承](association_basics.html#single-table-inheritance)
     ```
   - 変更後:
     ```markdown
     [単一テーブル継承](association_basics.html#single-table-inheritance-sti)
     ```

6. `configuring.md`（autoload パス）
   - 変更前:
     ```markdown
     [autoload paths](autoloading_and_reloading_constants.html#autoload-paths)
     ```
   - 変更後:
     ```markdown
     [autoload paths](autoloading_and_reloading_constants.html#config-autoload-paths)
     ```

7. `getting_started_with_devcontainer.md`（Rails プロジェクト作成のアンカー名変更）
   - 変更前:
     ```markdown
     (getting_started.html#creating-a-new-rails-project)
     ```
   - 変更後:
     ```markdown
     (getting_started.html#creating-a-new-rails-app)
     ```

8. `rails_on_rack.md`（誤コピペ修正とキャッシュ関連）
   - 誤コピペと思われる設定項目のアンカー修正:
     - 変更前:
       ```markdown
       (configuring.html#config-action-record-migration-error)
       ```
     - 変更後:
       ```markdown
       (configuring.html#config-action-dispatch-x-sendfile-header)
       ```
     - 「migration error」ではなく X-Sendfile 関連の設定へのリンクが正しい、という意図。
   - Conditional GET のアンカー:
     - 変更前:
       ```markdown
       (caching_with_rails.html#conditional-get-support)
       ```
     - 変更後:
       ```markdown
       (caching_with_rails.html#conditional-gets)
       ```

9. `security.md`（上と同じ Conditional GET のアンカー修正）
   - 変更前:
     ```markdown
     (caching_with_rails.html#conditional-get-support)
     ```
...
10. `routing.md`
    - 変更前:
      ```markdown
      (command_line.html#the-app-and-helper-objects)
      ```
    - 変更後:
      ```markdown
      (command_line.html#the-app-object)
      ```

11. `testing.md`
    - 変更前:
      ```markdown
      (active_job_basics.html#create-the-job)
      ```
    - 変更後:
      ```markdown
      (active_job_basics.html#defining-a-job)
      ```

### スキップされた修正（8-1-stable では不要なもの）
main ブランチ側でセクション名が変わっているが、8-1-stable ではまだ変わっていないものは適用していません。具体的には:

- `active_storage_overview.html` 内の以下セクションに関するリンク
  - `#analyzing-files`
  - `#requirements`
  - `#purging-unattached-uploads`
- `engines.md` → `engines.html#the-inheritance-hierarchy`

これらは 8-1-stable では該当セクション名・構成が異なるため、そのままバックポートすると逆に壊れる可能性がある、という判断です。

### 検証方法
- `RailsGuides::Markdown` の実際のレンダリングパイプラインを使って各ガイドを HTML 生成。
- 生成 HTML の `h2`〜`h5` 見出しに付与されている `id` を収集。
- 全ての新しいアンカーがその `id` と一致することを確認（13 個すべて成功）。
...
- 影響範囲:
  - Rails Guides（HTML / Markdown ドキュメント）のみで、フレームワーク本体の挙動・API には影響しません。
  - 8.1 系のガイドをブラウザで閲覧する際に、該当リンクをクリックしても 404 相当（ページトップへ飛ぶ、意図しない位置に飛ぶ等）だったものが、正しく該当セクションへスクロールするようになります。
- 注意点:
  - 自前で Rails Guides をミラーしている場合（社内サイトや翻訳版など）:
    - レイアウトや見出し構造が 8-1-stable と異なると、同様のアンカー問題が起こりうるため、自分のサイト側での ID を確認してリンクを合わせる必要があります。
  - ガイドの特定セクションに直接リンクしているブログ記事・ドキュメントは、すでに「新しい ID」でリンクしているなら問題ありませんが、「古い ID」を使っている場合は、読者が 8.1 の公式ガイドを見る際にずれが発生するかもしれません（この PR は公式ガイド側を現行の見出し ID に合わせているだけなので、逆に外部リンク修正は手動対応になります）。

---

4. 参考情報 (あれば)  

- 元 PR（main ブランチ向け）: #57806  
- 本 PR: #57809（8-1-stable バックポート）  
- Rails Guides のアンカー生成は `RailsGuides::Markdown` による見出し (`h2`〜`h5`) の ID 付与仕様に基づいており、`rake guides:generate:html` と同じロジックで検証済みであることが明記されています。

---


## [#57754](https://github.com/rails/rails/pull/57754) Make AbstractController::Caching.view_cache_dependency ractor safe {#pr-57754}

**マージ日**: 2026/6/22 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
`AbstractController::Caching.view_cache_dependency` が Ractor セーフになるよう実装が変更され、ビューキャッシュの依存関係を格納する配列およびその中の Proc を Ractor 共有可能な形で扱うようにした PRです。Ractors を利用するアプリでもビューキャッシュまわりでスレッド／Ractor 安全性を確保しやすくなります。
...
### 主なポイント
- `_view_cache_dependencies` 配列の更新方法を「破壊的変更」から「dup → 追加 → freeze → 再代入」のパターンに変更。
- 依存関係として登録される Proc を `ActiveSupport::Ractors.try_shareable_proc` に通して、Ractor 共有可能にできるようにした。
- これに合わせたテストが `actionpack/test/controller/caching_test.rb` に追加。

#### 配列の扱いを Ractor セーフに
従来は `_view_cache_dependencies` に対して破壊的操作（`<<` など）を行っていた可能性がありますが、Ractor 環境では「複数 Ractor から共有されているオブジェクトへの破壊的変更」が禁止されます。

この PR では、以下のようなパターンに統一しています（擬似コード）:

```ruby
def add_view_cache_dependency(proc)
  proc = ActiveSupport::Ractors.try_shareable_proc(proc)

  deps = _view_cache_dependencies.dup
  deps << proc
  self._view_cache_dependencies = deps.freeze
end
```

ポイント:
- `dup`：既存の配列を複製し、新しいインスタンスに対してのみ変更を加える。
- `<<`：複製した配列にのみ要素を追加。
- `freeze`：最終的な配列を immutability 保証のために凍結し、Ractor 共有を安全に。
- 「複数 Ractor から参照される配列」は変更されず、「新しい配列」を生成して差し替えるイミュータブル風パターン。

#### Proc を Ractor 共有可能に
`view_cache_dependency` では「ビューキャッシュのキー計算などに使う Proc（ブロック）」を依存関係として登録します。  
これらはコントローラのインスタンスに対して `instance_exec` されるため、Proc 自体を Ractor 共有可能な形にしておく必要があります。
...
を呼び出し、Ractor を利用しているアプリであれば「共有可能な Proc」に変換／検証し、問題がなければそのまま配列に入れます。  
`try_shareable_proc` は、内部で `Ractor.shareable?` や `Ractor.make_shareable` 相当の処理をラップしているヘルパで、Ractor を使っていない環境でも安全に呼べるようになっています。

#### テスト追加
`actionpack/test/controller/caching_test.rb` には、以下を確認するテストが追加されていると考えられます：

- 新しく追加される依存関係が、`try_shareable_proc` を通されていること。
- `_view_cache_dependencies` が再代入ベースで更新され、配列が `freeze` されていること。
- 複数の依存関係を追加しても正しく蓄積されること。

（具体的なテスト名までは PR 情報から分かりませんが、上記の動作が担保されるようになっています。）
...
- 対象機能:
  - `AbstractController::Caching.view_cache_dependency` を利用しているコントローラ/フレームワークコード。
  - ビューキャッシュのキー計算などに Proc を渡している部分。

- Ractor を「使っていない」アプリ：
  - 振る舞いはほぼそのままで、内部実装がより安全になった程度の影響です。
  - `_view_cache_dependencies` が常に `freeze` された配列として保持されるようになるため、この配列を外部で直接変更していた場合は例外が出るようになります（本来想定されていない使い方なので、問題になるケースは稀）。

- Ractor を「使っている / 使う予定がある」アプリ：
  - `view_cache_dependency` に渡す Proc の中で、Ractor 共有不可能なオブジェクト（ミューテーブルなグローバル状態やスレッドアンセーフな構造など）に依存していると、`try_shareable_proc` の内部でエラーになる、もしくは共有不可と判定される可能性があります。
  - Ractor を見据えるなら、これらの Proc は「引数とレシーバからのみ状態をとる」「外部のミューテーブル状態に依存しない」ようにしておくと安全です。

- パフォーマンス:
  - 依存追加のたびに `dup` + `freeze` + 再代入が走るため、理論上はごく小さなオーバーヘッドがありますが、ビューキャッシュ依存の登録頻度は一般的に低いため実用上の影響は軽微と考えられます。
...
- 同様の変更パターンを導入した PR:  
  ...
  → 「配列を破壊的に変更せず、dup → freeze → 再代入」という Ractor セーフなパターンを、他の箇所でも採用済み。

- Ractor と共有可能オブジェクトの概要（Ruby docs）：  
  - https://docs.ruby-lang.org/en/master/Ractor.html  
  - https://docs.ruby-lang.org/en/master/Ractor.html#class-Ractor-label-Shareable+objects

この PR により、`AbstractController::Caching` によるビューキャッシュの依存関係管理が、Rails 本体として Ractor 対応方針（イミュータブル化と shareable proc 化）に揃えられた形になります。

---


## [#57753](https://github.com/rails/rails/pull/57753) Make ExecutionContext.after_change ractor safe {#pr-57753}

**マージ日**: 2026/6/22 | **作成者**: [@andrewn617](https://github.com/andrewn617)

1. 概要 (1-2文で)  
`ActiveSupport::ExecutionContext.after_change_callbacks` を Ractor セーフにするため、コールバック配列の扱いを不変（freeze）かつ共有可能な形に変更した PRです。これに伴い、`ExecutionContext` を利用している `helpers.rb` や `query_logs.rb` 周りも、Ractor セーフな前提で動作するよう微調整されています。
...
### 背景・問題点  
- `ActiveSupport::ExecutionContext.after_change_callbacks` は **Proc の配列**として実装されており、  
  ```ruby
  ActiveSupport::ExecutionContext.after_change_callbacks # => [#&lt;Proc:...>, ...]
  ```
- Ractor セーフにするには、「Ractor 間で共有されるオブジェクト」は **不変かつ Ractor-shareable** である必要があります。
- これまでの実装では、配列をそのまま `<<` などで破壊的に更新しており、  
  - 配列自体が freeze されていない  
  - Ractor 間で共有されたときに「後から中身を書き換えられる」  
  という点で Ractor セーフではありませんでした。

### この PR の方針  
- 「登録される Proc 自体が Ractor shareable であること」は **呼び出し側の責務** とし、Rails 側では配列の扱いのみを Ractor セーフにする。
  - 説明にある通り、Proc は `instance_exec` ではなく単に `call` されるだけなので、Proc 内で何をキャプチャしているか（Ractor shareable かどうか）は呼び出し側で制御すべき。
- コールバック配列の更新を **非破壊 + freeze 付き** に変更することで、  
  「登録後の配列は変更されない（＝共有しても安全）」状態を保証する。

### コア実装イメージ（擬似コード）  

`activesupport/lib/active_support/execution_context.rb` の変更の趣旨は以下のような形です（概念的な擬似コード）:
  ...
  class ExecutionContext
    class << self
      def after_change(&block)
        # 以前は: @after_change_callbacks << block のような破壊的更新だったと考えられる
        callbacks = (@after_change_callbacks || []).dup
        callbacks << block
        callbacks.freeze
        @after_change_callbacks = callbacks
      end

      def after_change_callbacks
        @after_change_callbacks || EMPTY_FROZEN_ARRAY
      end
    end
  end
end
...
ポイント:
- `dup` して新しい配列を作り、そこに `block` を追加。
- その配列を `freeze` してからインスタンス変数に再代入。
- 呼び出し元が参照する配列は不変となり、Ractor 間で共有しても安全。

### 他ファイルの変更の方向性  

1. **`actionpack/lib/abstract_controller/helpers.rb`**  
   - コントローラの `helpers` 関連で `ExecutionContext` の変更フックを使っている箇所が、  
     新しい `after_change` の挙動（不変なコールバック配列）でも問題なく動くように微調整。
   - 具体的には、`ExecutionContext.after_change` への登録方法（ブロックの渡し方や、返り値・参照の仕方）を、今回のインターフェイスに揃えていると考えられます。

2. **`activerecord/lib/active_record/query_logs.rb`**  
   - Query Log 用に `ExecutionContext` を使っている箇所（コンテキスト変更時にログ情報を更新する処理）が、  
     新しいコールバック機構でも同様に動作するように 1 行レベルの修正。
   - たとえば:
     - コールバック登録のタイミング/方法
     - 実際に `ExecutionContext` から値を読む際の呼び出しパターン  
     などが、Ractor セーフな前提に合うようになっている。

3. **テスト (`activesupport/test/execution_context_test.rb`)**  
   - `+10` 行の追加で、`after_change` コールバックが:
     - Ractor セーフな形で保持されること
     - 期待通りのタイミングで呼ばれること  
     を検証するテストが追加されている。
   - これにより、将来的な変更でも Ractor セーフな挙動が壊れないよう担保している。
...
- **非公開 API（no-doc）である点**  
  - `ExecutionContext.after_change` は公式ドキュメント化されていない内部的な API であり、利用しているアプリ/ライブラリは少ない想定です。
  - とはいえ、独自に使っている場合は影響を受ける可能性があります。

- **コールバック Proc 自体の Ractor セーフ性は利用側の責任**  
  - この PR は「**配列の管理**」を Ractor セーフにしただけで、  
    各 Proc が Ractor shareable かどうかはチェックしません。
  - たとえば以下のように `self` や Ractor 非対応オブジェクトをキャプチャすると、Ractor で共有できない可能性があります:
    ```ruby
    obj = SomeNonShareable.new

    ActiveSupport::ExecutionContext.after_change do
      obj.do_something  # <- Ractor 間共有は NG になりうる
    end
    ```
  - Ractor を利用する場合は、ブロック内でキャプチャするオブジェクトや状態を Ractor shareable にする必要があります。

- **配列が freeze されることによる挙動の違い**  
  - 返ってきた `after_change_callbacks` の配列をアプリコード側で破壊的に変更していた場合、
    ```ruby
    ActiveSupport::ExecutionContext.after_change_callbacks << another_proc
    ```
    のようなコードは `FrozenError` になる可能性があります。
  - 正しくは `ExecutionContext.after_change` を通して登録する必要があります。

- **マルチ Ractor 環境での安全性向上**  
  - これにより、ExecutionContext のコールバック登録/実行に関しては、「登録後のコールバック配列を複数 Ractor 間で共有する」ことが現実的になります。
  - Ractor ベースで並列化を進めるアプリケーションや gem にとって、フレームワーク側の前提条件が揃ってきた形です。

---

4. 参考情報 (あれば)  

- Ruby Ractor の shareable オブジェクト要件  
  - 公式: https://docs.ruby-lang.org/en/master/doc/ractor_md.html  
- `ActiveSupport::ExecutionContext` は主に:
  - ログ（`ActiveRecord::QueryLogs` など）
  - コンテキスト依存の情報伝播（リクエストスコープ情報 等）  
  で使われることが多く、今回の変更はこうした周辺機能の Ractor 対応の一環と考えられます。

---


## [#57815](https://github.com/rails/rails/pull/57815) Revert pull request #57814 {#pr-57815}

**マージ日**: 2026/6/22 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1-2文で)  
このPR (#57815) は、直前のPR (#57814) で導入された ActiveSupport の JSON エンコード周りの変更を「丸ごとリバート」したものです。理由は、Ruby 3.3 の Ractor を起動した際にクラス属性周りで `[BUG] should have cvar cache entry` という VM クラッシュが CI で再発するためです。
...
> This reverts commit `abbcb8d7...`, reversing changes made to `dbafe650...`.

つまり、  
- `dbafe650...` で行った変更  
- それをさらに `abbcb8d7...` で上書き／調整した変更  

をまとめて打ち消しています。実体としては `activesupport/lib/active_support/json/encoding.rb` の実装と、そのテスト (`activesupport/test/json/encoding_test.rb`) を元の状態に戻しています。

変更差分の概要:

- `activesupport/lib/active_support/json/encoding.rb`
  - +14 / -13 行の変更  
  - JSON エンコードに関わる実装が、以前のバージョンに巻き戻されている
- `activesupport/test/json/encoding_test.rb`
  - -17 行  
  - 新たに追加されていたテストケースが削除され、以前のテスト構成に戻されている
- `Gemfile.lock`
  - +2 / -2 行  
  - 依存する gem のバージョン／リビジョンが、元の状態に合わせて微調整されている

PR本文にある通り、今回のリバート対象の変更は Ractor を使ったときに Ruby 本体のバグを引き起こしやすい（特にクラス変数／クラス属性のキャッシュ周り）ため、CI を安定させる目的で戻しています。

### 2-2. Ractor + class attributes で起きている問題

PR 説明から:

> as soon as we start a Ractor, class attributes may crash with `[BUG] should have cvar cache entry`, depending on test order.

- Rails 側コードで「クラス属性（class_attribute など）」を使う実装があり
- それが Ruby 3.3 系（おそらく 3.3.4 付近、PR 文中では「4.0.5 / 4.0.6」と書かれているので RubyGems / ランタイムの特定バージョンを指している可能性あり）と組み合わさると
- Ractor を起動したタイミングで Ruby VM が内部エラー (`[BUG] should have cvar cache entry`) を出し、プロセスごとクラッシュする

この問題自体は以前の PR (#57684) の時点で把握されており、そのときと同系列のクラッシュが今回の変更でも CI 上で再現したため、「一旦変更をなかったことにする」という判断になっています。
...
- ActiveSupport::JSON のエンコード動作は、#57814 適用前の挙動に戻ります。
  - #57814 で追加・変更された JSON エンコードの仕様変更（バグフィックスや最適化など）はいったん無効化されたと考えてよいです。
- 新しく追加されていた JSON エンコードに関するテストも削除されているため、テスト観点も以前の状態に戻っています。

### 3-2. CI / 実運用への影響

- 短期的には:
  - Rails の CI で発生していた Ruby VM クラッシュ（Ractor + class attributes）を避けられるため、CI の「フレーク（テスト順序依存の不安定な失敗）」が減ります。
- 中長期的には:
  - PR 本文にある通り、今後 Ruby 側の修正版（ここでは「4.0.6」と表現されているバージョン）がリリースされ、CI でそのバージョンが使えるようになった段階で、再度今回の変更に相当する修正を導入し、その際に `skip` を追加するなどの対処が行われる見込みです。

### 3-3. 開発者視点での注意点

- Ractor を利用した並列実行を自前で試している場合:
  - クラス属性（`class_attribute`、`cattr_accessor` など）と Ractor を組み合わせると Ruby 側のバグで VM がクラッシュする可能性があります。
  - これは Rails コードというより Ruby 本体の cvar キャッシュ周りの問題であり、Rails がそれを「踏みやすくしてしまった」ためにリバートした、という構図です。
- JSON エンコード周りで #57814 の変更前提のコードやワークアラウンドを検討していた場合:
  - 今回のリバートでその前提はなくなり、従来の挙動に依存して問題ない状態に戻ります。
  - #57814 の具体的な仕様変更に依存していた場合は、当面はその利用を見送る必要があります。

---

4. 参考情報 (あれば)  

- 元の変更: pull request #57814（今回リバートされた内容）
- 関連する既知の問題: pull request #57684（同様の Ractor + class attributes + `[BUG] should have cvar cache entry` が言及されている）
- Ruby 側のバグ:
  - `[BUG] should have cvar cache entry` は Ruby VM 内部のクラス変数キャッシュ関連のアサーションエラーであり、Ractor 対応や並列実行時のキャッシュ不整合が原因と考えられます。
  - PR 説明で言及されている「4.0.5 / 4.0.6」は、Ruby もしくは関連コンポーネントのバージョンを指しており、修正版（4.0.6）のリリース後に Rails 側で `skip` を追加しつつ変更を再適用する方針が示されています。

---


## [#57814](https://github.com/rails/rails/pull/57814) Make ActiveSupport::JSON encoders Ractor-shareable {#pr-57814}

**マージ日**: 2026/6/22 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1-2文で)  
ActiveSupport::JSON のエンコーダを「Ractor-shareable」（Ractor 間で共有可能）にするため、エンコーダを早い段階で `freeze` するように変更した PRです。これにより `Ractor.make_shareable` を明示的に呼ばずに、Ractor 内からそのまま JSON エンコード機能を利用できるようになっています。

---

2. 変更内容の詳細  

### a. 背景: Ractor と shareable オブジェクト
Ruby の Ractor では、Ractor 間で共有できるオブジェクトは `shareable` としてマークされたものだけです。  
- `Ractor.make_shareable(obj)` を使う  
- もしくは「凍結済み（`freeze` 済み）」「ミュータブルな状態を持たない」などの条件を満たす構造にしておく  
ことで、shareable にできます。

ActiveSupport::JSON のエンコーダは内部的にハッシュや Proc / lambda、クラスへのマッピングなどを保持しているため、そのままだと shareable ではなく、Ractor 内で使うために `Ractor.make_shareable` を呼ぶ必要がありました。

この PR では「そもそも最初から shareable な形にしておく」ことで、この呼び出しを不要にしています。

---

### b. ActiveSupport::JSON エンコーダの実装変更

`activesupport/lib/active_support/json/encoding.rb` の主なポイント:

- JSON エンコーダ関連のデータ構造を**初期化時点で `freeze`** し、以後変更しない設計に寄せている
  - 例:  
    - エンコード時に利用するマッピングテーブル  
    - クラスごとのエンコーダ登録テーブル  
- これにより、Ruby の Ractor が「このオブジェクトは不変であり、shareable」と判断できるようになる

PR 説明文にある以下の方針がそのまま実装に反映されています:
...
```ruby
# 変更前のイメージ
encoders = ActiveSupport::JSON::Encoding::ENCODERS
Ractor.make_shareable(encoders)

# 変更後のイメージ
# ライブラリ側で freeze して shareable になっているので
encoders = ActiveSupport::JSON::Encoding::ENCODERS
# そのまま Ractor 間で利用可能
```

のような感じで、アプリ側が `Ractor.make_shareable` を意識する必要がなくなります。

---

### c. テストコードの変更 (Ruby 3.4 / 4.0 対応)

`activesupport/test/json/encoding_test.rb` の主なポイント:

- Ruby バージョンごとにテストを分岐
  - Ruby 3.4 以下では、該当テストを**定義しない**ようにして、skip の失敗を回避
    - 「テストを実行して skip する」のではなく「そもそもテストケースを定義しない」方向で対処
- Ruby 4.0 向けには、JSON gem 側の挙動変更（またはバージョン更新）にあわせてテストが落ちないように追従
  - `Gemfile.lock` に JSON のバージョン更新が入り、テストの skip 条件などもそれに合わせて調整されている
...
に対応する変更で、  
「Ractor 関連テストが Ruby の特定バージョンで false positive な失敗を出さないようにした」調整です。

---

3. 影響範囲・注意点  

- **Ractor を使っていないアプリケーション**  
  - 実質的な挙動は変わらず、通常の JSON エンコード/デコードには影響しない想定です。
  - エンコーダの内部構造がよりイミュータブル寄りになった程度の影響です。

- **Ractor を使っている / 使う予定のアプリケーション**  
  - ActiveSupport::JSON のエンコーダを Ractor 内から利用する際、明示的な `Ractor.make_shareable` は不要になります。
  - 今後、Ractor ベースの並行処理で JSON エンコードを多用するケースで、より自然に ActiveSupport::JSON が使えるようになります。

- **カスタムエンコーダを追加している場合の注意点**  
  - ActiveSupport::JSON::Encoding に対して、ランタイムでミュータブルな形でエンコーダを追加・変更している場合、そのやり方が Ractor と相性が悪くなる可能性があります。
    - 例えば、クラス -> Proc のマッピングを後から書き換えるようなコードがあると、そのマッピングを shareable にできず、Ractor 間で共有できない可能性があります。
  - 「Ractor から参照したいエンコーダ」は原則として**アプリ起動時に確定させ、凍結された状態に置く**、という設計がより強く求められます。

- **Ruby バージョン依存のテスト挙動**  
  - ライブラリ開発者視点では、Ruby 3.4 以前と 4.0 以降で Ractor や JSON の挙動が異なるため、  
    テスト・CI で Ruby バージョンごとの分岐に注意が必要です。
  - Rails 本体側ではその分岐を吸収していますが、プラグイン / エクステンションを書く場合も同様の考慮が必要になります。
...
- Ractor-shareable オブジェクトの仕様（英語）:  
  https://docs.ruby-lang.org/en/master/doc/ractor_md.html#label-Shareable+objects
- ActiveSupport::JSON::Encoding (Rails ガイド / API リファレンス):  
  https://api.rubyonrails.org/classes/ActiveSupport/JSON/Encoding.html

---


## [#57789](https://github.com/rails/rails/pull/57789) Strip inline comments from unquoted `.env` values {#pr-57789}

**マージ日**: 2026/6/22 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails の `DotEnvConfiguration` が `.env` ファイルを読む際に、**未クォート値の末尾コメント（`#` 以降）を自動的に除去**するようになりました。これにより、`.env` では一般的な「値の右にコメントを書く」記法で、値が誤ってコメント込みで解釈される不具合が解消されます。
...
`DotEnvConfiguration` は `.env` 行について:

- 行頭のコメント行のみスキップしていた:
  ```ruby
  next if line.start_with?("#")
  ```
- `KEY=value` の形式を次のような正規表現でパースしていた:
  ```ruby
  \A([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*)\z
  ```
  ここで `(.*)` が **行末まで全部**を value として取得するため、末尾コメントも値に含まれていました。
...
```ruby
option(:port)   #=> "5432 # primary"  # 本当は "5432" が欲しい
require(:secret) #=> "hi # x"         # 本当は "hi"
```

エラーにはならず**サイレントに誤った値**が入るのが問題でした。

### 今回の修正

未クォート値に対して、末尾のインラインコメントを削除するメソッドが追加されました:
...
```ruby
unquote(strip_inline_comment(value))
```
...
| 入力行 | 解釈される値 | 挙動の理由 |
|---|---|---|
| `PORT=5432 # primary` | `"5432"` | `5432` の後ろに空白があり、`# primary` はコメントとして削除 |
| `DB=$(op read x) # note` | `$(op read x)` を実行 | `$(...)` 部分はそのまま。後ろの ` # note` は削除される |
| `URL=http://host#frag` | `"http://host#frag"` | `#` の前に空白がないため、コメントではなく値の一部とみなす |
| `PASS="a # b"` | `"a # b"` | 先頭が `"` なので `strip_inline_comment` はスキップ。引用符内部の `#` は値として保持 |

サンプルコードイメージ:

```ruby
# これまで:
ENV["PORT"]   #=> "5432 # primary"

# これから:
ENV["PORT"]   #=> "5432"

# URL のように # を含む値
ENV["URL"]    #=> "http://host#frag"  # 変化なし

# コマンド展開を使ったケース
ENV["DB"]     #=> (op read x の結果)
```

`.env` ツール群（dotenv gem, foreman, docker-compose など）が採用している慣習と同様のふるまいになります。
...
- 対象: Rails の `ActiveSupport::DotEnvConfiguration` を使って `.env` を読み込む箇所
- 影響するケース:
  - **未クォート値**で、値の後ろに「スペース + `#` コメント」を書いていたケース
  - これまでは `#` 以降も値に含まれていたが、今後は除去される

Rails 以前の挙動に依存して、あえてコメントを値に含めていたケースがあると、値が変わる可能性はありますが、`.env` の一般的な使い方からすると、今回の変更の方が自然かつ他ツールとの互換性があります。

### 既知の制約・注意点

1. **クォートされている値の末尾コメントはそのまま**
   - 例: `TWO="2" # two`
   - 現状、この `# two` は値からは削除されません（`"2" # two` という文字列として扱われる）。
   - 理由: 正しく削るには「終端のクォート位置を把握し、エスケープ `\"` も考慮したパース」が必要で、現時点ではスコープ外とされているためです。
   - 将来的に改善される可能性があります。

2. **未クォートの `$(...)` 内にある `#` も削られうる**
   - 例:
     ```env
     X=$(echo "a # b")
     ```
   - 正規表現は単純に「空白 + `#` 以降」を削除するため、`$(...)` 内部の ` # b")` もコメントとして扱われてしまう可能性があります。
   - dotenv gem も同様の制約を持っていると明示されており、「ごく稀なケース」として既知の制限として扱われています。
   - 必要なら、こうしたケースは値をクォートしたり、コマンド側で工夫する必要があります。

3. `.env` パーサーはまだ unreleased サブシステム
   - この機能はまだ公開 API として広く安定運用されている段階ではない前提で設計されているため、将来的に挙動が調整される余地があります（例えば、クォート値のコメント除去の追加など）。

---

4. 参考情報 (あれば)

- 本 PR が目指すのは「Rails 独自仕様」ではなく、既存ツール（dotenv gem, foreman, docker-compose など）の `.env` コメント解釈ルールと同等の挙動です。
- `.env` 設計のベストプラクティス:
  - 末尾コメントを使う場合は、**未クォート**値で:
    ```env
    PORT=5432 # primary DB
    ```
  - 引用符を使う場合でコメントを付けたいときは、コメントを**別行に分ける**方が安全です:
    ```env
    # Primary password
    PASS="a # b"
    ```
- 実際のコード変更:
  - `activesupport/lib/active_support/dot_env_configuration.rb` に `strip_inline_comment` の追加と、その呼び出しを 1 箇所挿入（+11/-1）
  - `activesupport/test/dot_env_configuration_test.rb` に対応テストを追加（+20）

---


## [#57800](https://github.com/rails/rails/pull/57800) Reword allow/let phrasings in guides {#pr-57800}

**マージ日**: 2026/6/22 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Railsガイド内の英語表現を、文法的により正確で自然な形にリライトしたPRです。コードやAPIの挙動には一切影響せず、ドキュメント中の「allow」「let」の使い方を修正しています。

---

2. 変更内容の詳細  

このPRは「文法的に正しい英語」に揃えるためのガイド文言修正で、いずれも意味は同じですが表現のみが変わっています。

### 2.1 `let` + 不定詞の修正

`let` は「to なしの原形不定詞」を取るため、次のように修正されています。

- `let the user to pass` → `let the user pass` （2箇所）

Railsとしての意図（「ユーザーに〜させる」）は変わらず、文法だけを正しています。

#### イメージ（英語パターン）
- 誤: `let the user to configure this option`
- 正: `let the user configure this option`

### 2.2 `allow` の目的語省略の修正

`allow` は通常「allow + 目的語 + to 不定詞」の形を取るため、「誰に（何に）」を明示するように変更されています。

- `This allows to ...` → `This allows you to ...`
- `allows for you to specify ...` → `allows you to specify ...`  
  （`allows for you to` は不自然なので `allows you to` に簡潔化）

また、動名詞を使う自然な表現に直したものもあります。

- `allows to print` → `allows printing`

#### イメージ（英語パターン）
- 誤: `This allows to configure the encryption options.`
- 正: `This allows you to configure the encryption options.`  
  または: `This allows configuring the encryption options.`
...
- **影響範囲**
  - 対象は Rails ガイド内のテキストのみ（4ファイル・5行）。
  - フレームワークのコード、挙動、API シグネチャには一切変更なし。
  - バージョンアップ時にアプリケーション側の修正は不要です。

- **注意点**
  - ドキュメントを翻訳・ローカライズしている場合は、**元の英語に追従するためのマージ差分**として認識しておくとよいです（意味は変わらないが、差分として検出されます）。
  - 英語で社内ドキュメントやブログを書く際に、同様のパターン（`let` + bare infinitive, `allow + object + to`）に揃えると、より自然で標準的な表現になります。
...
- Cambridge Grammar – Verb patterns (verb + infinitive or verb + -ing?)  
  - `let` / `make` は能動文では `to` なしの不定詞を取る:  
    例: `Let him go.`, `She made me do it.`
- Merriam-Webster – allow  
  - 例文はいずれも `allow + object + to-infinitive` 型:  
    例: `The system allows you to transfer data easily.`

これらに合わせる形で、Rails ガイドの表現も統一されています。

---


## [#57805](https://github.com/rails/rails/pull/57805) Fix `Http::Headers#merge` mutating the original request {#pr-57805}

**マージ日**: 2026/6/22 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionDispatch::Http::Headers#merge` が「新しい Headers を返して元のリクエストを変更しない」という仕様に反して、元の `request.env` を破壊的に変更していたバグを修正するPRです。`merge` が正しく新しい `Http::Headers` インスタンスを返し、呼び出し元のリクエストに副作用を与えないようになりました。
...
元々の `merge` の実装はドキュメント上は以下のようなイメージで説明されており、「新しい `Http::Headers` を作る」とされています:
...
- `request.headers` は一度生成されると `@headers ||= ...` でメモ化される
- `@headers` には元の `request` (`@req`) への参照が保持される
- `@req.dup` はインスタンス変数を「参照のまま」コピーする（浅いコピー）
- `Rack::Request::Env#initialize_copy` は `@env` だけを再コピーし、`@headers` はそのまま（参照共有）

という状態だったため、

1. `@req.dup` で複製したリクエストも、元のリクエストと **同じ `@headers` オブジェクト** を参照してしまう  
2. その `Http::Headers` インスタンスの中の `@req` は **元のリクエスト** を指している  
3. `merge` 内で `headers.merge!(headers_or_env)` が呼ばれると、その実装は `@req.set_header` を通して **元の request.env** に書き込む

結果として、「コピーしたつもり」のヘッダ操作が元のリクエストの env を直接書き換える、というバグになっていました。

### 今回の修正

PRでは `Http::Headers#merge` の実装が 1 行だけ変更されています（+1 / -1）。実際のパッチは次のような形です（意図を示すための概略）:

```ruby
def merge(headers_or_env)
  headers = @req.dup.headers
  # 以前は merge! で破壊的に書き換えていた
  # headers.merge!(headers_or_env)

  # 修正: 非破壊的な merge を使って新しい Headers を作る
  headers.merge(headers_or_env)
end
```
...
- `merge!`（破壊的）ではなく `merge`（非破壊的）を使うことで、
  - メモ化されて共有されている `Http::Headers` インスタンスを直接変更しない
  - 返り値として新しい `Http::Headers` を返す、というドキュメント通りの挙動になる
- これにより `merge` 呼び出し時に元の `request.env` に書き込まれることがなくなる

### テストの追加

`actionpack/test/dispatch/header_test.rb` に 14 行のテストが追加されています。テストの狙いは:

- `request.headers` を一度読み出した後に
- `request.headers.merge(foo: "bar")` を呼んでも
- 元の `request.env` / `request.headers` の内容が変化しないこと

を確認することです。これは「`headers` が既にメモ化されている実際のリクエストライフサイクル」で問題が再発しないことを検証しています。
...
- 主な影響範囲:
  - `ActionDispatch::Http::Headers#merge` を使ってリクエストヘッダを「コピーしてから一部上書き」しているコード
  - Rack ミドルウェアや Rails ミドルウェア、あるいはテストコード等で `request.headers.merge(...)` を利用している箇所

- これまでの挙動との違い:
  - 以前:  
    - `request.headers.merge(new_headers)` を呼ぶと、**元の `request.env` が（意図せず）書き換わる** 場合があった  
    - そのため「新しいヘッダセットを作っただけのつもりが、以降の処理で同じリクエストのヘッダが変わっている」というバグを引き起こしうる
  - 以後:  
    - `merge` は本当に「新しい `Http::Headers` を返すだけ」で、**呼び出し元のリクエストには副作用がない**
    - ドキュメント通りの直感的な挙動になる

- 既存コードへの注意点:
  - もし既存コードが「`merge` を呼ぶことで元のリクエストヘッダが書き換わる」挙動に暗黙的に依存していた場合、そのコードは動かなくなります（とはいえ、仕様的にはその依存はバグと言えるため、修正された挙動に合わせるべきです）。
  - リクエストのヘッダを本当に書き換えたい場合は、従来どおり `request.headers['X-Foo'] = 'bar'` などのインターフェイスを使うべきです。`merge` はあくまで「コピーを返す」メソッドです。

---

4. 参考情報 (あれば)  

- Ruby の `Object#dup` はインスタンス変数を「参照のまま」コピーするため、メモ化されたオブジェクトや、元オブジェクトへの参照を内部に持つオブジェクトがある場合、今回のような「浅いコピーによる共有バグ」が起こりやすいです。
- Rails 内部でも:
  - `initialize_copy` / `dup` のオーバーライド時に、どのインスタンス変数を再構築・再dupすべきか
  - メモ化キャッシュを複製時にクリアすべきか
  といった点は要注意箇所です。
- 類似の問題を避けるためには:
  - 「コピー後のオブジェクトが元のオブジェクトの状態に書き戻す」パス（今回でいう `@req.set_header`）がないか確認する
  - `dup` で解決しようとせず、明示的に新しいインスタンスを構築して値を移す、という設計も検討する価値があります。

---


## [#57808](https://github.com/rails/rails/pull/57808) Validate offset value at call time like limit {#pr-57808}

**マージ日**: 2026/6/22 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`offset` に渡す引数が整数でない場合、これまではサイレントに `to_i` で変換されていましたが、このPRにより `limit` と同様に `Integer(value)` でバリデーションし、不正な値は即座に `ArgumentError` を投げるようになりました。これにより、SQL生成時と in-memory finder 経路の挙動が整合的になり、バグを早期に検出できます。
...
`ActiveRecord::Relation#offset!` が、`limit!` と同様に引数を `Integer(value)` でチェックするようになりました。
...
# SQL 生成時:
offset_value.to_i  # ここで初めて整数化
```

このため:

```ruby
Post.offset("abc").to_sql # => "SELECT ... OFFSET 0"
Post.offset("5x").to_sql  # => "SELECT ... OFFSET 5"
```

のように、不正／不完全な文字列がサイレントに 0 や 5 に変換されていました。
...
```ruby
Post.offset("abc")  # => ArgumentError: invalid value for Integer(): "abc"
Post.offset("5x")   # => ArgumentError: invalid value for Integer(): "5x"
Post.offset("3")    # => OK, offset_value == 3
Post.offset(3)      # => OK, offset_value == 3
Post.offset(nil)    # => OK, offset_value == nil
```

と、`limit` と同じパターンのバリデーションになります。

### テストの追加・変更

変更されたテストは主に次の観点を確認しています:

- 文字列の数値 `"3"` を渡した場合は従来どおり正しく動くこと  
  （既存の `test_finder_with_offset_string` が引き続きパスすることを確認）

- 不正な文字列／数値に変換できない値を渡した場合に `ArgumentError` が発生すること  
  例: `"abc"`, `"5x"` など

これにより挙動変更が意図通りであり、かつ既存の正当なユースケースは壊していないことを担保しています。
...
- `ActiveRecord::Relation#offset` / `#offset!` を利用している全てのコード
  - 特に、コントローラやAPI層でリクエストパラメータをそのまま `offset` に渡している場合に影響が出る可能性があります。

### 互換性の観点

以前:
- `offset("abc")` → SQL 上は `OFFSET 0` になり、例外は出ない
- `offset("5x")` → SQL 上は `OFFSET 5` になり、例外は出ない

以後:
- どちらも `ArgumentError` が発生する

既存アプリで「不正な offset パラメータが来たら暗黙に 0 / 切り捨てとして扱う」という挙動に“依存していた”場合、今回の変更で例外が発生するようになります。  
そのため、外部入力（クエリパラメータなど）をそのまま渡している場合は、アプリケーション側でバリデーション／サニタイズを行う必要があります。
...
def index
  ...
  # 不正値を 0 にフォールバックしたい場合の一例
  offset =
    begin
      Integer(offset_param) if offset_param.present?
    rescue ArgumentError
      0
    end

  @posts = Post.offset(offset).limit(20)
end
```

### 一貫性の向上

- すでに `limit` は `Integer(value)` を使っており、「limit は即時エラー、offset はサイレント変換」という不整合が解消されます。
- in-memory finder パス（`offset_value + index` など）では元々「整数であること」を前提に演算していたため、ここで想定外の型が入り込むバグも早期に表面化します。

### セキュリティへの影響

PR本文でも明記されている通り、これは「挙動の一貫性とエラーメッセージの明確化」が目的であり、セキュリティ修正ではありません。  
すでに `offset_value.to_i` によって、SQL インジェクション的な利用は中和されていました。
...
- 関連挙動:
  - `ActiveRecord::Relation#limit!` が `Integer(value)` を用いるようになったコミット: `f46b838b3f`
- 影響を確認したい場合:
  - テスト・ログ・Sentry などで `ArgumentError: invalid value for Integer()` の新しい発生箇所がないかを確認すると、今回の変更による影響範囲を洗い出しやすいです。

---


## [#57811](https://github.com/rails/rails/pull/57811) Update actions/checkout to v7 {#pr-57811}

**マージ日**: 2026/6/22 | **作成者**: [@Uaitt](https://github.com/Uaitt)

1. 概要 (1–2文で)  
GitHub Actions の `actions/checkout` を v7 に更新する PRで、Rails 本体の CI ワークフローと、`rails new` / plugin generator が出力する GitHub Actions テンプレートの両方が対象です。アプリやライブラリの挙動には影響せず、CI環境と生成される GitHub Actions 設定の最新版追従が目的です。

---

2. 変更内容の詳細

### a. Rails リポジトリ内の CI ワークフロー更新

以下の GitHub Actions Workflow ファイルで、`actions/checkout` のバージョン指定が更新されています。
...
のように更新されています（実際の before/after の細かいバージョンは PR 内の diff に依存しますが、v7 へと揃える変更です）。

この変更により、Rails 自体の CI が `actions/checkout` v7 を利用するようになります。

---

### b. `rails new` が生成する GitHub Actions テンプレートの更新

`rails new myapp --github` のようなオプションを使った際に生成される GitHub Actions 用テンプレートが更新されています。
...
ここでも、`actions/checkout` のバージョン指定が v7 へ変更されています。

#### 想定される差分イメージ

アプリ用テンプレート (`github/ci.yml.tt`) の一部:
...
plugin 用テンプレート (`github/ci.yml.tt`) でも同様に `@v7` に更新されています。

このため、**今後の Rails 新規プロジェクトやプラグイン**で `--github` オプションを使用した場合、最初から `actions/checkout@v7` を使う CI 設定が生成されます。
...
- **アプリケーションコードへの影響はなし**  
  変更はすべて GitHub Actions のワークフローファイルおよびそのテンプレートに限られており、Rails のランタイム挙動や API には影響しません。

- **これから生成される GitHub Actions 設定のみが対象**  
  既に作成済みの Rails アプリ/プラグインの `.github/workflows/ci.yml` は、自動では書き換わりません。  
  既存プロジェクトで v7 を使いたい場合は、手動で:
...
- **`actions/checkout` v7 由来の差異に注意**  
  一般にメジャーバージョンアップでは、セキュリティ強化や非推奨オプションの整理など、細かな挙動変更が入ることがあります。  
  Rails 的には checkout アクションの標準的な使い方しかしていないため、大きな問題は起きにくい想定ですが、独自の GitHub Actions を組んでいる場合は、`actions/checkout` リポジトリのリリースノートを確認しておくと安全です。

- **CI の再現性への影響**  
  メジャーバージョンを追従することで、今後も GitHub Actions 側のサポート・バグ修正・セキュリティパッチを享受しやすくなります。一方で、「昔の挙動に固定したい」場合は、自身のプロジェクトで敢えて v4 ～ v6 を指定し続けることも可能です。

---

4. 参考情報 (あれば)

- `actions/checkout` 公式リポジトリ（リリースノート・ Breaking changes の確認用）  
  https://github.com/actions/checkout  

- GitHub Actions ドキュメント（`uses:` でのアクション指定方法）  
  https://docs.github.com/actions  

- Rails ガイド: Rails アプリの作成 (`rails new`)  
  https://guides.rubyonrails.org/command_line.html#rails-new  

この PR 自体はあくまで CI/テンプレートのバージョン追従であり、Rails 利用者に対するコード修正の必須事項はありません。

---


## [#57810](https://github.com/rails/rails/pull/57810) Fix use of `class_attribute` for `primary_key_definition` {#pr-57810}

**マージ日**: 2026/6/22 | **作成者**: [@byroot](https://github.com/byroot)

1. 概要 (1-2文で)  
`primary_key_definition` の `class_attribute` の定義タイミングを修正し、継承階層で同じアクセサが何度も再定義されてしまう問題を解消しました。`inherited` フックではなく、Concern が include されたタイミングで定義するように変更されています。
...
### 何が問題だったか
以前のPR (#57775) では、`primary_key_definition` を `class_attribute` で定義する際に、`inherited` フック（サブクラスが定義されたときに呼ばれる）を使っていました。

しかしこの方法だと:

- 継承チェーン（親クラス → 子クラス → 孫クラス …）の各クラスで  
  同じ `class_attribute :primary_key_definition` アクセサが何度も再定義される  
- 不要なメソッド再定義が発生し、挙動の追跡やパフォーマンス、今後の保守性に悪影響を与えうる

という問題があります。

### どう修正されたか
Active Record の `primary_key` 関連の Concern (`ActiveRecord::AttributeMethods::PrimaryKey` 付近) において:

- これまで: Concern が include されたモデルに対して、`inherited` フック内で `class_attribute :primary_key_definition` を定義していた
- これから: Concern が include されたタイミングで、一度だけ `class_attribute :primary_key_definition` を定義するように変更

Ruby on Rails の Concern では、

```ruby
included do
  # ここは include されたときに実行される
end
```

のようなフックが使われますが、この PR では「継承時 (`inherited`) ではなく、Concern が include されたとき (`included`) に `class_attribute` を定義する」という形に寄せています。

それに伴って、`primary_key_definition` 周りのテスト (`activerecord/test/cases/primary_keys_test.rb`) も、  
「継承時の再定義」ではなく「include 時に正しく定義され、継承しても問題なく振る舞うか」を担保するような形に微修正されています。
...
module ActiveRecord
  module AttributeMethods
    module PrimaryKey
      ...
      included do
        class_attribute :primary_key_definition, instance_accessor: false
      end
    end
  end
end
...
上記のように `included` ブロック内で `class_attribute` を定義することで、  
- その Concern を include したクラスに対して一度だけ  
- 適切にクラスレベルの属性を定義する  

という、Rails での典型的なパターンに揃えています。

---

3. 影響範囲・注意点  

- 主な対象: Active Record の `primary_key` 関連メカニズム（`primary_key_definition` を参照する内部実装、およびそれに依存している部分）
- 影響:
  - モデル継承構造（抽象クラス + サブクラス、多段継承、STIなど）で `primary_key_definition` がより正しく・一貫して扱われるようになります。
  - クラス階層が深い場合でも、同じ `class_attribute` が継承のたびに再定義されることがなくなります。
- 既存アプリへの実務的な影響:
  - `primary_key_definition` は基本的に内部用の API であり、通常のアプリケーションコードが直接触ることは少ないため、破壊的変更のリスクは低いと考えられます。
  - もし `primary_key_definition` をメタプログラミング的に操作している場合（特に継承と組み合わせている場合）は、継承チェーンでの値の引き継がれ方を念のため確認すると安心です。

---

4. 参考情報 (あれば)  

- フォローアップ元 PR: https://github.com/rails/rails/pull/57775  
  前回の `primary_key_definition` まわりの変更に対する修正（follow-up）として、この PR が入っています。
- 関連機能:  
  - `ActiveSupport::Concern#included` フック  
  - `ActiveSupport::ClassAttribute` (`class_attribute`) の動作と継承時の振る舞い

---


## [#57795](https://github.com/rails/rails/pull/57795) Support polymorphic associations with custom primary keys through `:inverse_of` {#pr-57795}

**マージ日**: 2026/6/22 | **作成者**: [@kamipo](https://github.com/kamipo)

1. 概要 (1–2文で)  
このPRは、「ポリモーフィック関連 + `:inverse_of`」の組み合わせで、逆側の関連に定義されたカスタム `:primary_key` が正しく使われるようにする変更です。これにより、ポリモーフィック関連先ごとに異なる主キー(例: `uuid`, `slug`, `id`)を自然に扱えるようになります。
...
- いくつかのモデルでは、歴史的な理由から「DBの `id` とは別のユニークな識別子列」を実質的な主キーとして使っている（例: `uuid`, `slug` など）。
- これらのカラム名はモデルごとに異なっており、`alias_attribute` で擬似的に統一していたが、ActiveRecord の関連実装は高速化のため `_read_attribute` を直接呼んでおり、`alias_attribute` が効かない。
  - 結果として、実運用アプリでは `_read_attribute` をオーバーライドして alias 解決するような苦しいワークアラウンドをしていた。

- ポリモーフィック関連 (`belongs_to :subject, polymorphic: true` など) は、コンパイル時に静的に `inverse_of` を決定できないため、自動検出が無効。
  - その影響で、ユーザが明示的に `inverse_of:` を指定していても、その情報が関連の内部実装で十分・一貫して利用されていなかった。
  - 特に「逆側の `:primary_key` 設定」がポリモーフィック関連に反映されないという問題があった。

このPRは、「ポリモーフィック関連でも、明示された `:inverse_of` を見に行って、逆側の `:primary_key` 設定を尊重する」ように実装を改善しています。

---

### 振る舞いの変更の要点

#### 1. ポリモーフィック `belongs_to` で `inverse_of` を見てカスタム `primary_key` を解決

`BelongsToReflection#association_primary_key` が拡張され、以下のような場合に「逆側の関連の `:primary_key`」を参照するようになりました。

- 関連が `polymorphic: true` である
- `inverse_of` が明示的に指定されている
- その逆側の関連(例: `has_many :comments, primary_key: :uuid, as: :commentable`) に `primary_key:` オプションが設定されている

これにより、例えば次のような構成が正しく動作します。

```ruby
# 例: コメントは commentable にポリモーフィックに属する
class Comment < ApplicationRecord
  belongs_to :commentable, polymorphic: true, inverse_of: :comments
end

# Post は uuid を主キー的に使う
class Post < ApplicationRecord
  # DB の primary key は id のままだが実質 uuid を関連キーに使いたいケース
  has_many :comments,
           as: :commentable,
           primary_key: :uuid,   # ここを尊重するようになる
           inverse_of: :commentable
end

# Article は slug を主キー的に使う
class Article < ApplicationRecord
  has_many :comments,
           as: :commentable,
           primary_key: :slug,   # ここを尊重
           inverse_of: :commentable
end

# Video はデフォルトの id を使う
class Video < ApplicationRecord
  has_many :comments,
           as: :commentable,
           # primary_key オプション省略 → :id が使われる
           inverse_of: :commentable
end
```

この状態で `comment.commentable` や `post.comments.build` などの処理を行う際、ポリモーフィック関連であっても、  
- Post なら `posts.uuid`
- Article なら `articles.slug`
- Video なら `videos.id`  
といった形で、モデルごとの `primary_key:` 設定が正しく使用されます。

これまでだと、ポリモーフィック関連では「常に `id` 前提」または「逆側設定が反映されない」挙動になるケースがありましたが、それが解消されます。

#### 2. AutosaveAssociation のロジック簡素化

`AutosaveAssociation` モジュール内で、関連オブジェクト保存時に「どのカラムを foreign key / primary key として使うか」を自前で解決していた箇所が、既存のリフレクションAPIを利用するようにリファクタされています。

- これまで:
  - autosave 用の処理の中で独自に primary key を推測・解決しており、`BelongsToReflection#association_primary_key` などとロジックが二重化・分散していた
- 変更後:
  - `reflection.association_primary_key` 等の既存メソッドを利用して primary key を取得
  - polymorphic + inverse_of + custom primary_key の対応をリフレクション側で一元的に扱える

これにより、autosave（`accepts_nested_attributes_for` 経由など）でポリモーフィック関連を保存する場合でも、逆側の `:primary_key` 設定が整合的に使われます。

#### 3. テストとドキュメント

- `belongs_to_associations_test.rb`, `autosave_association_test.rb` に、  
  - ポリモーフィック + `inverse_of` + カスタム `primary_key`  
 という組み合わせに対するテストケースが追加されています。
- `CHANGELOG.md` にこの挙動変更が追記されており、正式な仕様として扱われます。
- モデル定義 (`author.rb`, `comment.rb`, `person.rb`) および `schema.rb` がテスト用に微調整されています。
...
1. **ポリモーフィックな `belongs_to` を使っている箇所で、`inverse_of` を明示している場合**
   - 逆側（`has_many` / `has_one` 側）の `primary_key:` 設定が、これまでよりも厳密に反映されます。
   - これまで「たまたま `id` 前提で動いていた」コードが、明示的な `primary_key:` 設定に従うようになる可能性があります。

2. **`accepts_nested_attributes_for` など、autosave を伴う関連操作**
   - autosave の際に使われる primary key 解決ロジックが、リフレクションに一元化されたため、ポリモーフィック関連でも一貫性が増します。
   - 逆に、「独自に `_read_attribute` をオーバーライドして alias を吸収していた」ようなハックが、不要または挙動変化の対象になります。

### 互換性・注意点

- **ポリモーフィック関連で `inverse_of` を書かない場合**
  - 従来通り自動検出は無効のままなので、このPRの恩恵は受けません。
  - カスタム主キーを使いたい場合は、「関連両側で `inverse_of` を正しく指定する」ことが前提になります。

- **逆側関連で `primary_key:` を指定しているが、実際の外部キー列と整合していない場合**
  - 今回の変更により、その設定がより厳密に使われるため、関連のロード・保存が失敗したり、意図しない行を参照する可能性があります。
  - とくに、既存アプリで「`primary_key:` を暫定的に書いていたが、ポリモーフィック側が事実上無視していた」ようなケースは要確認です。

- **`alias_attribute` ベースのワークアラウンド**
  - `alias_attribute` ではなく「実際に `primary_key:` で使うカラムを関連オプションとして指定し、それが正しく使われる」形に寄せられるので、将来的に `_read_attribute` のオーバーライドなどを撤去できる余地があります。
  - ただし、このPR自体は `_read_attribute` の挙動を変更していないので、「alias が効くようになる」わけではなく、あくまで「カスタム主キーを正式な設定として渡せるようにした」改善です。

---

4. 参考情報 (あれば)

- 対象PR: https://github.com/rails/rails/pull/57795  
- 関連するキーワード:
  - `polymorphic: true`
  - `inverse_of:`
  - `primary_key:`
  ...
  - `AutosaveAssociation`
- 設計上のポイント:
  - ポリモーフィック関連は型（`*_type`）ごとに参照先モデルが変わるため、「逆側関連の情報をどう統一的に見るか」が難しい領域。
  - 今回は「明示的に `inverse_of` が指定されている場合」に限り、その逆側設定を尊重する方針をとることで、安全性と柔軟性のバランスを取っています。

---


## [#57806](https://github.com/rails/rails/pull/57806) Fix broken anchor links in guides {#pr-57806}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails Guides 内の壊れたアンカーリンク（`file.html#anchor` 形式のセクションリンク）を現状のセクション ID に合わせて修正し、16リンク分のスクロール不能問題を解消した PR です。残った一部のリンクは、該当セクション自体が削除・移動されており、別 PR での著者判断を要するものとして保留されています。
...
### 実施したチェック方法
- 76 本すべてのガイドを HTML 生成
- 生成 HTML の `id="..."` を「正」として収集
- Markdown ソース中の `](file.html#anchor)` 形式のリンクをすべてクロスチェック
- 実在しない ID を指しているリンクを洗い出し、現状の ID へ差し替え  
→ その結果、14 ファイル・16 箇所を修正。

### 主なリンク修正内容

※ 左が「これまでのリンク先」、右が「修正後の正しいアンカー」。

#### Active Record 関連
- `active_record_validations.md`  
  - `association_basics#the-has-one-association` → `association_basics#has-one`（×2 箇所）  
  - `association_basics#the-has-many-association` → `association_basics#has-many`（×2 箇所）
- `active_record_callbacks.md`  
  - `active_record_querying#dynamic-finders` → `active_record_querying#dynamic-finder-methods`
- `active_record_basics.md`  
  - `testing#the-low-down-on-fixtures` → `testing#fixtures`

#### キャッシュ関連
- `caching_with_rails.md`  
  - `association_basics#single-table-inheritance` → `association_basics#single-table-inheritance-sti`  
    （`form_helpers.md` ですでに `-sti` 付きが使われており、それに合わせた統一）

#### 設定・autoload
- `configuring.md`  
  - `autoloading…#autoload-paths` → `autoloading…#config-autoload-paths`  
    （`config.autoload_paths` 用のセクション ID に合わせる）

#### Active Storage / Action Text / メール
- `contributing_to_ruby_on_rails.md`  
  - `active_storage_overview#analyzing-files` → `active_storage_overview#analyzing-files-for-metadata`
- `action_text_overview.md`  
  - `active_storage_overview#requirements` → `active_storage_overview#third-party-software`（×2 箇所）  
  - `active_storage_overview#purging-unattached-uploads` →  
    `active_storage_overview#purging-unattached-uploads-and-detach`
- `action_mailer_basics.md`  
  - `i18n#translations-for-action-mailer-e-mail-subjects` → `i18n#action-mailer-e-mail-subjects`

#### Engines / Routing など
- `engines.md`  
  - `engines#the-inheritance-hierarchy` → `engines#inheritance-hierarchy`
- `getting_started_with_devcontainer.md`  
  - `getting_started#creating-a-new-rails-project` → `getting_started#creating-a-new-rails-app`
- `rails_on_rack.md` / `security.md`  
  - `caching_with_rails#conditional-get-support` → `caching_with_rails#conditional-gets`
- `routing.md`  
  - `command_line#the-app-and-helper-objects` → `command_line#the-app-object`  
    （リンク元テキストが「via the app object」のため、それに合わせる）
- `testing.md`  
  - `active_job_basics#create-the-job` → `active_job_basics#defining-a-job`

### 間違ったコピー＆ペースト由来の修正

- `rails_on_rack.md`  
  `config.action_dispatch.x_sendfile_header` のリファレンスリンクが、誤って  
  `#config-action-record-migration-error` を指していたのを、同じページ内の正しい兄弟リンクに合わせて  
  `#config-action-dispatch-x-sendfile-header` に修正。

※ 疑似例（実際の行番号などは省略）:

```md
# 修正前（誤リンク）
See [config.action_dispatch.x_sendfile_header](configuration.html#config-action-record-migration-error) for details.

# 修正後（正しいリンク）
See [config.action_dispatch.x_sendfile_header](configuration.html#config-action-dispatch-x-sendfile-header) for details.
```
...
- 対象は Rails Guides 内部のドキュメントリンクのみで、フレームワークの動作や公開 API には一切影響しません。
- ガイド閲覧時に、これまでクリックしても該当セクションまでスクロールしなかったリンクが、正しく該当箇所へジャンプするようになります。
- まだ残っている壊れたリンクは、以下のように「ターゲットのセクション自体が削除・移動されているもの」であり、この PR ではあえて直していません。リンクの意図（どのセクションへ飛ばすべきか）の判断が必要なためです。
  ...
  これらは別途、該当ガイドの著者やメンテナと相談しつつ、  
  - リンク先を別の現存セクションに変える  
  - あるいはリンク自体を削除する  
  といった判断が必要になります。

- ドキュメントの ID 変更やセクション名変更を今後行う場合も、同様の整合性チェックを CI などに組み込むと、今回のようなリンク切れを未然に防げます。

---

4. 参考情報 (あれば)  

- 該当 PR: https://github.com/rails/rails/pull/57806  
- Rails Guides: https://guides.rubyonrails.org/  
  - HTML 生成後の `id` と Markdown 内の `#anchor` を突き合わせる手法は、ドキュメントのリンク健全性チェックとして汎用的に使えるので、自前ドキュメントや社内ガイドにも応用可能です。

---


## [#57711](https://github.com/rails/rails/pull/57711) Fix `create_or_find_by` scope pollution on the non-transactional retry {#pr-57711}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`create_or_find_by` / `create_or_find_by!` が一意制約違反 (`RecordNotUnique`) でリトライする際、トランザクション外でのみ「スコープ汚染」により既存レコードを正しく取得できないバグが残っていたのを修正したPRです。あわせて、そのトランザクション外の挙動をカバーする新しいテストが追加されています。
...
`create_or_find_by` は、内部で INSERT を試みて `RecordNotUnique` が発生した場合、同じ属性で SELECT をやり直すことで「既に存在していたレコードを返す」挙動をとります。

以前の修正 (#57192) では、トランザクション内の場合のみ、次のようにスコープをリセットして再検索するように変えていました:
...
```ruby
  ...
  if connection.transaction_open?
    ...
    find_by!(attributes)   # ここは未修正で残っていた
  end
end
```

- トランザクション内 (`transaction_open? == true`):  
  `rewhere(attributes).lock.take!`  
  → もともとのスコープ条件を捨てて、`attributes` だけで検索する

- トランザクション外 (`else` ブランチ):  
  `find_by!(attributes)`  
  → 呼び出し元のスコープを維持したまま `attributes` を追加する

このため、呼び出し元でスコープを張っている場合にバグが生じていました。

```ruby
Subscriber.create!(nick: "bob")

# ここで where(nick: "alice") というスコープがついた relation で create_or_find_by
Subscriber.where(nick: "alice").create_or_find_by(nick: "bob")
# => ActiveRecord::RecordNotFound
# 実際のクエリ: WHERE "subscribers"."nick" = 'alice' AND "subscribers"."nick" = 'bob'
```

1. INSERT が一意制約に引っかかって `RecordNotUnique`
2. rescue 節で `find_by!(attributes)` を実行
3. ここでスコープ `nick = 'alice'` と `attributes` の `nick = 'bob'` が両方付く
4. 実レコードは `nick = 'bob'` なのでマッチせず、`RecordNotFound` になる

これは、#57192 が解決しようとしていた「スコープ汚染」と同じ問題ですが、トランザクション外の分岐だけ未対応だった、という位置づけです。

### 修正内容

今回のPRでは、トランザクション外の分岐もトランザクション内と同様、「元スコープを捨てて `attributes` のみで再検索」するように揃えています。ただし、トランザクション外なのでロックはかけません。
...
- `find_by!(attributes)` → `rewhere(attributes).take!`
  - `rewhere` により、これまで relation に付いていた `WHERE` 条件を捨てて、`attributes` の条件だけを付与
  - `take!` で 1件取得。見つからなければ `RecordNotFound` を投げる動作は維持
- `.lock` はトランザクション内でのみ意味があるため、トランザクション外ブランチでは付けていない

これにより、トランザクションの有無にかかわらず、`create_or_find_by` の「既存レコードを取りに行くとき」は常に「スコープではなく属性で検索する」という一貫した挙動になります。

### テストの追加

既存のテストスイートは「トランザクションフィクスチャ」を使っており、各テストはトランザクション内で実行されるため、`connection.transaction_open?` が常に true になり、問題の `else` ブランチが一切通っていませんでした。

このPRでは、トランザクション外の挙動を検証する専用テストファイルを追加しています:

- 追加ファイル: `activerecord/test/cases/create_or_find_by_outside_transaction_test.rb`
- 中身の特徴:
  - `self.use_transactional_tests = false` を設定し、テストをトランザクション外で実行
  - スコープ付きの relation から `create_or_find_by` / `create_or_find_by!` を呼び出し、
    - 一意制約により INSERT が失敗 → 再検索
    - 再検索時に、スコープが汚染されず既存レコードを正しく返すことをアサート
  - 修正前だと `RecordNotFound`（重複した `nick` 条件）でテストがレッドになることを確認済み
...
- 対象メソッド:
  ...
  - `ActiveRecord::Relation#create_or_find_by!`
- 影響条件:
  - 一意制約 (`UNIQUE` index 等) によって `RecordNotUnique` が発生しうるカラムを対象に `create_or_find_by` / `create_or_find_by!` を使用している
  - かつ、スコープを張った relation（`Model.where(...).create_or_find_by(...)` など）で呼び出している
  - かつ、周囲にトランザクションを張っていない利用ケース（コントローラから直接呼ぶなど）が存在する

この条件を満たす場合、**これまで「たまたま `RecordNotFound` が出ていて気づいていなかったバグ」**が、今回の修正で意図どおり「既存レコードを返す」挙動になります。

注意点:

- スコープに依存して動作していた（と期待していた）コードがあれば、挙動が変わります:
  - 以前:  
    スコープ + attributes の両方で検索するため、スコープで除外されているレコードは `RecordNotFound`
  - 今回以降:  
    スコープを無視して attributes のみで検索し、既存レコードを返す
- ただし `create_or_find_by` の設計意図は「指定 attributes を持つレコードを一意に取得する（なければ作る）」であり、**スコープ条件を再検索に持ち込まない方が自然かつ一貫性がある**ため、この変更はバグ修正として妥当です。

移行時に確認しておくとよいこと:

- もし `create_or_find_by` / `create_or_find_by!` を使っていて
  - 意図的に「トランザクション外」「スコープ付き」の組み合わせで挙動を変えていた場合は、その設計を見直す必要があります。
  - 「スコープに合致しないものはエラーにしたい」のであれば、`create_or_find_by` ではなく、明示的に `find_or_create_by` + バリデーション or 条件チェックなどの別のパターンを検討した方がよいです。

---

4. 参考情報 (あれば)  

- 元バグ修正 PR: [#57192](https://github.com/rails/rails/pull/57192)  
  - トランザクション内の `create_or_find_by` のスコープ汚染を修正したが、トランザクション外は取りこぼしていた
- 今回の PR: [#57711 - Fix `create_or_find_by` scope pollution on the non-transactional retry](https://github.com/rails/rails/pull/57711)  
  - トランザクション外のブランチを `rewhere(attributes).take!` に揃え、テストを追加してカバレッジを確保

---


## [#57782](https://github.com/rails/rails/pull/57782) Avoid mutating params hash passed to ActionDispatch::Http::URL.url_for {#pr-57782}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActionDispatch::Http::URL.url_for` が引数として受け取った `params:` ハッシュを破壊的に変更していた問題を修正し、呼び出し元のハッシュを変更しないようにした PR です。これにより、`request.query_parameters` などのメモ化ハッシュが意図せず書き換えられる不具合や、凍結されたハッシュでの `FrozenError` が解消されます。
...
これまで `ActionDispatch::Http::URL.url_for` の内部では、クエリパラメータを付与する `add_params` メソッドで、値が `nil` 扱いとなるキーを除外する際に破壊的メソッド `reject!` を使っていました。

```ruby
def add_params(path, params)
  params = { params: params } unless params.is_a?(Hash)
  params.reject! { |_, v| v.to_param.nil? }   # ここで呼び出し元の Hash が書き換わる
...
1. **`request.query_parameters` が汚染される**  
   - `OptionRedirect#path` では次のように、`request.query_parameters` をそのまま `params:` に渡しています:
     ...
     url_options = {
       ...
       path:     request.path,
       params:   request.query_parameters   # メモ化されている同一オブジェクト
     }.merge! options
     ```

   - ここで `?foo` のような「値なしクエリ」(パースすると `{"foo" => nil}`) があると、`reject!` によってこのキーが削除されます。
   - `request.query_parameters` は per-request にメモ化されたハッシュなので、一度削除されるとその後の処理からは **常にキーが存在しない状態** に見えてしまいます。

2. **凍結された `params:` を渡すと例外になる**  
   - 呼び出し側が `params` を `freeze` して不変オブジェクトとして扱っていると:

     ```ruby
     ActionDispatch::Http::URL.url_for(
       only_path: true,
       path: "/x",
       params: { a: 1, b: nil }.freeze
     )
     ```

   - 旧実装では `reject!` により `FrozenError: can't modify frozen Hash` が発生していました。
   - 期待される挙動は、`b` を無視して `"/x?a=1"` を返すことです。

### 修正内容

`reject!` を非破壊的な `reject` に置き換え、フィルタ済みハッシュをローカル変数に再代入する形に変更しています。

```ruby
def add_params(path, params)
  params = { params: params } unless params.is_a?(Hash)
  params = params.reject { |_, v| v.to_param.nil? }  # 非破壊で新しい Hash を作る
  query = params.to_query
  path << "?#{query}" unless query.empty?
end
```
...
- もともと `reject!` の戻り値はどこにも使われておらず、その後も `params` 自体を参照していたため、「URL の出力結果」はこの変更で変わりません。
- 変わるのは **「呼び出し元の `params` が書き換えられない」** という点だけです。
- これにより `FrozenError` も解消されます (`reject` は新しい Hash を返すだけなので元の Hash を変更しない)。

### テストの追加

`actionpack/test/dispatch/request_test.rb` に回帰テストが追加されています。おおよそ次を確認する内容です:

1. **渡した `params` が変更されないこと**

   ```ruby
   params = { a: 1, b: nil }
   ActionDispatch::Http::URL.url_for(only_path: true, path: "/x", params: params)
   assert_equal({ a: 1, b: nil }, params)
   ```

2. **凍結された `params` でも URL が生成できること**

   ```ruby
   params = { a: 1, b: nil }.freeze
   url = ActionDispatch::Http::URL.url_for(only_path: true, path: "/x", params: params)
   assert_equal "/x?a=1", url
   ```
...
- **主な影響対象**:
  - `ActionDispatch::Http::URL.url_for` を直接使っているコード。
  - あるいはそれを内部的に利用するコンポーネント（`OptionRedirect` など）、およびそれ経由で URL を組み立てる処理全般。

- **挙動の変化**:
  - URL 文字列としての出力は、これまでと同じになる想定です（値が `to_param.nil?` なキーは引き続きクエリに含まれない）。
  - 変わるのは「渡した `params` ハッシュのライフサイクル」で、  
    - 以前: `nil` 相当の値を持つキーが **破壊的に削除される**  
    - 今回以降: 渡したハッシュは **そのまま保持される**

- **バグ解消の具体例**:
  - これまで `request.query_parameters` を再利用している処理で、「さっきまであったキーが消えている」「一部のクエリが見えなくなっている」といった現象が出ていた場合、この変更で解消する可能性が高いです。
  - `url_for` に凍結済みのオプションハッシュや、アプリ側で明示的に不変として扱っている設定ハッシュを流しても安全になります。

- **互換性**:
  - アプリケーションコード側で、**あえて** `url_for` に破壊的変更を期待していたケース（＝`params` が削られることを前提にしていたコード）があると理論上は挙動が変わります。
  - ただし、URL ビルダにそんな前提を置くのは通常想定外であり、PR でも「URL ビルダが入力を変更すべきでない」という方針が明示されています。実用上は後方互換性に問題はほぼないと考えてよいです。

---

4. 参考情報 (あれば)  

- この PR で指摘されている問題は、過去の #23103 と同じ「Rails 内部が per-request にメモ化されたハッシュを破壊的に変更してしまう」クラスのバグです。
- 関連クラス / メソッド:
  - `ActionDispatch::Http::URL.url_for`
  - `ActionDispatch::Request#query_parameters`
  - `ActionDispatch::Routing::RouteSet::Generator`（過去の類似バグの修正対象）

---


## [#57720](https://github.com/rails/rails/pull/57720) Don't share the actions Hash across `ActionableError` subclasses {#pr-57720}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveSupport::ActionableError` でアクションを管理するクラス変数 `_actions` がサブクラス間で共有されてしまい、定義していないアクションが親・兄弟クラスに「漏れる」バグを、コピーオンライトによる再代入で修正した PR です。これにより、各 `ActionableError` サブクラスごとにアクション定義が正しく分離されます。
...
`class_attribute` の `default: {}` は 1 つの同じ Hash オブジェクトをデフォルトとして使うため、

- 親クラス `BaseError < ActionableError`
- サブクラス `ErrorA < BaseError`
- サブクラス `ErrorB < BaseError`

という構成のときに、`ErrorA.action(:do_a) { ... }` や `ErrorB.action(:do_b) { ... }` を呼ぶと、**親とサブクラスで同じ `_actions` Hash オブジェクトが共有**されます。

その結果:

- `ErrorA` で定義したアクションが `BaseError` や `ErrorB` からも見えてしまう
- `ActionableError.actions(error)` や `dispatch` が、本来そのクラスで定義していないアクションまで返す/実行できてしまう

というバグが発生します。

### 修正内容

`action` メソッドをミューテーションではなく「コピーオンライト」で再代入するように変更:
...
- `merge` によって **既存の `_actions` Hash から新しい Hash を生成**し、その新しい Hash を `self._actions` に代入します。
- 親クラスから継承した `_actions` の内容は `merge` のベースになるので、**継承自体は保持**されます（親で定義したアクションは子から見える）。
- ただし、新しい Hash を自クラスに再代入することで、**その後の変更はそのクラス専用の Hash 上で行われる**ため、親や兄弟クラスへの「漏れ」がなくなります。

これは最近の `StructuredEventSubscriber#debug_only` (#57676) の修正や、他の `class_attribute` の再代入パターンと同じ方針です。

### テスト追加

`activesupport/test/actionable_error_test.rb` に以下のような構造のテストが追加されています（概念的なイメージ）:

```ruby
class BaseError < ActiveSupport::ActionableError
end

class ErrorA < BaseError
  action(:a) { "A" }
end

class ErrorB < BaseError
  action(:b) { "B" }
end
...
変更前は、ベースクラスに `["A", "B"]` が見えてしまいテストが失敗（red）していましたが、修正後は各クラスが自分のアクションだけを見るようになりテストが成功（green）することが確認されています。

---

3. 影響範囲・注意点  

- 対象: `ActiveSupport::ActionableError` を継承し、`action` DSL を使っているアプリケーションやライブラリ。
- 期待される挙動としては「**あるサブクラスで定義したアクションが、他のサブクラスや親クラスには見えない**」はずなので、その意味ではバグフィックスであり、破壊的変更というより**正しいスコープに戻す修正**です。
- もしこれまで、
  - 親クラスでアクションを定義していないのに、
  - なぜかサブクラスのアクションが見えていて、
  - それに依存した実装やテストを書いていた
  という場合は、今回の修正でその挙動が**是正されて見えなくなる**ため注意が必要です。
- アクションを親クラスで共有したい場合は、**親クラスに明示的に `action` を定義**し、子では必要な追加分だけ `action` を定義する構成にするのが正しい使い方になります。
...
- 該当ファイル:
  ...
  - `activesupport/test/actionable_error_test.rb`
- 関連 PR:
  - `StructuredEventSubscriber#debug_only` の同種のバグ修正: PR #57676  
- 背景となる仕組み:
  - `class_attribute` の `default:` にミュータブルなオブジェクト（`{}`, `[]` など）を渡すと、**同一オブジェクトが継承チェーンで共有される**ため、今回のような「サブクラス間での状態共有バグ」が起こりやすい、という一般的な注意点があります。

---


## [#57792](https://github.com/rails/rails/pull/57792) Avoid mutating the override options passed to `ActiveModel::Errors#import` {#pr-57792}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveModel::Errors#import` が引数に渡された `override_options` ハッシュを書き換えてしまう問題と、凍結されたハッシュを渡すと `FrozenError` が発生する問題を修正する PRです。`override_options` を `dup` してから正規化することで、呼び出し元のハッシュを一切変更しないようにしています。
...
`ActiveModel::Errors#import` は、`attribute` / `type` のオプションをシンボル化する際に、渡されたハッシュ自体を書き換えていました。
...
def import(error, override_options = {})
  [:attribute, :type].each do |key|
    if override_options.key?(key)
      override_options[key] = override_options[key].to_sym # 呼び出し元の hash を破壊的変更
    end
  end
...
- 同じ `options` ハッシュを複数回使い回すと、**最初の呼び出しで文字列がシンボルに書き換えられてしまう**。
- 凍結されたハッシュ（`{}.freeze`）を渡すと、代入時に **`FrozenError` が発生**する。
...
options
# 実際:   { attribute: :age, type: :blank }
# 期待:   { attribute: "age", type: "blank" }

errors.import(error, { attribute: "age" }.freeze)
# 実際:   FrozenError: can't modify frozen Hash
# 期待:   attribute: :age としてエラーがインポートされる
```

`import` は public API であり、他モデルからエラーをコピーする公式な手段としてドキュメント化されているため、呼び出し元が所有するハッシュを変更すべきではありません。

### 修正内容

`override_options` を正規化する前に `dup` して、メソッド内部専用のコピーを使うようにしました。

修正後（要点のみ）:

```ruby
def import(error, override_options = {})
  override_options = override_options.dup  # ← ここを追加
  [:attribute, :type].each do |key|
    if override_options.key?(key)
...
- `dup` は浅いコピーなので、トップレベルの `:attribute` / `:type` の値を書き換える分には十分。
- `dup` したハッシュは凍結されていないため、`FrozenError` も解消される。
- 呼び出し元のハッシュは一切変更されない。

### テスト追加

`activemodel/test/cases/errors_test.rb` に以下のような回帰テストが追加されています（概念的には）:

1. `import` 呼び出し後に、渡した `options` ハッシュが変更されていないこと。
2. 凍結された `options` ハッシュを渡しても `FrozenError` が出ず、エラーが正しくインポートされること。
...
- **既存コードの挙動変化**:
  - これまで「`import` を呼ぶと `options` の `attribute` / `type` がシンボルに変わる」ことに依存していたコードがあれば、そこは挙動が変わります（ただし、その依存は想定外のものです）。
- **Frozen ハッシュの利用**:
  - 今後は `errors.import(error, SOME_OPTIONS.freeze)` のような呼び出しが安全に行えます。
- **パフォーマンス**:
  - 毎回 `override_options.dup` が走るため、理論上はごく小さなオーバーヘッドが増えますが、通常のフォームバリデーション等のユースケースでは無視できるレベルと考えられます。
- **API 安全性の向上**:
  - Rails 内部が「所有していないハッシュを破壊的に変更しない」という原則に沿う改修であり、`merge!` → `import` の内部呼び出しだけでなく、アプリケーションコードからの直接利用もより安全になります。
...
- 類似のバグとして挙げられている PR:
  - #57782: `ActionDispatch::Http::URL.url_for` が渡された `params:` ハッシュを破壊的変更していた問題
  - #23103: Rails 内部が渡されたハッシュを破壊的に変更していた別件のバグ
- 対象クラス: `ActiveModel::Errors`
- 対象メソッド: `import(error, override_options = {})`

---


## [#57802](https://github.com/rails/rails/pull/57802) Fix typos in generated app config templates {#pr-57802}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails が新規アプリ生成時に使う設定ファイルテンプレート内のコメントにあった英単語のスペルミスを修正する PR です。実行ロジックや挙動の変更はなく、生成される設定ファイルのコメント表記のみがきれいになります。

---

2. 変更内容の詳細  

対象は Rails アプリ生成テンプレート (`rails new` 時に使われる `.tt` ファイル) のコメント部分です。

### 修正された箇所

1. `config/environments/production.rb.tt`

```diff
- # Ensures that a master key has been made available in either ENV["RAILS_MASTER_KEY"] or in config/master.key. This key is used for decrypting credentials (and other encrypted files).
- # config.require_master_key = true # Uncomment to enforce credentials assigment.
+ # Ensures that a master key has been made available in either ENV["RAILS_MASTER_KEY"] or in config/master.key. This key is used for decrypting credentials (and other encrypted files).
+ # config.require_master_key = true # Uncomment to enforce credentials assignment.
```

スペルミス: `assigment` → `assignment`  
※ 実際の行はコメントのみの修正で、意味は「credentials の割り当てを強制するためにコメントアウトを外す」という説明です。

2. `config/environments/test.rb.tt`

```diff
- # Raise error for missing translations.
- # config.i18n.raise_on_missing_translations = true # Useful for catching translation assigment issues in tests.
+ # Raise error for missing translations.
+ # config.i18n.raise_on_missing_translations = true # Useful for catching translation assignment issues in tests.
```

スペルミス: `assigment` → `assignment`  
テスト環境で訳抜けを検出する設定に関するコメントの修正です。

3. `config/storage.yml.tt`

```diff
- # Use rails credentials:edit to set the AWS secrets (as aws:access_key_id and aws:secret_access_key)
- # and then checkin this file to your source control.
+ # Use rails credentials:edit to set the AWS secrets (as aws:access_key_id and aws:secret_access_key)
+ # and then check in this file to your source control.
```

`checkin` → `check in`（動詞として正しい表記に修正）  
意味としては「このファイルをバージョン管理にコミット（チェックイン）してください」という説明のままです。

※ PR 説明にもある通り、いずれも「コメント」「文言」だけの修正で、コードの実行パスや設定値そのものは一切変更されていません。
...
- 影響範囲:
  - この PR マージ以降に `rails new` で生成されるアプリの
    - `config/environments/production.rb`
    - `config/environments/test.rb`
    - `config/storage.yml`
    に含まれるコメント文言だけが修正されます。
  - 既存アプリには自動反映されません（`rails app:update` でも、コメント差分は基本的に手動マージ対象です）。

- 実行時挙動:
  - 設定値やメソッド呼び出しには一切変更なし。
  - テストもこれらの文字列に依存していないため、CI・本番挙動ともに影響はありません。

- 注意点:
  - 英文コメントをドキュメントや社内資料などにそのまま引用している場合、`assigment` の誤字が紛れ込んでいる可能性があります。必要ならコピペ元をこの PR に合わせて修正するとよいです。
  - `checkin` → `check in` は表記ゆれの修正で意味は同じなので、既存プロジェクトの `storage.yml` をわざわざ直す必要はありません（気になる場合のみ修正する程度で十分です）。

---

4. 参考情報 (あれば)  

- 対象 PR: https://github.com/rails/rails/pull/57802  
- 修正対象ファイル:
...
## [#57799](https://github.com/rails/rails/pull/57799) Fix grammar in guides {#pr-57799}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
このPRは、Rails公式ガイド内の明らかな英語の文法ミスを修正したものです。コードや挙動の変更は一切なく、ガイド文書の可読性・正確性を高めるドキュメント専用の修正です。

---

2. 変更内容の詳細  

PRの方針は「誰が見ても誤りと言えるレベルの文法ミスのみを修正する」もので、対象は以下の3カテゴリです。

### (1) 主語と動詞の一致 (Subject-verb agreement)

**active_record_validations.md**  
- `save` and `update` returns → **return**  
  複数主語に合わせて動詞を複数形に修正。

**getting_started.md**  
- parameters ... is → **are**  
  “parameters” に合わせて “are” に修正。

**routing.md**  
- method and the lambda gets → **get**  
  “method and the lambda” は複数なので “get” に修正。
- multiple routing files make → **makes**  
  こちらは逆に、文の主語が「configuration」など単数扱いになる文脈と合うように “makes” に修正されたと考えられます（3358行の記述と整合を取る意図）。

**active_storage_overview.md**  
- validations ... isn't → **aren't**  
  “validations” が複数形なので “aren’t” に修正。

**active_support_core_extensions.md**  
- The method X and Y → **The methods**  
  :3358行と表現を揃え、「2つのメソッド」を指すことが明確になるように修正。

### (2) 冠詞 / it's / 比較 (Articles / it’s / than)

**getting_started.md**  
- it's inventory → **its** inventory  
  所有を表す “its” に修正（it is ではない）。

**active_record_querying.md**  
- greater that 1 → greater **than** 1  
  比較で用いる “than” に修正。

**active_storage_overview.md**  
- an URL → **a** URL  
  “URL” の発音が “yoo-” で始まるため “a URL” が正しい。

**testing.md**  
- is AJAX request → is **an** AJAX request  
  “AJAX” が母音音から始まるため “an” を付与。

### (3) 語形・表現の修正 (Word form / phrasing)

**active_storage_overview.md**  
- How to setup → How to **set up**  
  動詞句 “set up” に修正（“setup” は名詞／形容詞）。
- a serviced called → a **service** called  
  誤った過去分詞 “serviced” を名詞 “service” に修正。

**action_view_overview.md**  
- a basic examples → a basic **example**  
  単数・複数の整合を取る修正。
- counter act → **counteract**  
  1語の動詞に修正。

**testing.md**  
- rather at → rather **than** at  
  比較構文 “rather than” に修正。

**routing.md**  
- "Alternatively, You" → "Alternatively, **you**"  
  文中の不要な大文字を修正。
- "used instead to `to:`" → used **instead of** `to:`  
  前置詞の用法を正しい形 “instead of” に修正。
- ", The ad URLs" → **the** ad URLs  
  不要なカンマと大文字をなくし自然な文に。
...
- **ランタイムやAPIには一切影響なし**  
  変更は `guides/source/*.md` のみで、フレームワーク本体やテストコードは触られていません。ビルド結果のHTMLガイドにだけ反映されます。
- **既存コード／アプリケーションへの影響なし**  
  仕様変更や挙動変更ではないため、アプリケーションのアップグレード時に追従が必要な点はありません。
- **ガイドの検索性・理解度が向上**  
  誤った表現に引きずられて実装例を誤解するリスクがわずかに減ります。特に英語学習中の開発者や新人向け教材として使う場合に読みやすくなります。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57799  
- 類似の「文書のみ修正」PRは、Railsリリースノートでも通常は互換性のない変更としては扱われません。  
- ドキュメントに独自翻訳や社内Wikiを運用している場合、このPR以降の原文との差分として取り込んでおくと、後々のマージ作業が楽になります。

---


## [#57798](https://github.com/rails/rails/pull/57798) Fix indefinite article in error messages {#pr-57798}

**マージ日**: 2026/6/21 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
英語メッセージ内の不定冠詞 ("a"/"an") の誤りを修正した、極めて小さな文言調整の PR です。挙動や API 仕様には変更はなく、ユーザー向けエラーメッセージ／ログの英語表現のみが改善されています。

---

2. 変更内容の詳細  

### 2-1. Active Storage (Vips 画像アナライザ)

対象ファイル:  
- `activestorage/lib/active_storage/analyzer/image_analyzer/vips.rb`

変更点:
- エラーメッセージの文言を `an Vips error` → `a Vips error` に修正。

「Vips」は発音的に頭が子音音（/v.../）なので、本来は "a Vips error" が正しい英語表現です。  
同ファイル内の近接した rescue 節ではすでに `a Vips error` が使われていたため、それに揃える形の文言統一です。

この修正により、例えば以下のようなログ／エラーメッセージが出力される場合の英語が自然になります:

```text
Failed to analyze image: a Vips error occurred
```

(実際の文全体はコード側に依存しますが、「a Vips error」という部分が修正箇所です。)

---

### 2-2. Active Record Migration (CommandRecorder)

対象ファイル:  
- `activerecord/lib/active_record/migration/command_recorder.rb`

`CommandRecorder` はマイグレーション操作を記録して逆操作を生成するための内部コンポーネントで、ここに含まれるユーザー向けメッセージの不定冠詞が修正されています。

変更点:
- `an using_index` → `a using_index`  
  - "using" は発音として /juːzɪŋ/ で、先頭が子音音 (/j/) のため "a using_index" が正しい。
- `an column_name` → `a column_name`  
  - "column" の先頭も子音音 (/k/) なので "a column_name" が正しい。

コード上は、おそらくマイグレーションコマンドの記録・再生時に、未知／未サポートのオプションや引数に関して例外を投げる文言などが以下のような形で修正されています:

```ruby
# 例 (イメージ)
raise ArgumentError, "Unknown keyword: a using_index"
raise ArgumentError, "Unknown keyword: a column_name"
```

同じ付近のコードには `an expression` といった表現があり、こちらは母音音ではじまる "expression" に対して "an" を使うのが正しいため、そのスタイルと整合しています。
...
- **実行時の挙動／ロジックへの影響はなし**  
  - 変更は文字列リテラル（エラーメッセージ／ログメッセージ）のみで、メソッドシグネチャやクラス構造、アルゴリズムには一切影響しません。
- **外部インターフェイスとしての影響は極小**  
  - Rails のエラーメッセージ文言をパターンマッチ（文字列完全一致）でテストしている場合のみ、テストが落ちる可能性があります。
  - その場合は "an using_index" → "a using_index" などに期待値を修正する必要があります。
- **国際化 (i18n) への影響はなし**  
  - 変更対象は英語メッセージであり、i18n のキーや翻訳ファイルには影響しません。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57798  
- 修正対象:
  - Active Storage の Vips アナライザでのエラーログ文言
  - Active Record マイグレーション `CommandRecorder` 内のエラー／ログ文言  
- 文法上のポイント: "a"/"an" は綴りではなく「発音（先頭の音が母音か子音か）」に依存する、という典型的な英語文法の修正です。

---


## [#57797](https://github.com/rails/rails/pull/57797) Fix :singleton-method: directive typo for protected_environments [ci skip] {#pr-57797}

**マージ日**: 2026/6/21 | **作成者**: [@fuentesjr](https://github.com/fuentesjr)

1. 概要 (1-2文で)  
ActiveRecordクラスメソッド `protected_environments` のRDoc用ディレクティブのスペルミスを修正し、APIドキュメントに正しく表示されるようにしたPRです。実行時の挙動や公開APIのインターフェイス自体には変更はなく、ドキュメント生成のみが対象です。

---

2. 変更内容の詳細  

- 対象ファイル: `activerecord/lib/active_record.rb`
- 変更点は1行のみで、RDocディレクティブのスペル修正です。

元のコードイメージ（誤り）:
```ruby
# :singlton-method:
# Returns the list of protected environments ...
...
- `ActiveRecord.protected_environments` は `singleton_class.attr_reader` によって定義されています。
- RDoc は通常の `def self.method_name` 形式でないメソッドを自動では拾えないため、  
  「このコメントはクラスメソッドに対するものだ」と教えるために `# :singleton-method:` ディレクティブを使います。
- これが `:singlton-method:` という誤記になっていたため、
  - RDocがディレクティブとして認識できず無視
  - コメントは単なる「ソース一覧中のテキスト」として扱われる
  - `protected_environments` メソッド自体がAPIドキュメントに現れない
 という状態になっていました。
- 同じパターンで定義されている `lazily_load_schema_cache` および `schema_cache_ignored_tables` はすでに正しい `# :singleton-method:` を使っており、今回それに合わせて整合性を取った形です。

結果として、edge APIドキュメント上では `protected_environments=` （セッター）だけがクラスメソッド一覧に出ていて、`protected_environments`（ゲッター）が欠落しているという不自然な状態が、今回の修正により解消されます。
...
- ランタイムへの影響:
  - 実行時のコードは一切変わっておらず、挙動・パフォーマンス・互換性への影響はありません。
- 影響範囲:
  - RDoc/edge APIドキュメント生成のみが対象です。
  - `ActiveRecord.protected_environments` を利用しているアプリケーションのコードには影響はありません。
- 注意点:
  - 「今まで `protected_environments` の存在がドキュメント上見えなかった」ことに依存しているようなケース（例えば、ツールがAPIドキュメントをパースしてメソッド一覧を作っている場合）では、新たに1メソッドが増えたように見える可能性がありますが、実際のAPIとしてはもともと存在していたものです。
  - Railsのソースコメントをベースに独自にRDocを生成している場合は、この修正を取り込むとドキュメントに同メソッドが現れるようになります。

---

4. 参考情報 (あれば)  

- 該当メソッド（edge API・修正前はゲッターが欠落）  
  - `protected_environments=`:  
    https://edgeapi.rubyonrails.org/classes/ActiveRecord.html#method-c-protected_environments-3D
- 類似のRDocディレクティブ:
  - `# :method:` / `# :singleton-method:` は、RDocに「これから書くコメントはインスタンスメソッド／クラスメソッド用だ」と伝えるためのマーカーです。
- RDocドキュメント（ディレクティブ仕様の参考）:  
  https://ruby.github.io/rdoc/ (公式RDocリファレンス)

---


## [#57793](https://github.com/rails/rails/pull/57793) Fix typos in Rails on Rack guide {#pr-57793}

**マージ日**: 2026/6/20 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Railsガイド「Rails on Rack」の文書中にあるスペルミスを4箇所修正したドキュメント専用のPRです。アプリケーションコードや挙動には一切影響せず、ガイドの可読性・正確性のみを向上させています。
...
- ミドルウェアのコメント中
  ...
  ```ruby
  # Before
  # Propgate the request to the next middleware
  # Propogate the response back to the client

  # After
  # Propagate the request to the next middleware
  # Propagate the response back to the client
  ```

- ガイド本文中
  - `"reponse"` → `"response"`
  - `"faciliate"` → `"facilitate"` (`Rack::ETag` に関する説明部分)

  ETag に関する箇所のニュアンスとしては、「レスポンスのキャッシングや conditional GET を facilitate（容易にする）」といった説明文の中の綴りを修正しているものと考えられます。

いずれも意味を変える変更ではなく、英単語の正しい綴りに合わせるだけの修正です。

3. 影響範囲・注意点  
- 影響範囲  
  - Rails本体のコード、Rackミドルウェアの実装、インターフェース等には一切変更なし。
  - 影響するのは「Rails on Rack」ガイド（英語版）の表示のみです。
- 注意点  
  - ドキュメントの更新に伴い、ガイドを引用している外部資料・ブログなどで古いスペルがそのまま残る可能性がありますが、機能上の問題はありません。
  - Railsの挙動やRackミドルウェアの書き方が変わったわけではないため、既存コードの修正は不要です。

4. 参考情報 (あれば)  
- 該当ガイド: Guides > Rails on Rack  
  https://guides.rubyonrails.org/rails_on_rack.html  
- `Rack::ETag` の公式ドキュメント（Rack側）:  
  https://www.rubydoc.info/gems/rack/Rack/ETag

---


## [#57794](https://github.com/rails/rails/pull/57794) Fix grammar in Model.inspect schema-not-loaded message {#pr-57794}

**マージ日**: 2026/6/20 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
ActiveRecord の `Model.inspect` がスキーマ未ロード時に返すメッセージ中の英語の文法ミス（"schema informations"）を、正しい "schema information" に修正した PR です。  
実装コードとそれに対応するテストのユーザ向け文字列のみが変更されています。
...
ActiveRecord のモデルクラスを、接続やスキーマがまだロードされていない状態で `inspect` すると、従来は次のようなメッセージが返されていました（概略）:

```ruby
# 修正前（イメージ）
User.inspect
# => "User(id: integer, ... schema informations not loaded)"
```

この "schema informations" という文言のうち、文法的に誤っていた複数形 "informations" を、不可算名詞である "information" に修正しました。

```ruby
# 修正後（イメージ）
User.inspect
# => "User(id: integer, ... schema information not loaded)"
```

※ 実際の文字列は Rails の実装に依存しますが、意図としては「スキーマ情報がロードされていない」というヒントメッセージ部分の語を修正しただけです。

### 変更ファイル

- `activerecord/lib/active_record/core.rb`
  - `inspect` がスキーマ未ロード時に返す文言を `"schema informations"` → `"schema information"` に1文字だけ修正。
- `activerecord/test/cases/invalid_connection_test.rb`
  - 上記文言を前提としているテストの期待値も、同じく `"informations"` → `"information"` に修正。

### PR本文で触れられているポイント

- "information" は英語で不可算名詞であり、複数形 "-s" をとらないため "informations" は誤りである。
- 既にコードベースの他の箇所では "schema information" と単数形で使われており、この箇所だけが例外だった。
- Merriam-Webster / Oxford いずれも不可算として扱っていることを根拠にしている。
- プロダクションコードのユーザ向け文字列とテストを変更しているため、`[ci skip]` すべきではない。
- 単なる文言の文法修正であり、機能としての変更ではないため CHANGELOG は不要という判断。
...
- **影響範囲**
  - 実行時に `SomeModel.inspect` を使ってログ出力やデバッグをしている場合、その出力文言が微妙に変わります。
  - 文字列を厳密一致で検証しているテスト（アプリケーション側のテストなど）が、旧 `"schema informations"` を前提にしていると、今回の変更後の Rails に上げたタイミングでテストが落ちる可能性があります。

- **互換性**
  - メソッドシグネチャや振る舞い（スキーマ未ロード時にヒントを返す、というロジック自体）は変わっていないため、機能的な互換性問題はほぼありません。
  - 国際化 (i18n) とは無関係であり、英語メッセージの typo 修正に近い位置づけです。

- **注意点**
  - ログ解析やスクレイピングなどで `inspect` の出力を機械的にパースしている場合は、メッセージの一部が変わる点だけ念のためチェックしてください（通常は `inspect` 出力をパースするのは推奨されません）。

---

4. 参考情報 (あれば)  

- 対象 PR: https://github.com/rails/rails/pull/57794  
- "information" が不可算名詞であることについては一般的な英和辞典 (Merriam-Webster / Oxford) に準拠。  
- Rails 内部でも他箇所ですでに `"schema information"` という単数形が使われており、今回で表記揺れが解消されています。

---


## [#57796](https://github.com/rails/rails/pull/57796) Make config.rake_eager_load more explicit {#pr-57796}

**マージ日**: 2026/6/20 | **作成者**: [@fxn](https://github.com/fxn)

1. 概要 (1-2文で)  
`config.eager_load` が Rake 実行時に「無視される」のではなく「書き換えられる」挙動と、そのトリガーである `config.rake_eager_load` の役割をドキュメントと生成テンプレート上で明示的にした PRです。Rails のオートロード／イager load の契約を文章レベルで分かりやすくし、開発者が本番環境やテスト環境での eager load の実際の挙動を誤解しにくくしています。
...
- 通常の `production.rb` では以下のように設定します:
...
- しかし Rake タスク実行時には、Rails の内部ロジックにより **`config.eager_load` が `false` に書き換えられる** 場合があります。その制御フラグが `config.rake_eager_load` です。
- デフォルトの Rails アプリでは、「本番は常に eager load される」と思いがちですが、**Rake タスク時はそうとは限らない**というのが実態です。

これまでは `production.rb` のコメントに

```ruby
# Eager load code on boot for better performance and memory savings (ignored by Rake tasks).
```

とだけ書かれており、「ignored（無視）」という曖昧な表現のせいで、  
- 実際には「ユーザーが設定した値が Rake 実行時に上書き(変更)される」  
- その裏で `config.rake_eager_load` が関わっている  

という契約が伝わりにくくなっていました。

この PR は **挙動自体は変えず**、「どういう契約になっているのか」をコメントとガイドで明文化します。

---

### 2-2. `production.rb` テンプレートのコメント更新

`railties/lib/rails/generators/rails/app/templates/config/environments/production.rb.tt` にある生成テンプレートのコメントがより正確な表現に変更されています。

概念的には以下のニュアンスになります（実際の英語コメントの意図）:

```ruby
# Eager load code on boot for better performance and memory savings.
# Note: For Rake tasks, eager loading is controlled by config.rake_eager_load
# and may override this value.
config.eager_load = true
```

ポイント:

- 「ignored by Rake tasks」という曖昧な表現から、
  - Rake 実行時には `config.rake_eager_load` によって
  - `config.eager_load` が上書きされうる
- という因果関係をはっきりさせている。

これによって、「Rake タスクだと eager load は常に無効になる」と思い込むのではなく、「`config.rake_eager_load` の設定次第で挙動が変わる」ことが読み取れます。

---

### 2-3. `test.rb` テンプレートへのコメント追加

`railties/lib/rails/generators/rails/app/templates/config/environments/test.rb.tt` にも、`eager_load` とテスト環境に関するコメントが追記されています。
...
のような設定があり、「CI では eager load する」などのパターンがありますが、ここでも
- eager load の意味
- どのような場面で有効になるか

がより明示される方向のコメント追加になっています（本 PR の主眼は production＋Rake の説明ですが、整合性を取るために test も補強された形）。

---

### 2-4. オートロード／リロードガイドの追記

`guides/source/autoloading_and_reloading_constants.md` にも 1 行ほど追記され、以下の点がドキュメントとして明記されます:

- `config.eager_load` は「本番では常に true」ではなく、Rake タスク時には `config.rake_eager_load` の設定によって上書きされることがある。
- したがって、「Rails は production で必ず eager load される」というのは厳密には誤りである。

これにより、ガイドからオートロード戦略／ブート時のロード戦略を理解するときに、Rake の存在を考慮できるようになります。
...
- コードのロジック変更はなく、「コメントとドキュメントの改善」のみです。
- 既存アプリの挙動（特に Rake 実行時の eager load の有無）は一切変わりません。

### 3-2. `config.rake_eager_load` の重要性が上がる

今後、ドキュメントやコメントを読んだ開発者は

- なぜ Rake では eager load されないのか
- Rake でも eager load させたい／させたくない場合はどうすればよいか

を改めて意識するようになります。実際の制御は `config.rake_eager_load` で行われているため、Rake タスクでの挙動をカスタマイズしたい場合はここを調整します。

例:

```ruby
# すべての Rake タスクで eager load したい場合
config.rake_eager_load = true

# デフォルトどおり Rake では eager load したくない場合
config.rake_eager_load = false
```

### 3-3. 誤解が減るが、インターフェイスは歴史的事情のまま

PR 作成者も述べている通り:

- 「ユーザー設定を裏でひっそり書き換える」のは好ましくないが、
- 互換性の問題から、今すぐこのインターフェイスを変えるのは難しい

という前提は変わりません。  
この PR は、**少なくとも挙動を正しく説明する**ことで、将来的な API 見直しの足場づくりと、現行仕様の理解促進を狙っています。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57796  
- 関連する設定：
  - `config.eager_load`  
  - `config.rake_eager_load`  
- ドキュメント（該当ガイド）:
  - `guides/source/autoloading_and_reloading_constants.md`  
    - オートロードと eager load の関係、および環境別の挙動を整理する際に参照価値があります。

---


## [#57791](https://github.com/rails/rails/pull/57791) Fix typos in comments {#pr-57791}

**マージ日**: 2026/6/20 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
Rails のコメント中のタイプミス（スペルミス）を 2 箇所だけ修正する PR で、実行時の挙動や API には一切影響がありません。ドキュメント品質・可読性向上のみを目的とした変更です。

---

2. 変更内容の詳細  

### 2-1. Action Cable: `socket.rb` のコメント修正

対象ファイル: `actioncable/lib/action_cable/server/socket.rb`  

`conenctions` という誤記を `connections` に修正しています。  
（コードのロジックやインターフェイスは変更されていません）

イメージとしては以下のような差分です:

```diff
- # This socket manages client conenctions
+ # This socket manages client connections
```

※実際のコメント文自体はこの例と異なる可能性がありますが、修正内容はこのような単語のスペル修正のみです。

Action Cable におけるソケット／クライアント接続関連の説明コメントが正しい綴りになり、コードリーディング時の違和感が解消されます。

---

### 2-2. Active Support: `RedisCacheStore` のコメント修正

対象ファイル: `activesupport/lib/active_support/cache/redis_cache_store.rb`  

`configued` という誤記を `configured` に修正しています。  

例:

```diff
- # Returns the configued redis connection
+ # Returns the configured redis connection
```

Redis キャッシュストアの設定や接続に関するコメントの英語が正しくなっただけで、動作や設定項目には変更がありません。

---

3. 影響範囲・注意点  

- 実行コードには一切手が入っておらず、「コメントのみ」の変更です。
- そのため:
  - 既存アプリケーションの挙動、パフォーマンス、互換性への影響はありません。
  - マイグレーション、設定変更、依存関係の更新なども不要です。
- テストも追加されていませんが、コメント修正のみのため妥当な対応と言えます。
- CHANGELOG の更新も行われていませんが、「ドキュメント的変更」であり挙動変更がないため、Rails の運用方針としても問題ない範囲です。

開発者視点では、「Rails の公式コメントを参考にコードを読む・社内ガイドに引用する」といったケースで誤記に引きずられない、という程度の改善です。
...
- 対象コンポーネント:
  - `Action Cable` (`ActionCable::Server::Socket` 周辺)
  - `ActiveSupport::Cache::RedisCacheStore`
- PR の性質:
  - 純粋なタイポ修正（リファクタリングや内部仕様変更ではない）
  - 挙動変更なし、後方互換性への懸念なし

バージョンアップ時にこの PR を理由に動作検証を厚くする必要はなく、通常のパッチレベル更新の一部として扱って問題ない内容です。

---


## [#57787](https://github.com/rails/rails/pull/57787) Fix `Rails.app.dotenvs` to honor an explicit path argument {#pr-57787}

**マージ日**: 2026/6/20 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`Rails.application#dotenvs` に `path` 引数を渡しても、最初の呼び出し時のパスしか反映されないバグを修正し、パスごとに別々の `.env` 設定を正しく扱えるようにした PR です。`dotenvs` のメモ化方法を「単一インスタンス」から「パスごとのインスタンス」に変更しています。
...
`Rails.application#dotenvs` は本来、任意の `.env` ファイルパスを指定できるインターフェースになっています:

```ruby
Rails.application.dotenvs("/path/to/.env.custom")
```

しかし実装では、戻り値をインスタンス変数1つでメモ化していました:
...
1. 最初の呼び出し時に渡した `path` だけが使われる
2. 2回目以降は、`path` 引数を変えても `@dotenvs` がそのまま返される
3. 実質的に「**最初に呼んだパス固定**」であり、それ以降の `path` は無視される

さらに、開発環境では `Rails.application.creds` が内部で `dotenvs` をデフォルトパス (通常は `Rails.root.join(".env")`) で呼び出します:
...
そのため、どこかで一度 `Rails.application.creds` を呼ぶと、そのタイミングで `.env` のパスが確定してしまい、後から

```ruby
Rails.application.dotenvs("/path/to/.env.custom")
```

と呼んでも、**常にデフォルトの `.env`** を参照するという、非常に気付きづらい誤動作になっていました。

### 他のメソッドとの整合性

同ファミリのメソッドは以下のような振る舞いになっています:

| accessor                          | path 引数 | メモ化             | path を尊重するか |
|-----------------------------------|----------|--------------------|-------------------|
| `encrypted(path, …)`             | あり     | なし (毎回 new)   | する              |
| `credentials`                    | なし     | あり (単一インスタンス) | n/a               |
| `dotenvs(path = …)` (従来)       | あり     | あり (単一インスタンス) | **しない (バグ)** |

`dotenvs` だけが、「パスを受け取るのに単一インスタンスでメモ化している」というハイブリッドで不整合な仕様でした。

### 修正内容

メモ化を「パスごとのインスタンス」を保持する方式に変更しています:
...
- `@dotenvs` を単一の `ActiveSupport::DotEnvConfiguration` ではなく、`path` をキーとする **Hash** に変更
- 各 `path` ごとに一度だけ `ActiveSupport::DotEnvConfiguration.new(path)` を呼び出してメモ化
- これにより:
  - 同じパスを何度も指定しても、再パースせずにキャッシュされたインスタンスを使える
  - パス違いの `.env` は、それぞれ独立した設定として扱える

`@dotenvs` はこのメソッド内でしか参照されておらず (`creds` もここ経由でデフォルトパスを取るだけ)、単一インスタンスからハッシュへの変更は外部 API 上は破壊的ではありません。

### テスト追加

`railties/test/application/creds_test.rb` にテストが 10 行追加されています。  
テストの意図としては、主に以下を確認していると考えられます:

- デフォルトパスで一度 `dotenvs` / `creds` を呼んだ後でも、
- 別パスを指定して `dotenvs(custom_path)` を呼ぶと、
- **正しく custom_path 由来の値が取得できること**  
  (＝「最初のパス固定」になっていないこと)
...
- 影響対象:
  - `Rails.application.dotenvs(path)` を複数パスで使っているアプリケーション
  - 開発環境で `creds` (=`Rails.application.creds`) と `dotenvs(custom_path)` の両方を利用しているケース
- 振る舞いの変化:
  - これまで: 最初に呼んだときの `path` だけが有効で、それ以降の呼び出しでは引数 `path` が無視される
  - 修正後: `path` ごとに独立した `DotEnvConfiguration` が使われ、**呼び出しごとに与えた `path` が正しく尊重される**
- パフォーマンス:
  - 同じ `path` に対しては相変わらず 1 度だけパースし、以降はキャッシュを利用するため、従来からの性能特性は維持
  - 新たに複数パスを使い始めた場合、それぞれのパスで `.env` が初回パースされるコストがかかるが、これは本来期待される挙動
- 互換性:
  - 公開インターフェース (`dotenvs` のシグネチャ・返り値の型) は変わっていない
  - ただし「バグとも取れる古い挙動（最初のパスが固定される）」に依存していた場合、結果が変わる可能性はあります
    - 例: 最初に意図せずカスタム `.env` を読んでしまい、後からデフォルト `.env` を読むつもりで `dotenvs(default_path)` を呼んでいた、など

実用上は「修正後の方が直感的で正しい」挙動なので、多くのアプリではバグ修正として歓迎されるはずです。

---

4. 参考情報 (あれば)  

- 該当メソッド: `railties/lib/rails/application.rb` の `Rails::Application#dotenvs`
- 関連 API:
  - `Rails::Application#creds`  
    → `ActiveSupport::CombinedConfiguration.new(envs, dotenvs, credentials)`
  - `ActiveSupport::DotEnvConfiguration`
- 類似の設計パターン:
  - `encrypted(path)` が非メモ化で毎回 new を返すのに対し、`dotenvs(path)` は「パスごとにメモ化」するハイブリッド設計  
    → パフォーマンスと API 一貫性を両立する意図が読み取れます。

---


## [#57788](https://github.com/rails/rails/pull/57788) Remove unused private `EnvConfiguration#lookup` {#pr-57788}

**マージ日**: 2026/6/20 | **作成者**: [@55728](https://github.com/55728)

1. 概要 (1-2文で)  
`ActiveSupport::EnvConfiguration` に定義されていた、実際には一度も呼ばれていない private メソッド `#lookup` が削除されました。動作仕様の変更はなく、内部コードの整理・不要コード削除のみです。
...
対象ファイル:  
- `activesupport/lib/active_support/env_configuration.rb` (+0 / -4)

削除されたのは private メソッド `#lookup(env_key)` だけです。PRの説明によると:

- `#require`, `#option`, `#keys` などのパブリックメソッドは、環境変数相当の値を取得する際に、`lookup` を経由せず **直接** インスタンス変数 `@envs` を参照していました。  
  例:  
  ```ruby
  @envs.fetch(envify(*key))
  @envs[envify(*key)]
  ```
- サブクラス `DotEnvConfiguration` も `lookup` を一切呼んでおらず、`parse_env_file` / `reload` 内で `@envs` を構築するのみでした。
- リポジトリ全体を grep した結果でも、`lookup` の定義以外に参照がまったく存在せず（`send(:lookup)` や `:lookup` など動的ディスパッチも含めて）、**完全に未使用のメソッド** であることが確認されています。

このため、`EnvConfiguration` クラスから以下のようなメソッド定義が削除された形になります（イメージ）:
...
- 公開 API への影響:  
  - `ActiveSupport::EnvConfiguration` および `DotEnvConfiguration` の **パブリックな振る舞いは一切変わりません**。  
  - 削除された `#lookup` は **private メソッド** であり、フレームワーク外から呼び出すことは想定されていなかったため、通常の利用者には影響ありません。

- 破壊的変更の可能性:  
  - メソッドが private であっても、以下のようにメタプログラミングで無理やり呼び出していたコードがあれば壊れます:
    ```ruby
    config = ActiveSupport::EnvConfiguration.new(...)
    config.send(:lookup, "FOO") # ← こういうコードは動かなくなる
    ```
  - ただし PR 説明の通り、Rails 本体ではそのような利用は確認されていません。

- サブクラスへの影響:  
  - `lookup` をオーバーライドしてカスタマイズしていた独自サブクラスがもし存在していれば、  
    - 元の private メソッドが呼ばれていなかったため、「オーバーライドしてもそもそも使われていない」状態だったと考えられます。  
    - 今回の削除により「オーバーライド対象そのものがなくなった」だけで、動作は従来どおり（＝そもそも何も起きていなかった）です。  
  - そのため、**実質的な挙動変更はなし** と考えて差し支えありません。

- テスト観点:  
  - 既存テストで `EnvConfiguration` / `DotEnvConfiguration` の挙動がカバーされていれば、そのままグリーンであることが期待されます。  
  - もしアプリ側で env 周りのラッパーやヘルパーを自作している場合、`send(:lookup, ...)` のようなコードがないか検索しておくとより安全です。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57788  
- 関連クラス:  
  - `ActiveSupport::EnvConfiguration`  
  - `ActiveSupport::EnvConfiguration::DotEnvConfiguration`（`.env` ファイルを読み込んで `@envs` を構築するサブクラス）  
- この変更は、未使用コードの削除による **内部実装のクリーンアップ** であり、Rails 7/8 系へのマイグレーション上の考慮点にはほぼなりません。

---


## [#57786](https://github.com/rails/rails/pull/57786) [ci skip]Fix typo in comment for ENV.fetch usage in application.rb {#pr-57786}

**マージ日**: 2026/6/20 | **作成者**: [@willnet](https://github.com/willnet)

1. 概要 (1-2文で)  
application.rb 内で `ENV.fetch` の使い方を説明しているコメントにあった誤字を修正するだけの PR です。コードの挙動や外部 API には一切変更がなく、ドキュメント品質の向上のみを目的としています。
...
- 変更内容: `ENV.fetch` の利用例を説明するコメント行の文言修正（typo 修正）

Rails の `application.rb` には、環境変数を使った設定の推奨パターンとして、以下のような `ENV.fetch` の利用例がコメントで記載されています:

```ruby
# For example, to use an environment variable:
#   config.x.some_api_key = ENV.fetch("SOME_API_KEY")
```

この PR では、このようなコメント例の中の誤字・表現ミスが修正されています。  
コードそのもの（`ENV.fetch` の使い方や設定のキーパスなど）は変更されておらず、修正はコメント行の 1 行のみ（+1 / -1）です。
...
- 実行時挙動への影響: なし
  - コメントのみの修正のため、アプリケーションの挙動、パフォーマンス、互換性に影響はありません。
- 公開 API / インターフェースへの影響: なし
- マイグレーションや設定変更の必要: なし
- テスト・CHANGELOG:
  - バグ修正や機能追加ではなくコメント修正のみのため、テスト・CHANGELOG は更新されていません（チェックボックスも未チェック）。

---

4. 参考情報 (あれば)  

- PR タイトルに含まれる `[ci skip]` は、CI を回すまでもない軽微な変更（コメント修正など）であることを示しています。
- `ENV.fetch` は環境変数が未設定の場合に例外を発生させられるため、Rails 公式コメント中でも「必須の環境変数を明示的に要求する」用途で推奨されることが多いです。  
  例:
  ```ruby
  config.x.payment.api_key = ENV.fetch("PAYMENT_API_KEY")
  ```
  こうした用法を説明するコメントがより正確になったという位置づけの変更です。

---


## [#57579](https://github.com/rails/rails/pull/57579) Add RFC 9110 compliant Accept header content negotiation opt-in {#pr-57579}

**マージ日**: 2026/6/19 | **作成者**: [@williantenfen](https://github.com/williantenfen)

1. 概要 (1-2文で)  
RFC 9110 に準拠した `Accept` ヘッダによるコンテンツネゴシエーションを行うオプション設定 `config.action_dispatch.respect_accept_header_rfc9110` が追加されました。デフォルトは従来通りの挙動（IE7 向けワークアラウンドを維持）で、明示的に opt-in したアプリだけが新しい挙動になります。
...
`config.action_dispatch.respect_accept_header_rfc9110` が追加されました。

- デフォルト: `false`
- `false` の場合（従来互換モード）
  - IE7 向けに 2010 年から存在するワークアラウンドがそのまま使われます。
  - `Accept` ヘッダに `*/*` が含まれていると「ブラウザからのリクエスト」とみなし、HTML を優先する（`BROWSER_LIKE_ACCEPTS` フィルタ）挙動が続行されます。
- `true` の場合（RFC 9110 準拠モード）
  - RFC 9110 に則り、`Accept` ヘッダの
    - メディアタイプの**具体性**（`text/html` > `text/*` > `*/*`）
    - `q` 値（品質値）
    を正しく考慮してネゴシエーションします。
  - `*/*` を含んでいても、より具体的なメディアタイプがあればそれが優先されます。

#### 挙動の具体例

```ruby
# 従来挙動 (respect_accept_header_rfc9110 = false)
# Accept: application/json, */*
# → IE7 ワークアラウンドにより、ブラウザライクとみなして HTML を選ぶ可能性が高い

# 新挙動 (respect_accept_header_rfc9110 = true)
# Accept: application/json, */*
# → RFC 9110 に従い、より具体的な application/json を優先して JSON を選択
```

この PR の説明中の例も同じで、`*/*` を含むブラウザ由来（に見える）ヘッダでも、JSON をきちんと選ぶようになります。

### 2-2. 既存設定とのコンフリクト警告

`config.action_dispatch.ignore_accept_header` と新フラグが両方とも `true` に設定された場合、ログに警告が出ます。

- 意味合い:
  - `ignore_accept_header` は「`Accept` ヘッダそのものを無視する」フラグ。
  - `respect_accept_header_rfc9110` は「`Accept` ヘッダを RFC 9110 に準拠して解釈する」フラグ。
- 両方 `true` の場合は論理的に矛盾するため、
  - Rails は `ignore_accept_header` を優先
  - そのことを警告ログで明示します。

### 2-3. アップグレードパス / new_framework_defaults

`config/initializers/new_framework_defaults_8_2.rb` のテンプレートに、コメントアウトされた形で以下のような設定が追加されています（抜粋イメージ）:

```ruby
# Rails.application.configure do
#   # Enable RFC 9110 compliant Accept header content negotiation.
#   # This will prioritize specific media types and quality values as per the standard.
#   config.action_dispatch.respect_accept_header_rfc9110 = true
# end
```

- 新規アプリやバージョンアップ時にこの initializer を確認すると、
  - 何が変わるのか
  - なぜ opt-in なのか
  がコメントで説明されています。
- CHANGELOG とガイド (`guides/source/configuring.md`) にも説明が追加され、設定項目として正式にドキュメント化されています。

### 2-4. 実装とテスト

変更ファイルの主なポイント:

- `action_dispatch/http/mime_negotiation.rb`
  - `BROWSER_LIKE_ACCEPTS` による IE7 ワークアラウンド適用を、上記フラグで切り替えられるように調整。
  - フラグ ON の場合は、`Accept` をフィルタリングせず、そのまま標準的な優先度計算にかける形になります。
- テスト (`respond_to_test.rb`) 追加:
  - `test_rfc9110_accept_header_respects_specificity`
    - `text/html, */*` vs `application/json` など、具体性の違いに応じた正しい選択を検証。
  - `test_rfc9110_with_xhr_request`
    - XHR リクエスト時（`XMLHttpRequest` / `fetch` など）に、従来モード・RFC 9110 モードの両方で期待どおりのフォーマットが選ばれることを検証。
...
- デフォルト値は `false` のため、**この PR のマージだけでは挙動は変わりません**。
- 既存アプリは、従来通りの「`*/*` を含む Accept をブラウザとみなして HTML を優先」のロジックが維持されます。

### 3-2. フラグを有効化した場合の影響

`config.action_dispatch.respect_accept_header_rfc9110 = true` にすると:

- これまで「HTML が返ってきていた」ケースで JSON や他のフォーマットが返るようになる可能性があります。
  - 例: API ではなく通常 HTML コントローラに対して、
    - ブラウザの拡張やクライアントが `Accept: application/json, */*` を送っていた場合、
    - 新挙動だと JSON レスポンスが選ばれることがあります。
- 特に `respond_to` を使って複数フォーマットに対応しているコントローラでは、
  - 「暗黙的に HTML が選ばれていた」挙動が変わる可能性があるため、
  - フラグ ON 前に統合テスト・E2E テストなどで挙動確認を推奨します。

### 3-3. `ignore_accept_header` との併用

- すでに `config.action_dispatch.ignore_accept_header = true` を使っているアプリでは、
  - 新フラグを `true` にしても意味がなく、`ignore_accept_header` が優先されます。
  - ログに警告が出るため、どちらの挙動を本当に望んでいるかを見直す必要があります。

### 3-4. 将来的な変更の可能性

- PR の説明にもある通り、「将来のメジャーバージョンで IE7 ワークアラウンドを削除できるようにするため」の布石となる変更です。
- 8.2 以降のどこかで:
  - `respect_accept_header_rfc9110` をデフォルトで `true` にする、
  - あるいはワークアラウンドを削除する、
 可能性が高いので、長期的にはアプリ側で RFC 9110 挙動への対応を進めておくと安全です。

---

4. 参考情報 (あれば)  

- PR: https://github.com/rails/rails/pull/57579  
- 関連 issue: Fixes #57353  
- RFC 9110 (HTTP Semantics) – Section on Content Negotiation / Accept header  
- 変更ファイル:
  ...
  などに設定項目と挙動の説明が反映されています。

---


## [#57780](https://github.com/rails/rails/pull/57780) Extract Template.validate_formats and Template.normalized_formats {#pr-57780}

**マージ日**: 2026/6/19 | **作成者**: [@etiennebarrie](https://github.com/etiennebarrie)

1. 概要 (1–2文で)  
このPRは、`Template::Types` へ直接アクセスしてリクエストフォーマットを処理していたコードを、`Template` クラス内のヘルパーメソッド（`validate_formats` / `normalized_formats`）経由にリファクタリングしたものです。あわせて、`Template` の autoload 定義の重複を削除し、並び順を整理しています。

---

2. 変更内容の詳細  

### 2-1. フォーマット処理の責務を Template に集約

これまで `ActionView::LookupContext` などで、テンプレートのフォーマット（`html`, `json` など）を扱う際に、`Template::Types` の実装に「直接」依存していました。このPRでは、その依存を隠蔽するために、`ActionView::Template` に以下の2つのクラスメソッドが追加されています：

- `Template.validate_formats(formats)`
- `Template.normalized_formats(formats)`

（※PR本文から機能だけが読み取れる状態で、実装詳細は推測を含みますが、Rails既存コードのパターンからみて、次のような役割が想定されます）

#### `Template.normalized_formats(formats)`
- 渡された `formats` を、`Template::Types` 依存の詳細を隠しつつ「正規化」するメソッド。
- 想定される処理例
  - `:html` / `"html"` / `Mime[:html]` のような入力を、内部で一貫した表現（シンボル配列や MIME 型配列など）にそろえる。
  - `nil` や空配列をデフォルトフォーマット（`[:html]` など）に変換。

#### `Template.validate_formats(formats)`
- 上記の正規化された `formats` が、`Template::Types`（または ActionDispatch が提供する MIME/フォーマット定義）にとって有効かどうかを確認するメソッド。
- 想定される処理例
  - 未定義フォーマット（例: `:foobar`）を検出してエラーを投げる、あるいは無視／警告する。
  - フォーマット指定が `nil` だったり、明らかに不正な型（数値など）の場合に例外を発生させる。

これらの実装では、内部的には `Template::Types` を使いつつも、その利用は `Template` クラス内部に閉じ込められます。これにより、利用側（`LookupContext` など）は `Template::Types` の実装差分や初期化状態を気にせず、`Template` のインターフェイスだけを使えばよくなります。

### 2-2. Action Dispatch 有無による Template::Types の差分を吸収
...
- `ActionDispatch` がロードされているかどうかで、`Template::Types` の中身や挙動が変わる実装になっている。
- その状態でアプリケーション側や他コンポーネントが `Template::Types` に直接触れると、Rails のロード順・環境によって挙動が変わる（または壊れる）リスクがある。
- そこで「フォーマットのバリデーション／正規化」という共通処理を、`Template` クラス内のメソッドにまとめることで、「どの状態でも正しく動く入口」を1つにした、という設計上の改善です。

### 2-3. LookupContext からの呼び出しを Template 経由に変更

`actionview/lib/action_view/lookup_context.rb` の変更は少ないですが、主旨は:

- 以前: `Template::Types` に直接アクセスしてフォーマットを扱っていた
- 以後: `Template.validate_formats` / `Template.normalized_formats` を経由するようにする

ことです。

これにより:

- `LookupContext` は「フォーマットの正規化・検証」を、`Template` に委譲するだけの薄いクラスになる。
- `Template::Types` の具体的な実装や、`ActionDispatch` のロード状況に依存しなくなる。

### 2-4. Template の autoload 定義の整理

`actionview/lib/action_view/template.rb` 内の、`autoload` 定義から:

- 重複して宣言されていた autoload を削除
- 宣言順を整理（ソート）して、今後の重複を見つけやすくした

というメンテナンスが行われています。

Rails は autoload による動的ロードが多いため、重複や順序が原因で予期せぬロード順になったり、`Zeitwerk` との兼ね合いで警告が出る原因になりえます。今回の整理は、そのような問題の種を減らす目的の微修正です。
...
- **外部 API の変更は基本的に無し**
  - 通常のビュー描画や `render` の利用に関して、アプリケーションコード側が意識すべき破壊的変更はなさそうです。
  - 変更は内部的な責務の整理と依存箇所のリファクタリングが中心です。

- **直接 `Template::Types` を利用しているコードへの注意**
  - 自作のライブラリやアプリコードで、`ActionView::Template::Types` を直接参照している場合、今回のPRで即座に壊れるわけではありませんが、「今後の変更でより隠蔽が進む」方向性が示されたと考えるとよいです。
  - もし「リクエストフォーマットの正規化や検証」を独自に行っている場合、将来的には `Template` 側に公開される（または既にある）メソッドを使う設計に寄せると安全です。

- **ActionDispatch ロード順に敏感なコードは見直し候補**
  - このPRの背景自体が、「ActionDispatch のロード有無によって Template::Types 実装が変わる」問題です。
  - もしアプリ側が「ActionDispatch ありき」の前提で `Template::Types` を触っていると、テスト環境やコンソール、特定のタスク実行時など、ロードパターンが異なる状況で不整合が生じる可能性があります。

- **autoloader 周辺の挙動**
  - autoload の重複削除と並び替えにより、「特定のファイルが想定外のタイミングでロードされる／されない」というレアケースの不具合が解消される可能性があります。
  - 逆に、新しい autoload の順序に依存していた極端に特殊なコードがあれば影響を受ける可能性はありますが、通常のアプリケーションでは問題にならないはずです。

---

4. 参考情報 (あれば)  

- 参考になりそうな関連コンセプト（ソース未提示のため概念レベル）
  - `ActionView::Template`: ビューのテンプレート1つを表すクラス。フォーマット、ハンドラ（ERB, Haml など）、変換済みソースなどを持つ。
  - `ActionView::LookupContext`: ビューを検索するコンテキスト。パス、フォーマット、ロケールなどを元に、どのテンプレートを使うか決定する。
  - `Template::Types`: 拡張子やシンボル（`html`, `json` 等）と `Mime::Type` のマッピングを持つ内部構造。
  - `ActionDispatch`: リクエスト/レスポンス、ルーティングなど HTTP レイヤーを提供するコンポーネントで、`Mime` やフォーマット周りの定義にも関与。

- 類似の変更意図としては、過去の Rails コアでも
  - 「コンポーネント間で内部構造を直接参照していた部分を、明示的なヘルパーメソッドに置き換える」
  - 「ロード順や環境差による subtle bug を避けるために責務を集約する」
  といったリファクタリングが繰り返し行われており、その延長線上の PR だと捉えると理解しやすいです。

---


## [#57775](https://github.com/rails/rails/pull/57775) Make ActiveRecord::Base.primary_key a class attribute {#pr-57775}

**マージ日**: 2026/6/19 | **作成者**: [@ilianah](https://github.com/ilianah)

1. 概要 (1-2文で)  
`ActiveRecord::Base.primary_key` が「クラス属性 (class_attribute)」として扱われるようになり、`table_name` と同様にサブクラスへオーバーライド内容が継承されるようになりました。これにより、抽象クラスや STI での主キー指定がより直感的で一貫した挙動になります。
...
- `ActiveRecord::Base.primary_key` が、「クラスごとに保持され、サブクラスにも継承される属性」として定義されるようになりました。
- これにより、あるモデルクラスで `self.primary_key = ...` と上書きすると、そのクラスを継承したサブクラスも同じ `primary_key` を使うのが基本挙動になります（`table_name` と同じ感覚）。

以前は、`primary_key` の解決ロジックがより動的で、継承との関係が直感的でないケースがありましたが、今回の変更で「クラス属性として継承される」というルールに整理されています。

### 典型的な利用パターン

#### 抽象クラスでの共通 primary_key 指定
...
今後はこのようなコードを書いた場合、`User` や `Post` も `uuid` を主キーとして扱うことが期待どおりに機能します。

#### STI (Single Table Inheritance) での利用
...
STI 親クラスで `primary_key` を変えておけば、子クラス側は特に何もしなくても同じキーを用いるようになります。

#### サブクラス側での上書き

クラス属性なので、もちろんサブクラス側で再度オーバーライドも可能です。

```ruby
class Animal < ActiveRecord::Base
  self.primary_key = "global_id"
end
...
class Dog < Animal
  self.primary_key = "dog_id" # ここだけ別の主キーに
end
```

この場合、`Animal` と `Cat` は `"global_id"`、`Dog` だけ `"dog_id"` になります。
...
- **既に `primary_key` をオーバーライドしていて、そのクラスのサブクラスも存在するアプリ**  
  以前の挙動が「暗黙的に継承されていない」状態に依存していた場合、今回の変更によりサブクラスも親クラスの `primary_key` を引き継ぐようになり、挙動が変わる可能性があります。

  例: 以前は「親で `primary_key` を変えたがサブクラスはデフォルト主キーのまま」という前提で設計していた場合、今後はサブクラスも新しい主キーを使う挙動になる可能性が高いです。

- **複雑な継承ツリー (抽象クラス + STI + 通常継承ミックス)**  
  継承階層のどこかで `primary_key` を変更すると、その下流のクラスにまで設定が伝播します。どのクラスで何を指定しているかを改めて確認した方が安全です。

### 移行時に確認したい点

- モデル階層で、どのクラスが `self.primary_key = ...` を呼んでいるかを grep などで洗い出す。
- 親クラスで指定した `primary_key` がサブクラスにも伝播して問題ないかをテーブル定義と照合する。
- STI を使っているテーブルで、すべてのサブクラスが同じ主キー定義で動作しているかを確認する。
- もし「サブクラスには継承してほしくない」ケースがある場合は、そのサブクラス側で明示的に `self.primary_key = ...` を再指定する。

---

4. 参考情報 (あれば)  

- `table_name` も同様にクラス属性として扱われており、「親で指定 → 子へ継承」という挙動です。本 PR は `primary_key` をこれと同じ一貫したモデルに揃える変更です。
- 変更ファイル:
  - `activerecord/lib/active_record/attribute_methods/primary_key.rb`  
    `primary_key` の定義がクラス属性的な扱いになるよう調整。
  - `activerecord/test/cases/primary_keys_test.rb`  
    継承や抽象クラス/サブクラスに関する動作テストが追加。
...
## [#57773](https://github.com/rails/rails/pull/57773) Remove outdated documentation for overriding primary_key {#pr-57773}

**マージ日**: 2026/6/19 | **作成者**: [@ilianah](https://github.com/ilianah)

1. 概要 (1-2文で)  
Rails 7.2系以降（PR #57638 の変更）では、`primary_key` の値だけを単独でオーバーライドすることができなくなったため、その古い方法を説明していたドキュメントコメントが削除されました。コードの挙動自体は変わらず、ドキュメント上の記述のみが整理されています。
...
削除されたのは、`primary_key` を「単にクラスメソッドをオーバーライドするだけで変更できる」かのようなドキュメントコメントです。  
PRの説明にある通り、前のPR (#57638) によって、`primary_key` の値は `primary_key_definition` と同期している必要があるように設計が変わっており、次のような昔ながらのやり方は正しい手順としては扱えなくなりました。
...
以前はこうした記述で「主キー名だけ」変えるケースが多く、ドキュメントにもそれを示唆するコメントがありましたが、現在の実装では「主キーの定義全体（型・オプションなど）を含む `primary_key_definition`」との整合性が必須になっているため、「`primary_key` だけ変えればよい」という説明は誤解を生むため削除されています。

コード上のロジックやメソッドの挙動は一切変更されておらず、削除されたのはコメント（ドキュメント）のみです。
...
- 直接の挙動変更はなし  
  - このPR自体はコメント削除のみなので、アプリケーションの動作は変わりません。

- しかし背景として:
  - PR #57638 以降の Rails では、以下の点に注意が必要です。
    - `primary_key` を単独でオーバーライドするのではなく、`primary_key_definition` と一貫性を持たせる必要がある。
    - 主キーをカスタマイズする場合は、マイグレーション（DB定義）と `primary_key_definition` の双方が、実際のテーブル定義と一致しているかを確認する必要があります。

- 既存コードの見直しポイント  
  - もしアプリ内で以下のようなコードだけで済ませていると、将来的なバージョンアップ時に不整合が起きる可能性があります。
    ```ruby
    class User < ApplicationRecord
      self.primary_key = "uuid"
    end
    ```
  - その場合:
    - DB側の主キー定義（例: `id: :uuid, default: ...`）  
    - Rails側の `primary_key_definition`（型・オプションを含む）  
    を合わせて整合性を取る必要があります。

- ドキュメントを信頼して実装している場合の注意  
  - 旧来のコメントを読んで「`primary_key` だけ変えればよい」と思っていた開発者が、今後はその手法を「推奨されていない／一部ケースで破綻する」ものとして再考すべき、というメッセージとも解釈できます。
...
- このPR:  
  - https://github.com/rails/rails/pull/57773  
- 関連PR（仕様変更の本体）:  
  - https://github.com/rails/rails/pull/57638  
- 主キーに関する Rails ガイド（英語）:  
  - Active Record Basics – Primary keys  
    https://guides.rubyonrails.org/active_record_basics.html#primary-keys  

主キーをカスタマイズしているアプリでは、`primary_key` だけでなく `primary_key_definition` の扱いと、DBの実際のテーブル定義との整合性を今後意識しておくと安全です。

---


## [#57651](https://github.com/rails/rails/pull/57651) Fix associations on a new record when the owner has a composite primary key {#pr-57651}

**マージ日**: 2026/6/19 | **作成者**: [@jsaubry](https://github.com/jsaubry)

1. 概要 (1-2文で)  
複合主キーを持つレコードに対する `has_many` / `has_one` の関連が、「未保存だがキーはすべて埋まっている」場合に常に空(`[]`/`nil`)になってしまう問題を修正したPRです。`foreign_key_present?` で複合主キーを正しく判定するようにし、単一主キーと同様に関連クエリが発行されるようになりました。
...
対象は、例えば以下のような複合主キーを持つモデルです（PR説明に出てくる例）:

```ruby
class Cpk::Book < ApplicationRecord
  self.primary_keys = [:author_id, :id]
...
このとき、まだ保存していないが複合主キーの値はすべてセットされているレコード:

```ruby
book = Cpk::Book.new(id: [1, 10])  # new_record? == true
# author_id + id の2つのキーは埋まっている前提
```
...
とすると、本来は `(author_id, id)` を条件にクエリが発行されて関連レコードが返ってくるべきです。しかし、これまでは **一切クエリが発行されず**、常に `[]` が返っていました。`has_one` の場合も同様に `nil` 固定になる挙動でした。

一方で、**単一主キー** のモデルでは、同じ状況（未保存だが主キー値がセット済み）のときにちゃんとクエリが発行されていました。この差異が「複合主キーだけ壊れている」状態になっていた、というのが問題です。

### 原因の詳細

原因は `ActiveRecord::Associations::ForeignAssociation#foreign_key_present?` の実装にあります。

- `reflection.active_record_primary_key` は、複合主キーの場合は配列（例: `["author_id", "id"]`）になります。
- しかし従来コードは、この配列をそのまま `attribute_present?` に渡していました。

疑似コードでいうと、以前はこんな形だったと考えられます:
...
`attribute_present?` は本来 `"author_id"` といった「単一のカラム名の文字列 or シンボル」を期待していますが、配列を渡されると:

- 配列全体を文字列化したキー（例: `["author_id", "id"]` → `"["author_id", "id"]"`）を探しにいく
- 当然そんなカラムは無いため、常に `false` を返す

その結果、

```ruby
null_scope? # has_many側で使われる
# => new_record? && !foreign_key_present?
```

が常に `true` になり、「未保存で外部キーが無いレコードだからクエリを飛ばさず空スコープにしておこう」という分岐に入ってしまい、クエリが発行されない状態になっていました。

`has_one` 側でも `Association#find_target?` 経由で同じ `foreign_key_present?` を参照しているため、同じ原因で `nil` 固定になっていました。

### 修正内容

`foreign_key_present?` を、**複合主キーなら各キーを個別にチェックする**ように変更しています。変更後のロジック:
...
- `Array(...)` で単一キーでも複合キーでも配列に正規化
- 各キー (`"author_id"`, `"id"` など) について `owner.attribute_present?(key)` を呼ぶ
- すべてのキーが present? のときに `true` を返す

これにより:

- 単一主キーの場合: 配列長1なので従来挙動と実質同じ
- 複合主キーの場合: 「すべての構成キーが埋まっているか」を正しく判定できるようになる

結果として:

```ruby
book = Cpk::Book.new(id: [1, 10])

book.chapters.to_a
# Before: []       ← クエリなし
# After:  [...]    ← (author_id, id) によるクエリが発行される
```
...
- `has_many_associations_test.rb`
  - モデル: `Cpk::Book` → `has_many :chapters`
  - ケース:
    - 複合主キーのすべてがセットされている場合に関連を正しく取得できること
    - どれか1つのキーが欠けている場合はクエリしない（空になる）こと

- `has_one_associations_test.rb`
  - モデル: `Cpk::Order` → `has_one :book`（複合キー前提）
  - 上記と同様に「全キーあり」「一部欠け」の両方を検証

`CHANGELOG.md` にもこの修正が追記されています。
...
- 影響を受けるのは:
  - 複合主キーを使っているモデル
  - かつ `has_many` / `has_one` の関連を持っているケース
  - かつ「未保存 (`new_record? == true`) だが、主キーカラムはすべて埋まっている」状態で関連を参照している箇所

- これまで:
  - そのような関連呼び出しは常に `[]` / `nil` を返していた
  - その挙動に依存しているコードがある場合、「クエリが発行され結果が返る」ように変わるため、挙動が変わります

- 本来は「単一主キーと同じく、キーが揃っているなら関連を解決できるべき」なので、この変更は **バグ修正** であり、期待される仕様への整合です。ただし以下には注意してください:
  - 新たに関連クエリが発行されるようになるため、パフォーマンスに影響する可能性があります（※今まで「たまたまクエリが飛んでいなかった」部分）。
  - 未保存レコードに対する関連の存在チェックや分岐ロジックで、空を前提にしていた場合は挙動見直しが必要になるかもしれません。

- 単一主キーのみを使っているアプリケーションには挙動変更はありません。
...
- このPR自体:  
  - タイトル: “Fix associations on a new record when the owner has a composite primary key”  
  - 番号: #57651  
  - マージ日時: 2026-06-19T12:07:40Z
...
- 関連コード:  
  ...
  - `CollectionAssociation#null_scope?`（`has_many` 側）  
  - `Association#find_target?`（`has_one` 側）
