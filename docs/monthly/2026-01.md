...
## [#56692](https://github.com/rails/rails/pull/56692) Improve diagnostics UI to make nested stack traces more discoverable {#pr-56692}

**マージ日**: 2026/1/30 | **作成者**: [@mctaylorpants](https://github.com/mctaylorpants)
...
- そのため、サマリーバーの端に「▶︎ のような矢印」を追加し、
  - 畳まれている時: 矢印が閉じた向き
  - 展開されている時: 矢印が回転して開いた向き
  という、よくあるディスクロージャ UI を導入しています。
...
## [#56696](https://github.com/rails/rails/pull/56696) Add documentation for foreign key options to add_reference [ci skip] {#pr-56696}

**マージ日**: 2026/1/30 | **作成者**: [@wakairo](https://github.com/wakairo)
...
これにより、`add_reference` の `foreign_key:` に指定できるオプションが、`add_foreign_key` と同じであることがドキュメント上も分かりやすくなりました。  
...
## [#56694](https://github.com/rails/rails/pull/56694) Update typemap for timezone during type_casted_binds {#pr-56694}

**マージ日**: 2026/1/29 | **作成者**: [@matthewd](https://github.com/matthewd)

1. 概要 (1-2文で)  
PostgreSQL アダプタで、**バインド変数を type cast するタイミングで timezone 用 typemap を確実に更新するようにした PR**です。  
...
- PostgreSQL アダプタの「timezone に依存する typemap 更新処理」を、
  ...
  - つまり「実際に outgoing binds を type cast する直前」
  に移動・紐づけました。

- 具体的には：
  - `activerecord/lib/active_record/connection_adapters/postgresql/quoting.rb` に  
...
- 対象：**PostgreSQL を使う Rails アプリケーションの ActiveRecord**（`postgresql` アダプタ）。
- 主な対象機能：
...
- この PR 単体では**互換性上の注意点はほぼない**と考えてよいです。
- 万が一、PostgreSQL アダプタを monkey patch している場合は、以下を確認すると安全です:
  - `ActiveRecord::ConnectionAdapters::PostgreSQLAdapter` まわりで
    ...
    - `type_casted_binds` や quoting 関連メソッド
    を上書きしていないか。
...
## [#56693](https://github.com/rails/rails/pull/56693) Combine duplicate `if respond_to?(:action_view)` {#pr-56693}

**マージ日**: 2026/1/29 | **作成者**: [@r7kamura](https://github.com/r7kamura)
...
- この変更の元になったコードは、以下の PR で追加された部分:
  - https://github.com/rails/rails/pull/55336/changes#diff-9f26e965e7e2a6842bd6ea755924a621038bc9dd8d63cfa6b34092aa32ede02cR371-R374  
- 今回の PR:  
  - タイトル: Combine duplicate `if respond_to?(:action_view)`  
  - 番号: #56693  
  - 作成者: r7kamura  
  - マージ日時: 2026-01-29T04:40:46Z  
...
## [#56687](https://github.com/rails/rails/pull/56687) Remove unused delegate loading from MemCacheStore {#pr-56687}

**マージ日**: 2026/1/27 | **作成者**: [@Saidbek](https://github.com/Saidbek)
...
- その後、キャッシュフォーマットの非推奨バージョン 6.1 のサポート削除に伴い、`DupLocalStore` と `DelegateClass` の利用自体が削除されました（コミット `38bfbcfbeacf461cafc4ef816cd5552696da2352`）。
...
- 互換性:
  ...
  - もし「`mem_cache_store.rb` が `delegate` を読み込んでくれていること」をアプリ側で暗黙的にアテにしていたケースがあれば、極めて例外的に影響し得ますが、通常そのような依存の仕方は推奨されません。
...
## [#56686](https://github.com/rails/rails/pull/56686) [Changelog] Fix overlapping acronyms order dependency in underscore#56679 {#pr-56686}
...
- 変更ファイルは `activesupport/CHANGELOG.md` のみで、Active Support に関する変更点として以下の内容（要旨）が追記されています:
  - `underscore` メソッドで、`config/initializers/inflections.rb` などに登録された複数の略語が「互いに部分的に重なっている」場合（例: `API` と `AP` のような関係）、登録順によって `underscore` の結果が変わってしまう問題があった。
...
- ただし、元になっている修正（以前のコミット）によって:
  ...
  - それらの略語が互いに部分一致する（`HTML` と `HT` など）場合、
...
- 該当する元コミット（実際のバグ修正本体）:  
  https://github.com/rails/rails/commit/38e8df6a5a3244e5a239e33c3a08ed0c52791b73  
- この PR:  
  https://github.com/rails/rails/pull/56686

---


## [#56282](https://github.com/rails/rails/pull/56282) Fix #56232: Clean output directory when generating guides to remove stale output files and add guide generation tests {#pr-56282}

**マージ日**: 2026/1/27 | **作成者**: [@harsh183](https://github.com/harsh183)
...
- 注意点:
  - 出力ディレクトリ内に**手動で置いていた独自ファイル**がある場合、ガイド生成を走らせると削除される可能性があります。
...
- 対応 issue: https://github.com/rails/rails/issues/56232  
  （古いガイド出力が残り検索エンジンにインデックスされる問題の報告スレッド）
- PR 本体: https://github.com/rails/rails/pull/56282  
- ガイドへの貢献ガイドライン:  
...
## [#56668](https://github.com/rails/rails/pull/56668) chore: Update outdated GitHub Actions versions {#pr-56668}

**マージ日**: 2026/1/25 | **作成者**: [@pgoslatara](https://github.com/pgoslatara)
...
## [#56679](https://github.com/rails/rails/pull/56679) Fix overlapping acronyms order dependency in underscore {#pr-56679}

**マージ日**: 2026/1/25 | **作成者**: [@Saidbek](https://github.com/Saidbek)

1. 概要 (1-2文で)  
Rails の `ActiveSupport::Inflector.underscore` が、重なり合う略語（例: `USD` と `USDC`）定義時に登録順によって結果が変わってしまう問題を修正した PR です。略語を正規表現に組み込む際に「長い略語を優先的にマッチさせる」ようにし、順序依存をなくしています。
...
- 対応 issue: https://github.com/rails/rails/issues/56675  
- 関連ドキュメント:  
  - ActiveSupport::Inflector の略語設定  
    ...
    ```  
- 正規表現のオルタネーション評価順 (Ruby 正規表現): 左から右に探索し、最初にマッチした分岐を採用する仕様に依存したバグ修正です。

---


## [#56678](https://github.com/rails/rails/pull/56678) Disable console query cache by default {#pr-56678}

**マージ日**: 2026/1/25 | **作成者**: [@camallen](https://github.com/camallen)
...
- このPR:  
  - https://github.com/rails/rails/pull/56678
- 関連PR（クエリキャッシュ挙動を変えた元の変更）:  
  - https://github.com/rails/rails/pull/56297
- 関連Issue（コンソールのクエリキャッシュ利用が問題になったIssue）:  
  - https://github.com/rails/rails/issues/56473

---


## [#56670](https://github.com/rails/rails/pull/56670) [ci skip] Fix guides for db:prepare {#pr-56670}

**マージ日**: 2026/1/25 | **作成者**: [@schmijos](https://github.com/schmijos)
...
- 該当実装（PR説明で示されているリンク先付近）  
  `activerecord/lib/active_record/tasks/database_tasks.rb`  
  https://github.com/rails/rails/blob/660fee087d4c92f0c7c31f11bc29b5f40e399caa/activerecord/lib/active_record/tasks/database_tasks.rb#L212-L213  

- 関連タスクの代表例:
  - `rails db:prepare`  
    - マイグレーション実行 + スキーマダンプ
  - `rails db:migrate`  
    - マイグレーション実行のみ（スキーマダンプは別タスク）
  - `rails db:schema:dump` / `rails db:structure:dump`  
    - スキーマ(または構造)だけをダンプするタスク

---


## [#56672](https://github.com/rails/rails/pull/56672) Add changelog for 6f9cbce21d8ec2de86c37b5a37073bac6d5f1d83 to Action Cable and Action Text {#pr-56672}

**マージ日**: 2026/1/24 | **作成者**: [@zzak](https://github.com/zzak)
...
## [#56666](https://github.com/rails/rails/pull/56666) Skip unique index lookup for insert_all! since it doesn't need conflict target {#pr-56666}

**マージ日**: 2026/1/23 | **作成者**: [@kirs](https://github.com/kirs)
...
## [#56652](https://github.com/rails/rails/pull/56652) Explicitly use Marshal serializer for MemCacheStore Dalli client {#pr-56652}

**マージ日**: 2026/1/23 | **作成者**: [@zzak](https://github.com/zzak)
...
- 動作としては元々 Marshal 前提だったため、**キャッシュフォーマット（Marshal でシリアライズされること）は従来通り**で、挙動自体が変わるわけではありません。
...
## [#56667](https://github.com/rails/rails/pull/56667) Use the released version of net-smtp in bug report templates {#pr-56667}

**マージ日**: 2026/1/23 | **作成者**: [@yahonda](https://github.com/yahonda)

1. 概要 (1-2文で)  
Rails のバグ報告テンプレート（`action_mailbox.rb` と `action_mailer.rb`）で、`net-smtp` の扱いを「リリース済み gem を使う」形に戻した PR です。以前の変更でテンプレート側に書かれていた一部設定を打ち消し、Gemfile 側の状態と整合を取っています。
...
- この PR で「部分的に revert」されているコミット:  
  - https://github.com/rails/rails/commit/d84afbb17c040162cb3a574276dae08d5f30c299
- Gemfile を元に戻したコミット:  
  - https://github.com/rails/rails/commit/acf7698858e6dbfee44260067b5d79b9bf4979b2
...
## [#56657](https://github.com/rails/rails/pull/56657) Prefer connected? for connection validity checks {#pr-56657}

**マージ日**: 2026/1/23 | **作成者**: [@matthewd](https://github.com/matthewd)
...
## [#56663](https://github.com/rails/rails/pull/56663) Upgrade from bundler 2.7.2 to 4.0.4 {#pr-56663}

**マージ日**: 2026/1/22 | **作成者**: [@larouxn](https://github.com/larouxn)
...
- **実行時挙動**
  ...
  - ただし、Bundler 4 は 2.x 系からいくつか動作や警告メッセージが変わっている可能性があるため、開発・CI 環境で `bundle install` や `bundle exec` の挙動がわずかに変わる可能性があります。
...
- 旧 Bundler:  
  - https://github.com/ruby/rubygems/releases/tag/bundler-v2.7.2
- Bundler 4 系リリース:  
  - https://github.com/ruby/rubygems/releases/tag/v4.0.0  
  - https://github.com/ruby/rubygems/releases/tag/v4.0.1  
  - https://github.com/ruby/rubygems/releases/tag/v4.0.2  
  - https://github.com/ruby/rubygems/releases/tag/v4.0.3  
  - https://github.com/ruby/rubygems/releases/tag/v4.0.4
- 解説ブログ:  
...
## [#56665](https://github.com/rails/rails/pull/56665) Add &block params to `ActionController` methods using yield {#pr-56665}

**マージ日**: 2026/1/22 | **作成者**: [@dduugg](https://github.com/dduugg)

1. 概要 (1-2文で)
- `ActionController` 内で `yield` や `block_given?` を使っているにもかかわらずメソッド引数に `&block` が明示されていなかった箇所に、すべて `&block` パラメータを追加した PR です。  
...
これらはいずれも内部で `block_given?` や `yield` を使っており、**元々ブロックを受け取れる実装になっていたが、シグネチャ上は&blockがなかった**、という状態でした。
...
- Rails コード上の近い例:  
  同じファイル内にすでに `each_pair(&block)` や `transform_keys(&block)` など、ブロックを明示した同系統メソッドが存在しており、それらとシグネチャを揃えたかたちです。

---


## [#56656](https://github.com/rails/rails/pull/56656) Update Dockerfile.test fixture to use Ruby 3.3 {#pr-56656}

**マージ日**: 2026/1/22 | **作成者**: [@Saidbek](https://github.com/Saidbek)
...
- この PR で参照されている Ruby 3.2 サポート削除 PR:  
  - https://github.com/rails/rails/pull/56511  
    - ここで Rails のサポート Ruby バージョンから 3.2 が除外され、最低要件が 3.3 に引き上げられています。
- Ruby 3.3 リリース情報:  
  - https://www.ruby-lang.org/ja/news/2023/12/25/ruby-3-3-0-released/  
    - Ruby 3.3 の新機能や変更点を確認することで、Rails を Ruby 3.3 上で動かす際の挙動差の理解に役立ちます。

---


## [#56660](https://github.com/rails/rails/pull/56660) Fix: Correct typo in routing guide [ci skip] {#pr-56660}

**マージ日**: 2026/1/22 | **作成者**: [@mutumagitonga](https://github.com/mutumagitonga)
...
## [#56661](https://github.com/rails/rails/pull/56661) Fix typos and minor errors in guides [ci skip] {#pr-56661}

**マージ日**: 2026/1/22 | **作成者**: [@eglitobias](https://github.com/eglitobias)
...
## [#56655](https://github.com/rails/rails/pull/56655) Detect bun JS package manager when using text-based lockfile {#pr-56655}

**マージ日**: 2026/1/22 | **作成者**: [@bai](https://github.com/bai)
...
- 影響範囲
  ...
  - すでに `bun.lock` を使っている Rails + bun プロジェクトでは、これまで `yarn` や `npm` など別のマネージャと誤認されていたケースが、今後は bun として処理される可能性があります。
...
- 対応する以前の PR:  
  - JS パッケージマネージャ検出を lockfile ベースに整理した PR:  
...
## [#56636](https://github.com/rails/rails/pull/56636) Detect JS package manager from lockfiles in generators {#pr-56636}

**マージ日**: 2026/1/21 | **作成者**: [@dfl](https://github.com/dfl)

1. 概要 (1-2文で)
Rails のジェネレータ（ActionText, ActionCable など）が、プロジェクト内の lockfile から JavaScript のパッケージマネージャ（bun / pnpm / npm / yarn）を自動判別してコマンドを実行するようになりました。これにより「常に yarn 前提」だった動作が解消され、npm や pnpm 等を使うプロジェクトでも手作業の修正が不要になります。
...
- `railties/test/generators/js_package_manager_test.rb`
  ...
  - いずれもないまたは `yarn.lock` があるときは yarn
  といった挙動が自動テストで保証されています。
...
- 複数の lockfile が混在しているような「汚れた」状態は想定外
  - たとえば `yarn.lock` と `package-lock.json` が共存している場合、実際の実装がどの順で探索しているかに依存します（PR 説明では bun → pnpm → npm → yarn の順と明記）。
...
## [#56552](https://github.com/rails/rails/pull/56552) [ci skip] Add sample code on API docs about Active Job Continuation Configuration {#pr-56552}

**マージ日**: 2026/1/21 | **作成者**: [@igaiga](https://github.com/igaiga)
...
## [#56250](https://github.com/rails/rails/pull/56250) Deduplicate logic for internal params handling in Action Controller log subscriber {#pr-56250}

**マージ日**: 2026/1/21 | **作成者**: [@lovro-bikic](https://github.com/lovro-bikic)
...
Rails 7.2 以降（PR #55334）で導入された structured events により、`ActionController::StructuredEventSubscriber` が `request_started` イベントを発火し、それを `ActionController::LogSubscriber` が受けてログ出力を行う流れになっています。
...
- **internal params フィルタの単一責務化**  
  ...
  - 除外条件の変更
  を行う場合は `StructuredEventSubscriber#start_processing` 周辺だけを見ればよくなります。  
...
- **カスタム Subscriber を書いている場合の影響**  
  ...
  - structured events を自前で listen している場合も、`request_started` の `:params` は「内部キーがすでに除外された状態の Hash」である、という前提で扱う必要があります。

- **テストへの影響**  
  - controller のログ出力を検証するテストで、
    - internal params のキーがログに含まれないこと
  を `LogSubscriber` の挙動として直接テストしている場合は、いまやその責務は `StructuredEventSubscriber` 側にあるため、テスト記述の意図を見直した方がよいです。  
...
- 該当 PR:  
  - [#56250 – Deduplicate logic for internal params handling in Action Controller log subscriber](https://github.com/rails/rails/pull/56250)
- 関連 PR（structured events の導入）:  
  - [#55334 – Add structured events for Action Controller](https://github.com/rails/rails/pull/55334)
- 実装箇所の参考リンク（PR 説明中で触れられているもの）:  
  - `ActionController::StructuredEventSubscriber#start_processing`  
    https://github.com/rails/rails/blob/1e1d8497a0fdad1f4bd3a6efbe7453217b4e56c5/actionpack/lib/action_controller/structured_event_subscriber.rb#L9-L12  
  - `request_started` イベントの emit と `LogSubscriber` 側のハンドリング  
    https://github.com/rails/rails/blob/1e1d8497a0fdad1f4bd3a6efbe7453217b4e56c5/actionpack/lib/action_controller/structured_event_subscriber.rb#L17-L22

---


## [#56401](https://github.com/rails/rails/pull/56401) [ci skip] Highlight Subscriber model changes {#pr-56401}

**マージ日**: 2026/1/21 | **作成者**: [@Tretent](https://github.com/Tretent)

1. 概要 (1-2文で)  
Railsガイド「wishlists.md」の中で、`Subscriber`モデルに対するコード変更部分にハイライト指定が漏れていた箇所を修正し、他モデルと同様に変更行が強調表示されるようにしたドキュメント専用のPRです。コードや挙動には一切影響せず、ガイドの可読性・一貫性のみを改善しています。
...
- 実際の変更内容  
  ...
  - 実コードの変更ではなく、**ガイド内のコード例表示用マークアップ**のみの変更です。
...
## [#56651](https://github.com/rails/rails/pull/56651) Fix comment doc in ActionText::Attachable [ci skip] {#pr-56651}

**マージ日**: 2026/1/21 | **作成者**: [@OutlawAndy](https://github.com/OutlawAndy)
...
## [#56647](https://github.com/rails/rails/pull/56647) Load minitest server plugin to run `minitest` with `--bisect` option {#pr-56647}

**マージ日**: 2026/1/20 | **作成者**: [@yahonda](https://github.com/yahonda)
...
- Minitest 6 では `minitest-bisect` が本体に同梱されましたが、Minitest 6 以降は「プラグインは opt-in（明示的にロードしないと有効にならない）」という方針になっています。  
  - 参考: https://github.com/minitest/minitest/commit/24071be0  
  - プラグイン opt-in 変更: https://github.com/minitest/minitest/blob/d5c88428ffb3786c69e2a7c0c7619457cafd4d4d/History.rdoc#L51-L54
...
- 関連 Issue: https://github.com/rails/rails/issues/56642  
- Minitest 本家での bisect 追加コミット:  
  https://github.com/minitest/minitest/commit/24071be0  
- プラグイン opt-in 方針の履歴:  
  https://github.com/minitest/minitest/blob/d5c88428ffb3786c69e2a7c0c7619457cafd4d4d/History.rdoc#L51-L54  
...
## [#56645](https://github.com/rails/rails/pull/56645) [8-1-stable] Handle Ruby 4.1 stabby lambda in Proc#source_location start_column {#pr-56645}

**マージ日**: 2026/1/19 | **作成者**: [@zzak](https://github.com/zzak)
...
- 背景となる Ruby 側の変更イメージ  
  Ruby 4.1 では、「stabby lambda（->）」で定義された `Proc` の `source_location` および `start_column` の扱いが従来と微妙に変わっています。具体的には、以下のようなコードでカラム位置（start_column）が、`->` の始点やブロック本体の始点をどのように数えるかが変わる、あるいは Ruby 側で明示的にサポートされるようになったと考えられます。
...
- Rails 側の対応内容（推測ベースの解説）  
  ActiveSupport のテスト関連アサーションは、ブロックや Proc の定義位置を検査するために `Proc#source_location` や `start_column` を用いています。  
...
- 注意点
  - Ruby 3.x 以前のバージョンでは、今回の変更は後方互換を壊さないように実装されていると考えられますが、もし自前で `Proc#source_location` や `start_column` を直接扱う独自アサーション・メタプログラミングをしている場合は、自分のコード側でも Ruby 4.1 の挙動差を確認しておくと安全です。
...
## [#56644](https://github.com/rails/rails/pull/56644) [8-0-stable] Handle Ruby 4.1 stabby lambda in Proc#source_location start_column {#pr-56644}
...
- 元 PR: [#56624](https://github.com/rails/rails/pull/56624)  
  - 今回の変更の本体。Ruby 4.1 の stabby lambda 対応がどのように実装されたかを確認できます。
- 対象 PR: [#56644](https://github.com/rails/rails/pull/56644)  
...
## [#56624](https://github.com/rails/rails/pull/56624) Handle Ruby 4.1 stabby lambda in Proc#source_location start_column {#pr-56624}
...
- Ruby 本体の変更:
  - https://github.com/ruby/ruby/commit/c970d2941d56a862bb9bb3b808cb588c2982f436
- Ruby のチケット:
  - https://bugs.ruby-lang.org/issues/21816
- 対象 PR:
  - Rails PR #56624: Handle Ruby 4.1 stabby lambda in Proc#source_location start_column
